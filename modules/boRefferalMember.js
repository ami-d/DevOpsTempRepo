...
var referralSearch = {
	getAdvancedSearchData : function(whereClause, firstName, lastName) {
		...
		var sql = "select m.MemberID as ServerMemberID, m.LastName, m.FirstName, m.ShippingCity,m.RegNumber, m.Phone1, s.StateABBR from " 
			+ tbname1 + " m left join " + tbname2 + " s on m.ShippingState = s.StateCode " + whereClause + ";";  
	 ...
	 getAdvancedSearchDataWSCallback: function(status, AdvanceSearchList, processOffline) {
        try {
            ...
            if (status == 400) {
                ...
                if (AdvanceSearchList["MembersList"] && AdvanceSearchList["MembersList"].length > 0) { //** MEG 6493
    ...
    displaySearchDataWS : function (response) {
		...
		if(refferalSearch_offline.length < NumOfMemberToShow && refferalSearch_WebService.length > 0){
			...
			for(var i=0; i < refferalSearch_WebService.length; i++)
			{
				...
				if(!isDuplicate)
					searchResultDataSet.push(refferalSearch_WebService[i]);
				if(searchResultDataSet.length >= NumOfMemberToShow)
					break;
			}
