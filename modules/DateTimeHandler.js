...
function supportTime(dateParam, hoursFormat, formatString) {
    ...
    if (hoursFormat == undefined || hoursFormat == "")
        hoursFormat = "24";
    if (formatString == undefined || formatString == "") {
        if (deviceLocale == "fr_FR") {
            if (hoursFormat == 12)
                formattedDate = formatDateTime(dateParam, frDateFormat12);
            else if (hoursFormat == 24)
                formattedDate = formatDateTime(dateParam, frDateFormat24);
        } else //if(deviceLocale=="en_US" || deviceLocale=="en_CA")
        {
            if (hoursFormat == 12)
                formattedDate = formatDateTime(dateParam, usDateFormat12);
            else if (hoursFormat == 24)
                formattedDate = formatDateTime(dateParam, usDateFormat24);
        }
    } else {
...
function getDateAfterXMonths(numberofMonths) {
    var fromdate = new Date();
    var dateAfterXMonth = new Date(fromdate.setMonth(fromdate.getMonth() + parseInt(numberofMonths)));
    var dd = dateAfterXMonth.getDate();
    var mm = dateAfterXMonth.getMonth() + 1;
...
function getLastDateOfMonth(expirationDate)
{
try{
	var mm = expirationDate.slice(2,4);
	kony.print("mm " + mm);
	var yy = expirationDate.slice(0,2);
	kony.print("yy " + yy);
	var tempDate = mm +"/01/"+yy;
	kony.print("tempDate " + tempDate);	
	var yyyy = new Date(tempDate).getFullYear();
	kony.print("yyyy " + yyyy);	
	var lastDateOfMonth = new Date(yyyy, mm , 0).getDate();
	kony.print("lastDateOfMonth " + lastDateOfMonth);
	var newDate = new Date(yyyy,mm-1,lastDateOfMonth)
	kony.print("newDate " + newDate);
	var formatedDate = supportTime(new Date(newDate), "", "yyyy-mm-ddTHH:NN:SS");
	kony.print("formatedDate " + formatedDate);	
	
    return (new Date(formatedDate) !== "Invalid Date") && !isNaN(new Date(formatedDate)) ? formatedDate : "";
...
{
	kony.print("Exception in getLastDateOfMonth " + err.toString());
	GlobalException(err);
}
...
/**
* Function added for calculating no of dayas between two different dates.
*/
