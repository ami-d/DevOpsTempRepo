    ...
    kony.print("glbLocationID------>>>" + glbLocationID);
     ...
     if(IsInfoSessionMeeting()) { //** MEG 7316
    	 // get members data for particular location id and display on home screen 
		  ...
		  var sql =  "SELECT ID as PreRegNumber,FirstName,LastName,Phone,MtngOccrID FROM PreRegMemberDetails where MeetingDate = '"+glbMeetingDate+"'  and MtngOccrID ='"+glbMeetingNum+"'"
 					+" UNION ALL "
          			+" SELECT PreRegNumber,FirstName,LastName,Phone1 as Phone,MtngOccrID FROM MemberDetails where  PreRegNumber not in (SELECT ID FROM PreRegMemberDetails) and PreRegNumber > 0"
          			+" and MtngOccrID ='"+glbMeetingNum+"' and RegStatus='" + MemberRegEnum.Pre_Registered + "'";
