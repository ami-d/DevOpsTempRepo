...
{
kony.print("ABA ---> setReEnrollSubcriptionData updatememberdetailsObject " +JSON.stringify(updatememberdetailsObject));		
 kony.print("ABA ---> setReEnrollSubcriptionData saleTransactionObj " +JSON.stringify(saleTransactionObj)); 
 if (glbFormName.lblSubType.text != kony.i18n.getLocalizedString("strPayg") && glbLocationTypeId != AT_WORK_LOCATION_TYPE_ID && !IsAWSLocationEnabled()) {
           ...
           boMeetings.getMeetingEndDate(function(meetingEndDate) {
              updatememberdetailsObject.ExpirationDate   = meetingEndDate;
          });
          	updatememberdetailsObject.CouponCode = updatememberdetailsObject.MtngOccrID;
           // updatememberdetailsObject.ExpirationDate = entryDate; //;
            updatememberdetailsObject.SubscriptnID =  0;
            updatememberdetailsObject.LastUsedDate = entryDate;
            updatememberdetailsObject.ProductID = (saleTransactionObj !== undefined && saleTransactionObj.length > 0) ? saleTransactionObj[0]['productDetail']['ProductID'] : 1;
            updatememberdetailsObject.SubscriptnType = "Series";
            updatememberdetailsObject.IsPAYG = "false";
            (checkValidString(gblLinkInfoForOnlineMember.IsLink) && updatememberdetailsObject.EnterpriseID != 0 && updatememberdetailsObject.EnterpriseID != "0") ? (updatememberdetailsObject.IsLink = gblLinkInfoForOnlineMember.IsLink) : (updatememberdetailsObject.IsLink = 'true');
...
function setWebsiteMembertData(updatememberdetailsObject, createWeightDetailsObject, createNoteDetailsObject, createBMINoteDetailsObject, updatedEmailFromMPActivated){
	kony.print("Link Type for IsWebSite Members");
            ...
            if(updatedEmailFromMPActivated != "" && updatedEmailFromMPActivated != undefined)
        		updatememberdetailsObject.Email = updatedEmailFromMPActivated;
