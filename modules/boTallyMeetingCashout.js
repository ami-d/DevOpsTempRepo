                  ...
                  if(data.length  == 2)
                    {
                      ...
                      var updateObj = {"BankDepositSlipNumber": ""};
               ...
               var amtData = [];
        		...
        		if(res.length > 0){
                    ...
                    amt = parseFloat(amt).toFixed(2);
                  	kony.print("amt " + amt);
                  		if(checkValidString(amt))
                          isCashChkAmtPrest = true;
                  
                  	kony.print("isCashChkAmtPrest " + isCashChkAmtPrest);
                  	kony.print("amt " + amt);
                  	amtData.push({"amtCheck" : amtCheck, "amtCash":amtCash,"amt":amt });
    ...
    getDepositeAmountFromSplitPayment:function(amtData,isCashChkPrest,callback){
        ...
        function getDepositeAmountFromSplitPaymentSuccessCallBack(res){
                ...
                kony.print("res.length "+res.length);
              	if(isCashChkPrest && res.length == 2 && amtData1.length > 0 && amtData1[0].amt > 0)
                 res.unshift({
        		...
        		var arr = [{'amount':amtData1[0].amt,'checks':amtData1[0].amtCheck, 'cash':amtData1[0].amtCash}];
