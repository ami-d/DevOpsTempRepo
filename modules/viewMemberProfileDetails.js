            ...
            if (responseObj != null) {
                ...
                if (gblSubType == "MonthlyPass" && !glbIsSelectedMemberExpired) {
                   ...
                   frmEditMemberProfile.imgEmail.isVisible = false;
                  	if (in_array(glbMemberType.toUpperCase(),lifeTimeTypes)){
                      	kony.print("7280 -- 1");
                        frmViewMemberProfile.lblExpDate.text = getLocalizedString("strSeriesAbbr"); //**MEG 7280
                        frmViewMemberProfile.lblExpDate.skin = "lblwwTextBook38px";
                        frmViewMemberProfile.imgMonthlyPassImage.src = "";
                      	kony.print("::HR:: inside series lifetime gblSubType = " + gblSubType + " gblExpirationDate = " + gblExpirationDate + " glbMemberType = " + glbMemberType );
                        
                    }else{
                            //frmViewMemberProfile.lblExpDate.skin = "lblwwTextBook38px";
                            frmViewMemberProfile.lblExpDate.text = getLocalizedString("strSeriesAbbr"); //strSeriesAbbr
                            frmViewMemberProfile.imgMonthlyPassImage.src = "";
                            kony.print("::HR:: not glbIsSelectedMemberExpired gblSubType = " + gblSubType + " gblExpirationDate = " + gblExpirationDate + " glbMemberType = " + glbMemberType );
                   }
				...
				if(checkValidString(responseObj[0].lblCurrentDPTData))
                	frmViewMemberProfile.lblCurrentDPTData.text = responseObj[0].lblCurrentDPTData;  //"0"//doubt
				
				
				if(checkValidString(responseObj[0].lblWPAData))
					frmViewMemberProfile.lblWPAData.text = responseObj[0].lblWPAData;
                ...
                } else if (responseObj[0].lblSubscriptionType == kony.i18n.getLocalizedString("strSeries") && !glbIsSelectedMemberExpired ) {
                    frmEditMemberProfile.lblSubTypeInfo.text = kony.i18n.getLocalizedString("strSeries");
                    hiddenMemberSubscription = kony.i18n.getLocalizedString("strSeries");
                }else {
...
function eventOnClickWeighIconProfilePage() {
    if (!IsNoMeetingSelected) {
        ...
        frmProcessMember.lblWeightLossFocusInfo.text = (selectedMemberData["WeightLossFocus"] === 0 || selectedMemberData["WeightLossFocus"] === '0' || selectedMemberData["WeightLossFocus"] === false || selectedMemberData["WeightLossFocus"] === 'false') ? kony.i18n.getLocalizedString("strLblNo") : kony.i18n.getLocalizedString("strLblYes");
...
function eventOnClickSegmentMemberProfileSearch() {
    kony.print("ABA --> Dileep --> Inside eventOnClickSegmentMemberProfileSearch");
    ...
    glbIsSelectedMemberExpired = checkExpirationDate(kony.sync.getString(selectedMemberData["ExpirationDate"]));
  	...
  	if(glbIsSelectedMemberExpired){
      	glbIsSelectedMemberExpired = checkGracePeriod(selectedMemberData["IsGracePeriod"]);
    }
    ...
    kony.print("ABA --> Dileep --> selectedMemberData = " + JSON.stringify(selectedMemberData));
    displayProgressView();
    ...
    kony.print("ABA  --> selectedMemberData[isOnlineSearch] :: " + selectedMemberData["isOnlineSearch"]);
...
function addMemberdataToLocaldb(v, isSegmentClicked) {
    ...
    createMemberDetailsObject.SessionNumber = checkUndefined(v.SessionNumber);
  	createMemberDetailsObject.WeightLossFocus = checkUndefined(v.WeightLossFocus);
    ...
    createMemberDetailsObject.PersonalGoalWeightDate = checkUndefined(v.PersonalGoalWeightDate);
  	...
  	createMemberDetailsObject.Usertype = checkUndefined(v.UserType);
...
function eventonclickToSaveweigh() {
    ...
    if (IsBatchAddMember == FlowForScreens.BatchAddMember) {
        ...
        if (error != "") {
			...
			tempWeightData.IsMemberAtRisk = false;//** MEG 6826
        ...
        if (error != "") {
            ...
            if (!IsUpdateWeightHistory) {
              ...
              var isMemberAtRiskFlag = false;
				if (in_array(deviceLocale,Countries["CA"])) {
                  if (popupAddWeigh.imgPopupCheckbox2.src == "atrisk_disable.png") {
    					 isMemberAtRiskFlag =  false;
                    kony.print("createWeightDetailsObject Setting flag AtRisk : false" );
     					 
   					  } else {
  				      	 isMemberAtRiskFlag = true;
                        kony.print("createWeightDetailsObject Setting flag AtRisk : true" );
 				    }	
                   
                   // createWeightDetailsObject.IsMemberAtRisk =isMemberAtRiskFlag;
                }
               createWeightDetailsObject.IsMemberAtRisk =isMemberAtRiskFlag;//** MEG 6826
              //**End
              kony.print("createWeightDetailsObject Weight update ismemberrisk flag ")
              ...
              if (in_array(deviceLocale,Countries["CA"])) {
                 ...
                 data = {
                    ...
                    isNWIAllowed: glbBatchAddToShowActionNWI,
                    isMemberAtRisk:isMemberAtRiskFlag
                };
              }else
                {
                  data = {
              ...
              //** MEG 6767
				...
				if (in_array(deviceLocale,Countries["CA"])) {
                  ...
                  if (popupAddWeigh.imgPopupCheckbox2.src == "atrisk_disable.png") {
                    ...
                    isMemberAtRiskFlag = false;
                    kony.print("updateObj Setting flag AtRisk : false" );
     					 
   					  } else {
                        isMemberAtRiskFlag = true;
                        kony.print("updateObj Setting flag AtRisk : true" );
 				    }	
                   // updateObj.IsMemberAtRisk = isMemberAtRiskFlag;
                }
             	 updateObj.IsMemberAtRisk = isMemberAtRiskFlag;//** MEG 6826
              //**End
                ...
                kony.print("ABA ---> updateWeightDetails At Risk " + JSON.stringify(updateObj));
...
function eventonRowClickSegWeighHistory() {
  kony.print("ABA ---> eventonRowClickSegWeighHistory");
    if (frmEditMemberProfile.segEditMemberProfileDetails.selectedIndex[1] != 0) {
      ...
      kony.print("ABA --- > eventonRowClickSegWeighHistory -- selectedIndexforUpdate :" + selectedIndexforUpdate);
        ...
        kony.print("ABA ----> Selected Data == " + JSON.stringify(frmEditMemberProfile.segEditMemberProfileDetails.selectedItems));
        ...
        if (selectedDate.setHours(0, 0, 0, 0) == today.setHours(0, 0, 0, 0)) {
          ...
          kony.print("ABA --- > In Else of date")
            ...
            if (popupAddWeigh != undefined && popupAddWeigh != null) {
          	...
          	showAtRiskImgOnPopup(frmEditMemberProfile.segEditMemberProfileDetails.selectedItems[0].isMemberAtRisk);  //** MEG 6767
            popupAddWeigh.show();
        }
    } else {
      kony.print("ABA --> Select first row");
        IsUpdateWeightHistory = false;
...
function eventonClickBatchSegWeighHistory() {
    ...
    popupAddWeigh.vboxPopupCheckAtRisk.setVisibility(false);//**MEG 6767
    popupAddWeigh.show();
}
...
//** MEG 6767
function eventonClickAtRiskCheckBoxOnAddWeightPopup() {
  if (popupAddWeigh.imgPopupCheckbox2.src == "atrisk_disable.png") {
    	popupAddWeigh.imgPopupCheckbox2.src = "atrisk.png";
       // IsAtRisk = true;
     } else {
        popupAddWeigh.imgPopupCheckbox2.src = "atrisk_disable.png";
       // IsAtRisk = false;
     }
}
//** End
//** MEG 6767
function showAtRiskImgOnPopup(isAtRiskFlag)
{
  
 if(!in_array(deviceLocale,Countries["CA"])) {
       popupAddWeigh.vboxPopupCheckAtRisk.setVisibility(false);
    } else //Added for MEGCN-14
    {
        popupAddWeigh.vboxPopupCheckAtRisk.setVisibility(true);
        kony.print("In canada");
        // Added to solve ‘At Risk’ checkbox reset on back
        if (isAtRiskFlag == true) {
             popupAddWeigh.imgPopupCheckbox2.src = "atrisk.png";
        } else {
             popupAddWeigh.imgPopupCheckbox2.src = "atrisk_disable.png";
        }
...
function onSelectingFromPopup() {
    ...
    } else if (option == "1") //1==Change Member Status
    {
        ...
        if (selectedMemberData["MemberStatus"] == MemberStatusEnum[4]) {
            ...
            popupChangeMemberStatus.dismiss(); //MEG 7228
    ...
    } else if (option == "2") //2==Change Member Type To Lifetime
    {
        ...
        if (isLifetime) {
            ...
            popupChangeMemberType.txtChangeMemberType.text = ""; 
          	popupChangeMemberType.dismiss(); //MEG 7228
           	popupEditMemberProfile.dismiss();//MEG 7228
            var context1 = {
    ...
    if (glbLocationTypeId == AT_WORK_LOCATION_TYPE_ID) {
        ...
        popupSubscriptionType.setContext(context);
      	if(IsAWSLocationEnabled()){
          	popupSubscriptionType.pickerSubType.masterData = getSubscriptionDropDownData(11);
        } else {
...
function UpdateMemberProfile() {
  	...
  	if(IsAWSLocationEnabled()){
      	if (( gblSubType == "PAYG" || gblSubType == "Pay as you go" ) && frmEditMemberProfile.lblSubTypeInfo.text == kony.i18n.getLocalizedString("strSeries")){
          alertForMessages(kony.i18n.getLocalizedString("strEditPAYGSeriesAlert"));
          return;
        }
      
      	if (( gblSubType == "MonthlyPass" || gblSubType == "Monthly Pass" ) && frmEditMemberProfile.lblSubTypeInfo.text == kony.i18n.getLocalizedString("strSeries")){
          alertForMessages(kony.i18n.getLocalizedString("strEditPAYGSeriesAlert"));
          return;
        }  
    }
    ...
    if (isValid) {
        if (frmEditMemberProfile.lblSubTypeInfo.text == "Monthly Pass") {
            boEnrollMember.checkForValidMP("", "", "", "", "", "", "", true, resMemberId, successMonthlyCheckInEditProfile);
        } else {
        ...
        function successMonthlyCheckInEditProfile() {
            ...
            //Added for 4513
            updateObj.PersonalGoalWeight = (frmEditMemberProfile.txtPersonalGoalWeight.text > 0) ? parseFloat(frmEditMemberProfile.txtPersonalGoalWeight.text).toFixed(1) : 0; //** MEG 6870
	    ...
	    if (glbIsWeightAdded) {
              	// Get latest weigh record for member to update the last attendance date
              	 boEnrollMember.getLastWeightDataOfMember(updateObj.MemberID, function(dataExists,res){
					if(dataExists){
						kony.print("Last weigh record found for member");
						updateObj.LastAttendanceDate = kony.sync.getString(res.WeighInDate);
					}else{
						kony.print("No weigh record Found to update last attendance date");
					}
				});
             ...
             */
          	//**MEG 6638
          	kony.print("linkChangeDetected " +linkChangeDetected);
          	kony.print("currentMemberLinkValues "+ JSON.stringify(currentMemberLinkValues));
            if (linkChangeDetected) {
          	...
          	// MEG-7415 - Added MP condition - If MP is changed for Member then only link data set to default
          	if (glbMeetingNum == "" && updateObj.SubscriptnType == "MonthlyPass") {
              	kony.print("glbCouponCode : "+glbCouponCode+ " currCouponCode : " +updateObj.CouponCode);
              	if(linkChangeDetected && (glbCouponCode == updateObj.CouponCode)){
                  	kony.print("Set update obj for link with curr member values in no meeting scenaios");
                    updateObj.IsLink = currentMemberLinkValues.IsLink;
                    updateObj.LinkType = currentMemberLinkValues.LinkType;
                    updateObj.EmailID = currentMemberLinkValues.EmailID;
                    updateObj.EnterpriseID = currentMemberLinkValues.EnterpriseID;
                    ...
                    kony.print("Set update obj for link with gbl values in no meeting scenaios");
                    updateObj.IsLink = gblLinkInfoForOnlineMember.IsLink;
                    updateObj.LinkType = gblLinkInfoForOnlineMember.LinkType;
                    updateObj.EmailID = gblLinkInfoForOnlineMember.EmailID;
                    updateObj.EnterpriseID =  gblLinkInfoForOnlineMember.EnterpriseID;
                    updateObj.UserName = gblLinkInfoForOnlineMember.UserName;
...
function validateMemberDetails() {
    ...
    if (glbLocationTypeId == AT_WORK_LOCATION_TYPE_ID) {
        res = valid
            .checkEnrollFirstName(frmEditMemberProfile.txtFirstName.text)
            .checkEnrollLastName(frmEditMemberProfile.txtLastName.text)
            .checkEmailForNonMandatory(frmEditMemberProfile.txtEmail.text)
            .validateDate(frmEditMemberProfile.lblDOBInfo.text)
            .heightValidation(frmEditMemberProfile.lblHeightInfo.text)
            .validateGoalWeight(parseFloat(frmEditMemberProfile.txtGoalWeight.text), parseFloat(startweight))
            .isValid();
    } else {
        res = valid
            .checkEnrollFirstName(frmEditMemberProfile.txtFirstName.text)
            ...
            .validateDate(frmEditMemberProfile.lblDOBInfo.text)
            .heightValidation(frmEditMemberProfile.lblHeightInfo.text)
            .checkForRequiredFields(frmEditMemberProfile.lblSubTypeInfo.text, "Subscription Type")
            .validateGoalWeight(parseFloat(frmEditMemberProfile.txtGoalWeight.text), parseFloat(startweight))
            .isValid();
    }
    ...
    if (frmEditMemberProfile.txtCity.text != "") {
        city = valid
            .checkCity(frmEditMemberProfile.txtCity.text)
            .isValid();
    }
    if (frmEditMemberProfile.txtNickName.text != "" && frmEditMemberProfile.txtNickName.text != undefined) {
        nickName = valid
            .checkNickName(frmEditMemberProfile.txtNickName.text)
            .isValid();
    }
    ...
    if (in_array(deviceLocale,Countries["CA"])) {
		if (frmEditMemberProfile.txtZip.text != "") {
	        postal = valid
	            .checkPostalCode(frmEditMemberProfile.txtZip.text)
	            .isValid();
	    }
	}else{
		if (frmEditMemberProfile.txtZip.text != "") {
	        postal = valid
	            .validateZipCode(frmEditMemberProfile.txtZip.text)
	            .isValid();
	    }
	}
    if (res && frmEditMemberProfile.lblSubTypeInfo.text != kony.i18n.getLocalizedString("strPayg") && frmEditMemberProfile.lblSubTypeInfo.text != kony.i18n.getLocalizedString("strSeries")) {
        var msgtype = "";
        ...
        expDate = valid
            .checkForRequiredFields(frmEditMemberProfile.lblExpDateInfo.text, getLocalizedString("strDateExp"))
            .validSubscriptiodIdLength(frmEditMemberProfile.txtSubID.text, msgtype)
            .isValid();
        //Ami add Start
        if (expDate) {
    ...
    if (frmEditMemberProfile.txtGoalWeight.text != "") {
        if (frmEditMemberProfile.txtGoalWeight.text == "0.0" || frmEditMemberProfile.txtGoalWeight.text == "0") {
...
function eventOnClickDoneChangeMemberStatus() {
    ...
    if (resMemberShipStatus != selectedStatus) {
        if((MemberStatusEnum[parseInt(currentstatusIndex) + 1]).toUpperCase() == 'INACTIVE' )
        	popupChangeMemberStatus.lblChangeMemberStatus.text = getLocalizedString("strInactive");
        else if((MemberStatusEnum[parseInt(currentstatusIndex) + 1]).toUpperCase() == 'ACTIVE' )
        	popupChangeMemberStatus.lblChangeMemberStatus.text = getLocalizedString("strActive");
...
function onPostShowViewMemberProfile(){
	...
	if(checkAppSettingEnable(Settings["DPT"])){
  	...
  	if(checkAppSettingEnable(Settings["NMILESTONE"])){
		frmViewMemberProfile.hbox12443534675379461.isVisible = false;
      	frmViewMemberProfile.hbox12443534675379471.isVisible = false;
	}else{
		frmViewMemberProfile.hbox12443534675379461.isVisible = true;
      	frmViewMemberProfile.hbox12443534675379471.isVisible = true;
	}
}
...
function getCCTransactionsForMember(flow,email){
	...
	if(flow == "updatedEmail"){
		...
		if((frmViewMemberProfile.lblEmailData.text != "") && (frmViewMemberProfile.lblEmailData.text != undefined))
			isMemberEmailExist = true;
		else
			isMemberEmailExist = false;
	}
	...
	if(isMemberEmailExist){
		boEnrollMember.getCCTranSalePaymentForMember("data", function(res){
			...
			if(res != null && res != undefined){
				var updatedData = _.uniq(res, function(x){ return x.SaleTransactnId; });
				kony.print("updatedData = "+JSON.stringify(updatedData));
				
				glbSendReceiptDisplayCounter = updatedData.length;
				
				for(var i=0; i<updatedData.length; i++){
					var salePaymentObj = _.filter(res, function (o) { return o.SaleTransactnId == updatedData[i].SaleTransactnId; });
					
					var isReturnTran = false;
					_.each(salePaymentObj, function(o) { return (o.RefundAmount == 1 || o.RefundAmount == true) ? isReturnTran = true : isReturnTran; });
					
					if(isReturnTran){
						var totalAmount = 0;
						_.each(salePaymentObj, function(o) { totalAmount = totalAmount + o.Amount; });
						salePaymentObj = _.filter(salePaymentObj, function (o) { return o.RefundAmount != 1; });
