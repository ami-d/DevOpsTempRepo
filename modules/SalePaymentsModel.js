...
// ****************Generated On Tue Mar 27 10:00:15 UTC 2018SalePayments*******************
...
com.kony.WeightWatchers.MemberSyncScope.SalePayments.prototype = {
	...
	set SalePaymentID(val){
		this._SalePaymentID = val;
	},
	get PaymentGateway(){
		return this._PaymentGateway;
	},
	set PaymentGateway(val){
		this._PaymentGateway = val;
	},
	get NoOfCheques(){
		return this._NoOfCheques;
	},
	set NoOfCheques(val){
		if(!kony.sync.isEmptyString(val) && !kony.sync.isNull(val) && !kony.sync.isValidNumberType(val)){
			sync.log.error("Invalid data type for the attribute NoOfCheques in SalePayments.\nExpected:\"int\"\nActual:\"" + kony.type(val) + "\"");
		}
		this._NoOfCheques = val;
	},
};
...
com.kony.WeightWatchers.MemberSyncScope.SalePayments.create = function(valuestable, successcallback,errorcallback,markForUpload){
	...
	if(kony.sync.enableORMValidations){
		errMsg = "SalePaymentID=" + valuestable.SalePaymentID;
		pks["SalePaymentID"] = {key:"SalePaymentID",value:valuestable.SalePaymentID};
		com.kony.WeightWatchers.MemberSyncScope.SalePayments.getAllDetailsByPK(pks,createSuccesscallback,errorcallback)
	}
	else{
		kony.sync.single_insert_execute(dbname,tbname,valuestable,successcallback,errorcallback,markForUpload);
	}
...
*		valuesArray[0].SalePaymentID = "SalePaymentID_0";
*		valuesArray[0].PaymentGateway = "PaymentGateway_0";
*		valuesArray[0].NoOfCheques = 0;
*		valuesArray[1] = {};
...
*		valuesArray[1].SalePaymentID = "SalePaymentID_1";
*		valuesArray[1].PaymentGateway = "PaymentGateway_1";
*		valuesArray[1].NoOfCheques = 1;
*		valuesArray[2] = {};
...
*		valuesArray[2].SalePaymentID = "SalePaymentID_2";
*		valuesArray[2].PaymentGateway = "PaymentGateway_2";
*		valuesArray[2].NoOfCheques = 2;
...
*		inputArray[0].whereClause = "where SalePaymentID = '0'";
*		inputArray[1] = {};
...
*		inputArray[1].whereClause = "where SalePaymentID = '1'";
*		inputArray[2] = {};
...
*		inputArray[2].whereClause = "where SalePaymentID = '2'";
...
com.kony.WeightWatchers.MemberSyncScope.SalePayments.filterAttributes = function(valuestable, insert){
	...
	attributeTable.SalePaymentID = "SalePaymentID";
	attributeTable.PaymentGateway = "PaymentGateway";
	attributeTable.NoOfCheques = "NoOfCheques";

	var PKTable = {};
	PKTable.SalePaymentID = {}
	PKTable.SalePaymentID.name = "SalePaymentID";
	PKTable.SalePaymentID.isAutoGen = false;
	var newvaluestable = {};
...
com.kony.WeightWatchers.MemberSyncScope.SalePayments.prototype.getValuesTable = function(isInsert){
	...
	if(isInsert===true){
		valuesTable.SalePaymentID = this.SalePaymentID;
	}
	valuesTable.PaymentGateway = this.PaymentGateway;
	valuesTable.NoOfCheques = this.NoOfCheques;
	return valuesTable;
};
...
com.kony.WeightWatchers.MemberSyncScope.SalePayments.prototype.getPKTable = function(){
	...
	var pkTable = {};
	pkTable.SalePaymentID = {key:"SalePaymentID",value:this.SalePaymentID};
	return pkTable;
};
...
com.kony.WeightWatchers.MemberSyncScope.SalePayments.pkCheck = function(pks,wcs,errorcallback,opName){
	...
	if(kony.sync.isNull(pks)){
		sync.log.error("Primary Key SalePaymentID not specified in  " + opName + "  an item in SalePayments");
		kony.sync.verifyAndCallClosure(errorcallback, kony.sync.getErrorTable(kony.sync.errorCodePrimaryKeyNotSpecified,kony.sync.getPrimaryKeyNotSpecifiedMsg("SalePaymentID",opName,"SalePayments")));
		return false;	
	}
	else if(kony.sync.isValidJSTable(pks)){
		if(!kony.sync.isNull(pks.SalePaymentID)){
			if(!kony.sync.isNull(pks.SalePaymentID.value)){
				wc.key = "SalePaymentID";
				wc.value = pks.SalePaymentID.value;
			}
			else{
				wc.key = "SalePaymentID";
				wc.value = pks.SalePaymentID;
			}
		}else{
			sync.log.error("Primary Key SalePaymentID not specified in  " + opName + "  an item in SalePayments");
			kony.sync.verifyAndCallClosure(errorcallback, kony.sync.getErrorTable(kony.sync.errorCodePrimaryKeyNotSpecified,kony.sync.getPrimaryKeyNotSpecifiedMsg("SalePaymentID",opName,"SalePayments")));
...
com.kony.WeightWatchers.MemberSyncScope.SalePayments.checkPKValueTables = function (valuetables)	{
