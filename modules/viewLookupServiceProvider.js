...
function onclickSaveEmpPopupBtn(){
	 	...
	 	if(result && result.length > 0){
		 	...
		 	if(res){
				 ...
				 createWeightDetailObject.WeeklyPointsAllowance = parseInt(0);	
              	 createWeightDetailObject.SessionNumber = parseInt(1);
	             createWeightDetailObject.ManualWeighIn = false;
              	 createWeightDetailObject.IsMemberAtRisk = "0";	
	 ...
	 function onsuccessCreateWeight(){
    	...
    	saleDetailsObject.IsPreActivated = false;
    ...
    function onsuccessSaleItem(){
        ...
        salePaymentObject.SalePaymentID = gblDeviceID + "_" + generateSaleItemID();
