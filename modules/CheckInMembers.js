    ...
    getFinalCheckedInResult: function() {
        ...
        if (weightData.length > 0) {
            ...
            if (finalObj.length > 0) {
              kony.print(" Preactivation retrieval of final object - Step pre 2 ");
                for (k in finalObj) {
                    var mName = "";
...
//                   kony.print(" Preactivation retrieval of final object - iteration of each object - Step pre 3 ");
//                   setDataForActivationIcons("",0,finalObj);
//                 }
            }
        }
...
function setDataForActivationIcons(imagename, index, finalObj){
      kony.print(" Preactivation retrieval of final object - setdata method - Step pre 4 ");
      kony.print("::SS::finalObj::" + JSON.stringify(finalObj));
      if (index > 0) {
         kony.print("::SS::index::" + index);
         kony.print(" Preactivation retrieval of final object - set image name - Step pre 9 ");
         finalObj[index-1]["ActivationStatusIcon"] = imagename;
      }
      
      if (finalObj[index] != undefined || finalObj[index] != null) {
         kony.print("::SS::finalObj-index::" + JSON.stringify(finalObj));
         kony.print("::SS::index-finalObj::" + index);
              kony.print(" Preactivation retrieval of final object - setdata method - individual call - Step pre 5 ");
         getActivationStatus(finalObj[index]["MemberID"],finalObj[index]["CouponCode"], index, finalObj, setDataForActivationIcons);
      }
      else{
      	return finalObj;   
      }  
...
// function getActivationStatus(memberID,couponCode,index, finalObj,callback)
// {
// 		kony.print("IN getActivationStatus");
// 		                  kony.print(" Preactivation retrieval of final object - getActStatus method - Step pre 6 ");

// 		var returnImageName = "";
  
//    		function getActivationStatusErrorCallback(err)
//         {
//                                   		                  kony.print(" Preactivation retrieval of final object - getActStatus success callBack - error callback - Step pre 8 ");
// 			callback.call(null,returnImageName, index + 1,finalObj);
//         }
// 		function getActivationStatusSuccCallback(res)
// 		{
//           		                  kony.print(" Preactivation retrieval of final object - getActStatus success callBack - Step pre 7 ");
...
//                 kony.print("icon set checkinMem 1------>>>",ActivationStatus);
...
// 						break;
// 				}
//                         		                  kony.print(" Preactivation retrieval of final object - getActStatus success callBack - increment - Step pre 8 ");
//               callback.call(null,returnImageName, index + 1,finalObj);
// 			}
// 			else{
// 			                        		                  kony.print(" Preactivation retrieval of final object - getActStatus success callBack - no rec found - Step pre 8 ");
...
//               	function getFailedVoucherInfoErrorCallback(err)
//                 {
//                     callback.call(null,returnImageName, index + 1,finalObj);
//                 }
...
// 			         if(isFailedVaocherInfo == true)
// 			            returnImageName = 'icn_mp_v_green.png';
// 			         }
//                   callback.call(null,returnImageName, index + 1,finalObj);
// 			   }
			  
// 			   kony.sync.single_select_execute(kony.sync.getDBName(), whereCondition, null, getFailedVoucherInfoSuccessCallback, getFailedVoucherInfoErrorCallback);
...
// 		DBPreActivationController.find(whrCondition, getActivationStatusSuccCallback, getActivationStatusErrorCallback);
...
function getActivationStatus(memberID,couponCode)
{
		...
		var whrCondition = "where MemberID='"+memberID+"' and CouponCode = '"+couponCode+"' and MtngOccrID = '" + glbMeetingNum + "' and MeetingDate = '"+ glbMeetingDate + "'";
	        	   		...
	        	   		if (res.length > 0) {
	                		for (i in res) {
	                    		...
	                    		kony.print("IN IF=" + keyObj["MemberID"] +"--"+keyObj.MemberID);
	                    		                kony.print("icon set checkinmem 2------>>>");
...
function showMpActivatedMemberData() {
  ...
  function customErrorCallback(err){
    //kony.print("::custom error:"+JSON.stringify(res)); MEG-6799
    		kony.print("::custom error object is:"+JSON.stringify(err));
   			
  }
  
	kony.print("::Sql::----" + query);
  
    kony.sync.single_select_execute(kony.sync.getDBName(), query, null, getMPMemberSuccessCallback, customErrorCallback);
}
				...
				if(v.ActivationStatus.trim() == ""){
					...
					status = getLocalizedString("strLblActivationFailed");
					 ...
					 b = mapKeys(viewMPMembers, {
			              ...
			              btnIssueVoucher: (v.ActivationStatus == "true" || v.ActivationStatus == "Voucher Issued" /*|| v.ActivationStatus.trim() == ""*/) ? {text: "", isVisible: false } : { text: kony.i18n.getLocalizedString("strLblIssueVoucher"), isVisible:true}		         
		           		 });
...
function onClikCancelMPactivatePopup(){
	...
	popupMPActivation.dismiss(); //**MEG 7332// close the popup
}
