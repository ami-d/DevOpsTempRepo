...
Network.makeServiceCall = function(inputParam, callBack, processOffline) {
    try {
		...
		if(_.find(listOfServicesNotDisplayLoading,function(serviceName){ return (serviceName == inputParam.serviceID); }))
        ...
        function handleCallback(status, response) {
            ...
            try {
                ...
                if(_.find(listOfServicesNotDisplayLoading,function(serviceName){ return (serviceName == inputParam.serviceID); })){
                	if(kony.application.getCurrentForm().id == frmViewMemberProfile.id && inputParam.serviceID == Services.SaleReceipt)
                		removeProgressView();
                	else
