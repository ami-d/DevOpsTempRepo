...
function getProgramDuration() {
    if (glbFormName.lblSubType.text == kony.i18n.getLocalizedString("str3MPBuy")) return 3;
    else if (glbFormName.lblSubType.text == kony.i18n.getLocalizedString("str6MPBuy")) return 6;
    else if (glbFormName.lblSubType.text == kony.i18n.getLocalizedString("strMPBuyNew")) return 1;
        ...
        return 1;
    else if (glbFormName.lblSubType.text == kony.i18n.getLocalizedString("strLTC3")) //** MEG-6958
        return 1;
    else return 0;
}
...
function convertPriceToSaveByLocale(str) {
    //fr_CA price format changes
    if (deviceLocale == 'fr_CA') {
        if (str != "" && str != undefined && str != null) str = str.replace(",", ".");
    }
    return str;
}
...
function ClearTangibleProductsSegments(all) {
    if (all) {
        ...
        if (in_array(deviceLocale, Countries["CA"])) frmDirectSaleProductList.lblCatType.text = getLocalizedString("strAllProduct");
        else frmDirectSaleProductList.lblCatType.text = getLocalizedString("strFeatured");
        glbMembershipType = "";
        ...
        if (in_array(deviceLocale, Countries["CA"])) frmAddAndWeighMemberTransaction.lblCatType.text = getLocalizedString("strAllProduct");
        else frmAddAndWeighMemberTransaction.lblCatType.text = getLocalizedString("strFeatured");
        shoppingcart = [];
    } else {
...
function convertEnumToArray(Enum, callback) {
    ...
    for (var prop in Enum) {
        if (Enum.hasOwnProperty(prop)) arr.push([prop, Enum[prop]]);
    }
...
function updateMemberDetailsSetDefaults(updatememberdetailsObject, callback) {
    ...
    function getMembersDetailsForUpdateRequest(res) {
        ...
        if (res.length > 0) {
            ...
            MemberValuesFromDB.IsPAYG = checkValidString(res[0].IsPAYG) ? res[0].IsPAYG : 'true'; //MEG-7033 - To fix SubscriptionType blank issue change flag to 'true' 
            ...
            MemberValuesFromDB.CouponCode = checkValidString(res[0].CouponCode) ? res[0].CouponCode : ""; //MEG-7033
            ...
            updatememberdetailsObject.MemberRole = (checkValidString(memberInfo.MemberRole)) ? memberInfo.MemberRole : MemberValuesFromDB.MemberRole;
            updatememberdetailsObject.EnterpriseID = (checkValidString(memberInfo.EnterpriseID) && memberInfo.EnterpriseID != 0) ? memberInfo.EnterpriseID : MemberValuesFromDB.EnterpriseID;
            updatememberdetailsObject.EmailID = (checkValidString(memberInfo.EmailID)) ? memberInfo.EmailID : MemberValuesFromDB.EmailID;
            updatememberdetailsObject.LinkType = (checkValidString(memberInfo.LinkType)) ? memberInfo.LinkType : MemberValuesFromDB.LinkType;
            updatememberdetailsObject.UserName = (checkValidString(memberInfo.UserName)) ? memberInfo.UserName : MemberValuesFromDB.UserName;
            updatememberdetailsObject.IsLink = (checkValidString(memberInfo.IsLink) && checkValidString(memberInfo.EnterpriseID) && memberInfo.EnterpriseID != "0") ? memberInfo.IsLink : "true";
            updatememberdetailsObject.IsFreshStart = (checkValidString(memberInfo.IsFreshStart)) ? memberInfo.IsFreshStart : MemberValuesFromDB.IsFreshStart;
            updatememberdetailsObject.RefreshDate = (checkValidString(memberInfo.RefreshDate)) ? memberInfo.RefreshDate : MemberValuesFromDB.RefreshDate;
            updatememberdetailsObject.EmpID = (checkValidString(memberInfo.EmpID)) ? memberInfo.EmpID : MemberValuesFromDB.EmpID;
            //MEG-7033
            if (checkValidString(memberInfo.IsPAYG)) {
                updatememberdetailsObject.IsPAYG = memberInfo.IsPAYG;
            } else {
                if (checkValidString(MemberValuesFromDB.CouponCode)) {
                    updatememberdetailsObject.IsPAYG = "false";
                } else {
                    updatememberdetailsObject.IsPAYG = MemberValuesFromDB.IsPAYG;
                }
                updatememberdetailsObject.CouponCode = (checkValidString(memberInfo.CouponCode)) ? memberInfo.CouponCode : MemberValuesFromDB.CouponCode;
            }
            updatememberdetailsObject.BillingState = (checkValidString(memberInfo.BillingState)) ? memberInfo.BillingState : MemberValuesFromDB.BillingState;
            updatememberdetailsObject.ShippingState = (checkValidString(memberInfo.ShippingState)) ? memberInfo.ShippingState : MemberValuesFromDB.ShippingState;
            updatememberdetailsObject.WeeksCompleted = (checkValidString(memberInfo.WeeksCompleted)) ? memberInfo.WeeksCompleted : MemberValuesFromDB.WeeksCompleted;
            updatememberdetailsObject.SubscriptnID = (checkValidString(memberInfo.SubscriptnID) || (memberInfo.SubscriptnID === 0)) ? memberInfo.SubscriptnID : MemberValuesFromDB.SubscriptnID;
            // updatememberdetailsObject.CouponCode = (checkValidString(memberInfo.CouponCode)) ? memberInfo.CouponCode :MemberValuesFromDB.CouponCode;
            updatememberdetailsObject.MemberID = (checkValidString(memberInfo.MemberID)) ? memberInfo.MemberID : glbMemberId;
            updatememberdetailsObject.ProductID = (checkValidString(memberInfo.ProductID)) ? memberInfo.ProductID : MemberValuesFromDB.ProductID;
            updatememberdetailsObject.TrackerID = (checkValidString(memberInfo.TrackerID)) ? memberInfo.TrackerID : MemberValuesFromDB.TrackerID;
            updatememberdetailsObject.MaintenanceMode = (checkValidString(memberInfo.MaintenanceMode) || (memberInfo.MaintenanceMode === 0)) ? memberInfo.MaintenanceMode : MemberValuesFromDB.MaintenanceMode;
            updatememberdetailsObject.TrackerStartDate = (checkValidString(memberInfo.TrackerStartDate)) ? memberInfo.TrackerStartDate : MemberValuesFromDB.TrackerStartDate;
            updatememberdetailsObject.FailedDate = (checkValidString(memberInfo.FailedDate)) ? memberInfo.FailedDate : MemberValuesFromDB.FailedDate;
            updatememberdetailsObject.Eligible = (checkValidString(memberInfo.Eligible)) ? memberInfo.Eligible : MemberValuesFromDB.Eligible;
            updatememberdetailsObject.EligibleDate = (checkValidString(memberInfo.EligibleDate)) ? memberInfo.EligibleDate : MemberValuesFromDB.EligibleDate;
            updatememberdetailsObject.WeightCountMet = (checkValidString(memberInfo.WeightCountMet)) ? memberInfo.WeightCountMet : MemberValuesFromDB.WeightCountMet;
            updatememberdetailsObject.PaidLastFee = (checkValidString(memberInfo.PaidLastFee)) ? memberInfo.PaidLastFee : MemberValuesFromDB.PaidLastFee;
            updatememberdetailsObject.SessionNumber = (checkValidString(memberInfo.SessionNumber)) ? memberInfo.SessionNumber : MemberValuesFromDB.SessionNumber;
            updatememberdetailsObject.RedeemedDate = (checkValidString(memberInfo.RedeemedDate)) ? memberInfo.RedeemedDate : MemberValuesFromDB.RedeemedDate;
            updatememberdetailsObject.ReedeemedPasses = (checkValidString(memberInfo.ReedeemedPasses)) ? memberInfo.ReedeemedPasses : MemberValuesFromDB.ReedeemedPasses;
            updatememberdetailsObject.MissWeekPasses = (checkValidString(memberInfo.MissWeekPasses)) ? memberInfo.MissWeekPasses : MemberValuesFromDB.MissWeekPasses;
            updatememberdetailsObject.IsDateRedeemed = (checkValidString(memberInfo.IsDateRedeemed)) ? memberInfo.IsDateRedeemed : MemberValuesFromDB.IsDateRedeemed;
            updatememberdetailsObject.UserStsChngRsn = (checkValidString(memberInfo.UserStsChngRsn)) ? memberInfo.UserStsChngRsn : MemberValuesFromDB.UserStsChngRsn;
            updatememberdetailsObject.UserStsEndPrd = (checkValidString(memberInfo.UserStsEndPrd)) ? memberInfo.UserStsEndPrd : MemberValuesFromDB.UserStsEndPrd;
            updatememberdetailsObject.LastAttendanceDate = (checkValidString(memberInfo.LastAttendanceDate)) ? memberInfo.LastAttendanceDate : MemberValuesFromDB.LastAttendanceDate;
...
function checkIfMemberAlreadyExistsOnServer(RegNumber, memberID, callback) {
    ...
    //** MEG 6493
    if (JsonService) {
        ValidateRegNumber_inputparam["httpheaders"] = setRESTHeader();
    } else {
        ...
        try {
            ...
            if (status == 400) {
                ...
                if (validateRegNumberResponse["opstatus"] == 0) {
                    isValid = (validateRegNumberResponse.SearchMemberResponse && validateRegNumberResponse.SearchMemberResponse[0]["IsUsed"].toString() === "0") ? true : false;
...
function checkForMemberInCompleteProfile(memberObj, isEditProfile, isProcess) {
    ...
    if (isEditProfile) {
        inCompleteFlag = (checkValidString(memberObj.FirstName.trim()) && checkValidString(memberObj.LastName.trim()) && checkValidString(memberObj.RegNumber.trim()) && checkValidString(memberObj.Gender.trim()) && checkValidString(memberObj.ShippingAddr1) && checkValidString(memberObj.ShippingState) && checkValidString(memberObj.ShippingCity) && checkValidString(memberObj.ShippingZipCode) && (memberObj.Height > 0));
    } else if (isProcess) {
        inCompleteFlag = (checkValidString(selectedMemberData.lblMPSFirstName.trim()) && checkValidString(selectedMemberData.lblMPSLastName.trim()) && checkValidString(memberObj.RegNumber.trim()) && checkValidString(selectedMemberData.Gender.trim()) && (checkValidString(memberObj.ShippingAddr1) || checkValidString(selectedMemberData.lblMPSStreet)) && (checkValidString(memberObj.ShippingState) || checkValidString(selectedMemberData.lblMPSState)) && (checkValidString(memberObj.ShippingCity) || checkValidString(selectedMemberData.lblMPSCity)) && (checkValidString(memberObj.ShippingZipCode) || checkValidString(selectedMemberData.ShippingZipCode)) && checkValidString(selectedMemberData.MembershipType) && (memberObj.Height > 0));
    } else {
        inCompleteFlag = (checkValidString(memberObj.FirstName.trim()) && checkValidString(memberObj.LastName.trim()) && checkValidString(memberObj.RegNumber.trim()) && checkValidString(memberObj.Gender.trim()) && checkValidString(memberObj.ShippingAddr1) && checkValidString(memberObj.ShippingState) && checkValidString(memberObj.ShippingCity) && checkValidString(memberObj.ShippingZipCode) && checkValidString(memberObj.MemberType) && (memberObj.Height > 0));
    }
