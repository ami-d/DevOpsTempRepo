...
var boMeetings = {
    getMeetingsByLocationId: function(locationId) {
         ...
         */
        strWhere = "SELECT M.ID,M.LineOfBsinessID,M.DTCTime,M.IsAtWork,M.SeriesCompMeetings,M.IsTallySubmitted,M.IsInfoSession, " + "CASE M.MeetingStatus WHEN 'Submitted' THEN 'Disable' " + "ELSE CASE M.TallyStatus WHEN 'LeaderSignOff' THEN 'Close' ELSE IFNULL(MI.MeetingStatus, 'Open') END END AS MeetingStatus, " + "M.MeetingTypeID, M.isTimesheetModified, M.DayTimeCodeID, M.DayCodeID, M.MeetingDate, M.BackOfficeRefID, M.WeekNo, M.StartDate " + "from Meetings M LEFT JOIN MeetingInstance MI ON MI.MeetingOccID == M.ID AND " + "MI.MeetingDate like '%" + dateQuery + "%' AND MI.locationID = " + locationId + " " + "Where M.CountryID = '" + getCountryID() + "' AND M.locationID = " + locationId + " and M.MeetingDate like '%" + dateQuery + "%'";
    ...
    getWeekNoForAtWorkMeetingClosed: function(meetingID, meetingDate, callback) {
        ...
        var strWhere = "Select  ifnull(M2.WeekNo,M1.SeriesCompMeetings)  as WeekNo from Meetings M1 left join MeetingsUpdate M2 on  " + "M1.ID = M2.ID  where M1.ID ='" + meetingID + "' and M1.MeetingDate like '%" + meetingDate + "%'";
