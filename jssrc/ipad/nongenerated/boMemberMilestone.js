...
var boMemberMilestone = {
        ...
        try {
            ...
            function getAchievedMilestonesForMemberSuceessCallback(res) {
                ...
                if (res.length > 0) {
                    ...
                    //** MEG 6493
                    if (JsonService) {
                        GetMemberMilestoneDetails_inputparam["httpheaders"] = setRESTHeader();
                    } else {
                        GetMemberMilestoneDetails_inputparam["deviceID"] = gblDeviceID;
                        GetMemberMilestoneDetails_inputparam["accessToken"] = glbSPAuthToken;
                        GetMemberMilestoneDetails_inputparam["SPID"] = glbEmployeeId;
    ...
    getMemberMilestoneDetailsWSCallback: function(status, GetMemberMilestoneDetails) {
        try {
            ...
            if (status == 400) {
                ...
                if (GetMemberMilestoneDetails["AchievedMilestones"] && GetMemberMilestoneDetails["AchievedMilestones"].length > 0) { //** MEG 6493
                    ...
                    for (var milestoneIterator = 0; milestoneIterator < GetMemberMilestoneDetails["AchievedMilestones"].length; milestoneIterator++) {
                        if (GetMemberMilestoneDetails != null && GetMemberMilestoneDetails != undefined) {
                            ...
                            tempObj["WeekNumber"] = getWeekNumber(GetMemberMilestoneDetails["AchievedMilestones"][milestoneIterator]["AchWeighInDate"].split('T')[0]);
                            // Dileep Chejerla - MEG-2716
                            tempObj["MemberMilestoneID"] = GetMemberMilestoneDetails["AchievedMilestones"][milestoneIterator]["AchMemberMilestoneID"];
                ...
                if (GetMemberMilestoneDetails["EligibleMilestones"] && GetMemberMilestoneDetails["EligibleMilestones"].length > 0) { //** MEG 6493
    ...
    getMasterMilestones: function(callback, weightData, startwt, goalwt, isFromBatchAdd, doCreateEligble) {
        var strWhere = "where 1=1 AND (date(StartDate) <= date('now','localtime') OR UPPER(StartDate) = 'NULL') AND (date(EndDate) > date('now','localtime') OR UPPER(EndDate) = 'NULL')";
    ...
    getEligibleMilestoneByMemberId: function(callback, weighDataArr, startwt, goalwt, lastwt) {
        var sql = "select me.*,ml.MilestoneName from MilestoneEligible me,MilestoneLookUp ml on me.MilestoneID = ml.MilestoneID and ml.CountryID = '" + getCountryID() + "' where me.MemberID like '" + glbMemberId + "' AND (date(ml.StartDate) <= date('now','localtime') OR UPPER(ml.StartDate) = 'NULL') AND (date(ml.EndDate) > date('now','localtime') OR UPPER(ml.EndDate) = 'NULL') and ml.CountryID='" + getCountryID() + "'";;
    ...
    checkWeightDetailsForLifetimeMemberEligibility: function(memberId, goalWeight, goalAchDate, callback) {
        ...
        function checkWeightDetailsForLifetimeMemberEligibilitySuccessCallback(res) {
            ...
            if (isMaintainace) boMemberMilestone.checkWeightEntriesTillSixthWeek(memberId, goalAchDate, goalWeight, callback);
            else {
                callback.call(null);
            }
        }
