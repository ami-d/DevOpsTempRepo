...
function eventOnSavePreRegMember() {
    ...
    var res = valid.checkEnrollFirstName(frmPreRegAddMember.txtFirstName.text).checkEnrollLastName(frmPreRegAddMember.txtLastName.text).heightValidation(frmPreRegAddMember.lblHeightInfo.text).validateDate(frmPreRegAddMember.lblDOBInfo.text).checkForRequiredFields(frmPreRegAddMember.txtAddr1.text, "", "strValidAddress1").checkForRequiredFields(frmPreRegAddMember.lblStateInfo.text, "", "strSelectState").checkEmail(frmPreRegAddMember.txtEmail.text).CheckTenDigitPhone(frmPreRegAddMember.txtPhoneNo.text).isValid();
    if (frmPreRegAddMember.txtCity.text != "") {
        city = valid.checkCity(frmPreRegAddMember.txtCity.text).isValid();
    }
    if (frmPreRegAddMember.txtZip.text != "") {
        zipcode = valid.validateZipCode(frmPreRegAddMember.txtZip.text).isValid();
    }
    if (frmPreRegAddMember.txtNickName.text != "") {
        nickName = valid.checkNickName(frmPreRegAddMember.txtNickName.text).isValid();
    }
    if (res && city && nickName && zipcode) {
        ...
        function createPreRegMemberSuccessCallback(res) {
            ...
            if (isSaveAndAddPreRegMember) frmPreRegAddMember.show();
            else frmHomeScreen.show();
            isSaveAndAddPreRegMember = false;
        }
