    ...
    if (!in_array(deviceLocale, Countries["CA"])) {
        popupSubscriptionType.pickerSubType.masterData = [
            [
                ...
                ["15-1-1", kony.i18n.getLocalizedString("strLTC3")],
...
function onPostShowfrmAddBatchMember() {
    kony.print("ABA --> PostShow frmBatchAddMember  onPostShowfrmAddBatchMember");
...
function eventOnclickSaveBtn() {
    ...
    if (isPayAsYouGo) {
        var res = valid.checkEnrollFirstName(frmBatchAddMember.txtFirstName.text).checkEnrollLastName(frmBatchAddMember.txtLastName.text)
            //Ami MEG-2586 
            .checkForRequiredFields(frmBatchAddMember.lblMemberType.text, "Membership type").checkForRequiredFields(frmBatchAddMember.lblMemberStatus.text, "Status").heightValidation(frmBatchAddMember.lblHeightInfo.text).validateDate(frmBatchAddMember.lblDOBInfo.text).validateWeight(frmBatchAddMember.txtStartWeight.text).validateStartDate(frmBatchAddMember.lblStartDateInfo.text, frmBatchAddMember.lblDOBInfo.text).checkMemberID(frmBatchAddMember.txtMemberId.text).checkForMonthlyPassForBatch(glbSelectSubType, frmBatchAddMember.txtEmail.text, frmBatchAddMember.lblSubTypeInfo.text).CheckRemainingMissedWeekPasses(frmBatchAddMember.txtWeeksCompleted.text, 0).CheckRemainingMissedWeekPasses(frmBatchAddMember.txtRemainingMissedWeek.text, 1).isValid();
        SubRes = true;
    } else {
        ...
        var msgtype = "";
        if (frmBatchAddMember.lblSubTypeInfo.text == getLocalizedString("strMonthlyPass") || frmBatchAddMember.lblSubTypeInfo.text == getLocalizedString("str3MonthPass") || frmBatchAddMember.lblSubTypeInfo.text == getLocalizedString("str6MonthPass") || frmBatchAddMember.lblSubTypeInfo.text == getLocalizedString("strLTC6") || frmBatchAddMember.lblSubTypeInfo.text == getLocalizedString("strLTC3")) {
            msgtype = "MP";
        } else if (frmBatchAddMember.lblSubTypeInfo.text == getLocalizedString("str20WkPass")) {
            msgtype = "20WP";
        }
        var res = valid.checkEnrollFirstName(frmBatchAddMember.txtFirstName.text).checkEnrollLastName(frmBatchAddMember.txtLastName.text).checkForRequiredFields(frmBatchAddMember.lblMemberType.text, "Membership Type").checkForRequiredFields(frmBatchAddMember.lblMemberStatus.text, "Status").heightValidation(frmBatchAddMember.lblHeightInfo.text).validateDate(frmBatchAddMember.lblDOBInfo.text).validateDate(frmBatchAddMember.lblExpDateInfo.text, "strExpirationDate").validSubscriptiodIdLength(frmBatchAddMember.txtSubID.text, msgtype).validateWeight(frmBatchAddMember.txtStartWeight.text).validateStartDate(frmBatchAddMember.lblStartDateInfo.text, frmBatchAddMember.lblDOBInfo.text).checkMemberID(frmBatchAddMember.txtMemberId.text).checkForMonthlyPassForBatch(glbSelectSubType, frmBatchAddMember.txtEmail.text, frmBatchAddMember.lblSubTypeInfo.text).CheckRemainingMissedWeekPasses(frmBatchAddMember.txtWeeksCompleted.text, 0).CheckRemainingMissedWeekPasses(frmBatchAddMember.txtRemainingMissedWeek.text, 1).isValid();
    ...
    if (frmBatchAddMember.txtCity.text != "") {
        city = valid.checkCity(frmBatchAddMember.txtCity.text).isValid();
    }
    if (frmBatchAddMember.txtBillingCity.text != "") {
        cityBill = valid.checkCity(frmBatchAddMember.txtBillingCity.text).isValid();
    }
    if (frmBatchAddMember.txtZip.text != "") {
        zipcode = valid.validateZipCode(frmBatchAddMember.txtZip.text).isValid();
    }
    if (frmBatchAddMember.txtNickName.text != "") {
        nickName = valid.checkNickName(frmBatchAddMember.txtNickName.text).isValid();
    }
    if (in_array(deviceLocale, Countries["CA"])) {
        if (frmBatchAddMember.txtZip.text != "") {
            postal = valid.checkPostalCode(frmBatchAddMember.txtZip.text).isValid();
        }
    }
    if (frmBatchAddMember.txtGoalWeight.text != "") {
    ...
    function BatchAddSuccessCallbackMemberExistence() {
        ...
        if (res && SubRes && isGoalWeight && city && cityBill && nickName && postal && zipcode) {
            ...
            if (batchWeightData.length > 0) {
                ...
                startWeightData.IsMemberAtRisk = false; //** MEG 6826
	 ...
	 Date - 08/14/2014 */
                ...
                } else if (frmBatchAddMember.lblSubTypeInfo.text == getLocalizedString("strLTC3")) {
                    createMember.CommitmentDuration = "3";
                    createMember.PlanType = PlanTypeEnum[frmBatchAddMember.lblSubTypeInfo.text];
                    createMember.CouponCode = frmBatchAddMember.txtSubID.text.substring(0, 9);
                    createMember.SubscriptnID = 0;
                    createMember.LastUsedDate = entryDate;
                ...
                createMember.PersonalGoalWeight = 0; //** MEG 6870
                //Link Unlink  DJP
                ...
                //**MEG 6848
                boEnrollMember.createBatchMember(createMember, batchWeightData, function() {
                    startWeightData = {};
                    if (isSaveAndAdd) {
                        batchWeightData = [];
...
function onPostShowBatchAddMember() {
    kony.print("ABA ----> onPostShowBatchAddMember");
    frmBatchAddMember.btnUnlink.text = getLocalizedString("strLink");
    currentMemberLinkValues.EmailID = "";
    currentMemberLinkValues.EnterpriseID = 0;
    currentMemberLinkValues.LinkType = "None";
        ...
        if (masterMilestoneRes.length > 0) {
            ...
            for (var j in masterMilestoneRes) {
                ...
                switch (milestoneID.toString()) {
                    ...
                    case "42":
                        kony.print("IN Switch MilestoneID ===>>> " + milestoneID);
                        if (weightDifferencInLbs("10", startwt, currentweight)) {
                            if (isFromAdd && checkIfToday(wdate)) alertMessages.push(kony.i18n.getLocalizedString("strMsg10lbAward"));
                            mileStoneObj.push(createObj);
                            AchievedMilestArr.push(milestoneName);
                        }
                        break;
                    ...
                    case "43":
                        kony.print("IN Switch MilestoneID ===>>> " + milestoneID);
                        if (weightDifferencInLbs("5", startwt, currentweight)) {
                            if (isFromAdd && checkIfToday(wdate)) alertMessages.push(kony.i18n.getLocalizedString("strMsg5lbOneTimeAward"));
                            mileStoneObj.push(createObj);
                            AchievedMilestArr.push(milestoneName);
                        }
                        break;
...
function setDptWpa(strtwghtInKG, heightInMeters, ageval, gender, flow, callback) {
    ...
    var DptValue = "";
    var WPA = " ";
    var NDPT = checkAppSettingEnable(Settings["NDPT"]) ? true : false; //** MEG 6963
    if (checkDPTSettings) {
        //new DPT rules
        /*if(ageval < 13){
