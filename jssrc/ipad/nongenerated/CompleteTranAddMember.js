...
function setAddMemberSubscriptionData(createMemberDetailsObject, updatedEmailFromMPActivated, entryDate, flow) {
    kony.print("ABA ---> setAddMemberSubscriptionData createMemberDetailsObject " + JSON.stringify(createMemberDetailsObject));
    kony.print("ABA ---> setAddMemberSubscriptionData saleTransactionObj " + JSON.stringify(saleTransactionObj));
    if (glbFormName.lblSubType.text != kony.i18n.getLocalizedString("strPayg") && glbLocationTypeId != AT_WORK_LOCATION_TYPE_ID && !IsAWSLocationEnabled()) {
        ...
        boMeetings.getMeetingEndDate(function(meetingEndDate) {
            createMemberDetailsObject.ExpirationDate = meetingEndDate;
        });
        createMemberDetailsObject.CouponCode = createMemberDetailsObject.MtngOccrID; //glbFormName.txtSubscriptionID.text;
        // createMemberDetailsObject.ExpirationDate = entryDate; //;
        createMemberDetailsObject.SubscriptnID = 0;
        createMemberDetailsObject.LastUsedDate = entryDate;
        createMemberDetailsObject.ProductID = (saleTransactionObj !== undefined && saleTransactionObj.length > 0) ? saleTransactionObj[0]['productDetail']['ProductID'] : 1;
        createMemberDetailsObject.SubscriptnType = "Series";
        createMemberDetailsObject.IsPAYG = "false";
    }
...
function setAddMemberWeightData(createWeightDetailsForAddMember, createWeightDetailsObject, entryDate) {
    ...
    createWeightDetailsForAddMember.IsMemberAtRisk = false; //**MEG 6866
    //** For Start weight IsMemberAtRiskFlag is set to false
    /*
  		//Added for MEGCN-14
