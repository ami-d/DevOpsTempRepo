...
function createExceptionDetailErrorCallback(err) {
    if (err != null && err["errorMessage"] != null) kony.print(constructErrorMsg("Error", err));
    //callAlert(constructErrorMsg("Error",err),"error");
    else kony.print(getMessageTemplate("generalError"));
    dismissSyncLoadingScreen();
    removeProgressView();
}
...
function showSyncLoadingScreen(text) {
    kony.print("Display sync loading screen");
    text = " " + text + " \n";
...
function dismissSyncLoadingScreen() {
    kony.print("Remove sync loading screen");
...
function checkUndefined(val) {
    if (val == 0) return val;
    else if (val == null || val + "" == "undefined" || val == "" || val == undefined || val == NaN) return "";
    else return val + "";
}
...
function getMPAnd3MPWithExpiry(MpPass, value) {
    try {
        if (checkValidString(MpPass)) {
            var pass;
            var expdate;
            if (value == kony.i18n.getLocalizedString("strMPBuyNew") || value == kony.i18n.getLocalizedString("strLTC6") || value == kony.i18n.getLocalizedString("strLTC3") || value == '1') {
                pass = 1;
...
function IsAWSLocationEnabled() {
    kony.print("IsAWSLocationEnabled " + IsAWSLocation);
    if (IsAWSLocation && deviceLocale == Countries["US"]) {
        return true;
    } else {
        return false;
    }
...
function setAWSLocation(IsAtWork) {
    kony.print("IsAtWork:::" + IsAtWork);
    if (IsAtWork == true || IsAtWork == 'true') {
        IsAWSLocation = true;
    } else {
        IsAWSLocation = false;
    }
    kony.print("IsAWSLocation:::" + IsAWSLocation);
}
//**MEG 7180
function setAWSLocationOnHomeScreen(meetingQuery, dateQuery, callback) {
    var whereClause = "where ID ='" + meetingQuery + "' and MeetingDate like '%" + dateQuery + "%'";

    function setAWSLocationOnHomeScreenSuccessCallback(res) {
        kony.print("setAWSLocationOnHomeScreenSuccessCallback res " + JSON.stringify(res));
        if (res && res.length > 0) {
            kony.print("res[0].IsAtWork " + res[0].IsAtWork);
            kony.print("res[0].MeetingTypeID " + res[0].MeetingTypeID);
            //setInfoSessionMeeting(res[0].MeetingTypeID); //** MEG 7312
            setInfoSessionMeeting(res[0].IsInfoSession); //** MEG 7344
            callback.call(null, res[0].IsAtWork)
        } else {
            setInfoSessionMeeting(0); //** MEG 7312
            callback.call(null, false);
        }
    }

    function setAWSLocationOnHomeScreenErrorCallBack(res) {
        kony.print("setAWSLocationOnHomeScreenErrorCallBack res " + JSON.stringify(res));
        setInfoSessionMeeting(0); //** MEG 7312
        callback.call(null, false);
    }
    kony.print("setAWSLocationOnHomeScreen whereClause " + whereClause);
    DBMeetingsController.find(whereClause, setAWSLocationOnHomeScreenSuccessCallback, setAWSLocationOnHomeScreenErrorCallBack);
}

function getCurrentWeekNumberByData(b) {
    kony.print("::getWeek::---" + b);
    var b = b.split('T')[0];
    var dt = b.split('-');
    b = new Date(dt[0], parseInt(dt[1]) - 1, dt[2]);
    ...
    var utc1 = Date.UTC(a.getFullYear(), a.getMonth(), a.getDate());
    var utc2 = Date.UTC(b.getFullYear(), b.getMonth(), b.getDate());
    var result = Math.floor((utc1 - utc2) / _MS_PER_DAY);
                ...
                if (obj.metadata.type == "WeighDetails") {
                    if (obj.MemberID && !in_array(obj.MemberID, FailedRowWeighMemberIDs)) {
                        if (!in_array(obj.MemberID, WeighMemberIDs)) {
                            ...
                            if (!in_array(obj.WeekNumber, WeighWeekNumber)) WeighWeekNumber.push(obj.WeekNumber.toString());
                        }
                    }
                }
            });
...
/**
 * Function added for identify passduration from expiryDate which user entered.
 */
function identifyPassDuration(mppass) {
    try {
        kony.print("IN identifyPassDuration===" + mppass);
        var passDuration = 1;
        var tmp = mppass.slice(-6);
        kony.print("tmp===" + tmp);
        if (!isNaN(tmp)) {
            ...
            var dateVal = tmp.substring(0, 2) + "/" + tmp.substring(2, 4) + "/" + tmp.substring(4, 6);
            kony.print("dateVal===" + dateVal);
            var userEnteredDate = new Date(dateVal);
            kony.print("userEnteredDate===" + userEnteredDate);
            if (userEnteredDate == "Invalid Date") {
                return passDuration;
            }
            var diff = calculateDaysBetweenTwoDates(new Date(), userEnteredDate);
...
//**MEG 7312
function IsInfoSessionMeeting() {
    kony.print("IsInfoSessionMeeting " + IsInfoSession);
    if (IsInfoSession && deviceLocale == Countries["US"]) {
        return true;
    } else {
        return false;
    }
...
//**MEG 7312
/*function setInfoSessionMeeting(MeetingTypeId){
  	kony.print("setInfoSessionMeeting MeetingTypeId:::"+MeetingTypeId);
  	var meetingTypeId = parseInt(MeetingTypeId);
	if(meetingTypeId == 17 && IsAWSLocationEnabled()){
		IsInfoSession = true;
	}
	else{
		IsInfoSession = false;
	}
  	kony.print("setInfoSessionMeeting IsInfoSession:::"+IsInfoSession);
}*/
function setInfoSessionMeeting(isInfosessionMeetting) {
    kony.print("setInfoSessionMeeting MeetingTypeId:::" + isInfosessionMeetting);
    if (!checkValidString(isInfosessionMeetting)) {
        IsInfoSession = false;
        return;
    }
    if (isInfosessionMeetting == "true" || isInfosessionMeetting == "1" || isInfosessionMeetting == true || isInfosessionMeetting == 1) IsInfoSession = true;
    else IsInfoSession = false;
    kony.print("setInfoSessionMeeting IsInfoSession:::" + IsInfoSession);
}
//**MEG 7312
function enableDisableBtnOnHomeScreen() {
    kony.print("enableDisableBtnOnHomeScreen");
    if (IsInfoSessionMeeting()) {
        hboxHomeScreenHeader.vbxAddnWeightMemberHome.setEnabled(false);
        hboxHomeScreenHeader.imgAddnWeightMemberHome.src = "icn_addweigh_member_hover.png";
        hboxHomeScreenHeader.vboxMPActivate.setEnabled(false);
        hboxHomeScreenHeader.vboxMPActivate.isVisible = false;
        hboxHomeScreenHeader.vboxDirectSale.setEnabled(false);
        hboxHomeScreenHeader.imgDirectSale.src = "icn_shopping_cart_hover.png";
    } else {
        hboxHomeScreenHeader.vbxAddnWeightMemberHome.setEnabled(true);
        hboxHomeScreenHeader.imgAddnWeightMemberHome.src = "icn_addweigh_member.png";
        hboxHomeScreenHeader.vboxMPActivate.setEnabled(true);
        if (in_array(deviceLocale, Countries["US"]) && checkAppSettingEnable(Settings["CC"]) && (glbIsCreditCardEnabled == true || glbIsCreditCardEnabled == "true")) hboxHomeScreenHeader.vboxMPActivate.isVisible = true;
        else hboxHomeScreenHeader.vboxMPActivate.isVisible = false;
        hboxHomeScreenHeader.vboxDirectSale.setEnabled(true);
        hboxHomeScreenHeader.imgDirectSale.src = "icn_shopping_cart_green.png";
    }
}
//Function returns expiry date from MP Pass if valid else returns blank value
function getExpirationDateFromMPPass(mpPass) {
    var pass = mpPass.slice(-6);
    var expirydate = "";
    kony.print("getExpirationDateFromMPPass tmp===" + pass);
    if (!isNaN(pass)) {
        if (checkForCorrectDate(pass)) {
            var strdate = pass.substring(0, 2) + "/" + pass.substring(2, 4) + "/" + pass.substring(4, 6);
            expirydate = new Date(strdate);
            if (expirydate == "Invalid Date") {
                return "";
            } else {
                expirydate = changeDateFormatTo(expirydate);
                kony.print("getExpirationDateFromMPPass expiryDate===" + expirydate);
                return expirydate;
            }
        } else {
            return expirydate;
