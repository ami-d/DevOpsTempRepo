...
function eventinitDOBPopUp(FormBirthdate) {
    ...
    if (kony.application.getCurrentForm().id == frmTallyMeetingCashout.id) {
        popupDateOfBirth.calEnrollMemberDOB.validStartDate = [curday, curmon, curyear];
        popupDateOfBirth.calEnrollMemberDOB.validEndDate = [01, 01, 2100];
    } else {
...
function eventonClickvboxWeighSection() {
    try {
        if (IsReEnrollScreen == FlowForScreens.ReEnroll) {
            ...
            frmEnrollWeighMember.lblWeightLossFocusInfo.text = (termMemberInfo.WeightLossFocus === 0 || termMemberInfo.WeightLossFocus === '0' || termMemberInfo.WeightLossFocus === false || termMemberInfo.WeightLossFocus === 'false') ? kony.i18n.getLocalizedString("strLblNo") : kony.i18n.getLocalizedString("strLblYes");
            frmEnrollWeighMember.hboxWeightLossFocus.setVisibility(true);
            ...
            if (validateEmail && stateRes) {
                var res = valid.checkMemberID(frmEnrollReturningMember.txtRegistrationID.text, "Registration ID").checkEmail(frmEnrollReturningMember.lblEmailInfo.text).heightValidation(frmEnrollReturningMember.lblHeightInfo.text, "Height").validateDate(frmEnrollReturningMember.lblDOBInfo.text).isValid();
                if (res) {
                ...
                var res = valid.checkMemberID(frmEnrollReturningMember.txtRegistrationID.text, "Registration ID").checkEmailForNonMandatory(frmEnrollReturningMember.lblEmailInfo.text).heightValidation(frmEnrollReturningMember.lblHeightInfo.text, "Height").validateDate(frmEnrollReturningMember.lblDOBInfo.text).isValid();
                if (res) {
                    checkIfMemberAlreadyExistsInDB(frmEnrollReturningMember.txtRegistrationID.text, glbMemberId, function() {
                        frmEnrollWeighMember.lblProcessMembersubHeader1.text = (termMemberInfo.FirstName).trim() + " " + (termMemberInfo.LastName).trim();
                        demoEmail = frmEnrollReturningMember.lblEmailInfo.text;
            ...
            frmEnrollWeighMember.hboxWeightLossFocus.setVisibility(false);
            isNWI = false;
            ...
            if (frmAddIndividulaMember.lblMemberShipInfo.text.toUpperCase() == "AT WORK") {
                var res = valid.checkEnrollFirstName(frmAddIndividulaMember.txtFirstName.text, "FirstName").checkEnrollLastName(frmAddIndividulaMember.txtLastName.text, "LastName").checkEmailForNonMandatory(frmAddIndividulaMember.txtEmailID.text).validateDate(frmAddIndividulaMember.lblDOBInfo.text).validateStartDate(frmAddIndividulaMember.lblStartDateInfo.text, frmAddIndividulaMember.lblDOBInfo.text).checkForRequiredFields(frmAddIndividulaMember.lblMemberShipInfo.text, "MembershipType").heightValidation(frmAddIndividulaMember.lblHeightInfo.text, "Height").CheckWeeksCompleted(frmAddIndividulaMember.lblStartDateInfo.text, frmAddIndividulaMember.txtWeeksCompleted.text).validateWeight(frmAddIndividulaMember.txtStartWeight.text).CheckRemainingMissedWeekPasses(frmAddIndividulaMember.txtRemainingWeeks.text, 1).isValid();
            } else {
                var res = valid.checkEnrollFirstName(frmAddIndividulaMember.txtFirstName.text, "FirstName").checkEnrollLastName(frmAddIndividulaMember.txtLastName.text, "LastName").checkEmailForNonMandatory(frmAddIndividulaMember.txtEmailID.text).validateDate(frmAddIndividulaMember.lblDOBInfo.text).validateStartDate(frmAddIndividulaMember.lblStartDateInfo.text, frmAddIndividulaMember.lblDOBInfo.text).checkForRequiredFields(frmAddIndividulaMember.lblMemberShipInfo.text, "MembershipType").checkMemberID(frmAddIndividulaMember.txtMemberID.text, "MemberId").heightValidation(frmAddIndividulaMember.lblHeightInfo.text, "Height").CheckWeeksCompleted(frmAddIndividulaMember.lblStartDateInfo.text, frmAddIndividulaMember.txtWeeksCompleted.text).validateWeight(frmAddIndividulaMember.txtStartWeight.text).CheckRemainingMissedWeekPasses(frmAddIndividulaMember.txtRemainingWeeks.text, 1).isValid();
            ...
            if (res) {
                ...
                checkIfMemberAlreadyExistsInDB(addmemberid, glbMemberId, function() {
                    ...
                    frmEnrollWeighMember.lblProcessMembersubHeader1.text = toTitleCase(frmEnrollWeighMember.lblProcessMembersubHeader1.text);
                    eventonDoneTextAreaWeighCalculations(); //** Moved here for MEG 6963
                });
                //eventonDoneTextAreaWeighCalculations(); //** Commented for MEG 6963
            }
            ...
            checkIfMemberAlreadyExistsInDB(frmEnrollNewMember.txtMemberIDInfo.text, glbMemberId, function() {
                var weightLossFocusValue = (frmEnrollNewMember.switchWeightLossFocus.selectedIndex == "0") ? true : false;
                frmEnrollWeighMember.lblWeightLossFocusInfo.text = (weightLossFocusValue === false) ? kony.i18n.getLocalizedString("strLblNo") : kony.i18n.getLocalizedString("strLblYes");
                frmEnrollWeighMember.hboxWeightLossFocus.setVisibility(true);
                if (validateEmail) {
                    //added by  Ami MEG-3875 - start
                    var resEmail = true;
...
function eventonClickDoneDOBPopUp() {
    ...
    if (kony.application.getCurrentForm().id == frmTallyMeetingCashout.id) {
        ...
        var dateToShow = montharray[month - 1] + " " + day + ", " + year;
        var dateStr = month + "/" + day + "/" + year; // eg.02/23/2015
        if (popupDateOfBirth.lblHiddenVal.text == frmTallyMeetingCashout.lbl1stSlip.text) frmTallyMeetingCashout.lblSlipDate1.text = dateStr;
        else if (popupDateOfBirth.lblHiddenVal.text == frmTallyMeetingCashout.lbl2ndSlip.text) frmTallyMeetingCashout.lblSlipDate2.text = dateStr;
        else if (popupDateOfBirth.lblHiddenVal.text == frmTallyMeetingCashout.lbl3rdSlip.text) frmTallyMeetingCashout.lblSlipDate3.text = dateStr;
    ...
    if (popupDateOfBirth.calEnrollMemberDOB.date > TodayDate) {
        ...
        if (popupDateOfBirth.calEnrollMemberDOB.date == null) {
            ...
            //**MEG7312
            if (kony.application.getCurrentForm().id == frmPreRegAddMember.id) {
                frmPreRegAddMember.lblDOBInfo.text = changeDateFormate(MyDateString, kony.i18n.getLocalizedString("strDisplayDateFormat"));
            }
            //**End
        } else {
            ...
            //**MEG7312
            if (kony.application.getCurrentForm().id == frmPreRegAddMember.id) {
                frmPreRegAddMember.lblDOBInfo.text = changeDateFormate(popupDateOfBirth.calEnrollMemberDOB.date, kony.i18n.getLocalizedString("strDisplayDateFormat"));
            }
            //**End
        }
        popupDateOfBirth.dismiss();
...
function eventonClickDoneHeightPopUp() {
    ...
    if (popupHeight.pckrSelectHeight.selectedKeys != null && !isFromPreActivationHeight) {
        ...
        if (kony.application.getCurrentForm().id == frmPreRegAddMember.id) { //** MEG 7312
            frmPreRegAddMember.lblHeightInfo.text = PickerHeightSelectedKeys[0] + " " + getLocalizedString("strFeetAbbr") + " " + PickerHeightSelectedKeys[1] + " " + getLocalizedString("strInchesAbbr") + " ";
        }
    }
