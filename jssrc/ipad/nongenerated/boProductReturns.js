var boTransacations = {
    ...
    getTransacationsByMeetingId: function(MeetingId, isMember, isSort, bindTransactionsData) {
        ...
        if (IsAWSLocationEnabled()) {
            if (isMember == "members") {
                transactionSQLQuery = "Select * from (SELECT B.LastName, B.FirstName, A.TotalSalePrice as SalePrice, A.TotalSaleTax, max(strftime('%H:%M', A.SaleDate)) " + " as SaleDate1,(sum((C.UnitPriceTax+C.UnitPrice) * C.Quantity) - A.subsidyamount)  " + " as TotalSalePrice  ,A.RegNumber, A.MemberID, A.MeetingOccurID " + " FROM SaleDetails A " + " inner join MemberDetails B on B.RegNumber=A.RegNumber and(B.Memberid = B.PreRegNumber or B.PreRegNumber = 0 or B.Memberid GLOB '*[A-z]*' )  " + " inner join SaleItems C on  C.SaleTransactnId=A.SaleTransactnId " + " where A.CountryID = '" + getCountryID() + "' AND  A.MeetingOccurID='" + MeetingId + "' and A.MeetingDate like '%" + today + "%' " + " and (A.IsSaleVoid is null or A.IsSaleVoid= '' or A.IsSaleVoid= 0) " + " and (A.IsReturn is null or A.IsReturn= '' or A.IsReturn= 0) and (C.IsSaleItemVoid is null or C.IsSaleItemVoid= '' or C.IsSaleItemVoid= 0) " + " and C.Quantity >0 " + " and   C.ProductID in (select ProductID from ProductDetail where ProductID=C.ProductID)  and (A.IsServiceProvider is null or A.IsServiceProvider= '' or A.IsServiceProvider= 0) and  A.LocationID=" + glbLocationID + " group by A.MemberID " + orderClause + ")"; //**MEG 7383
            } else if (isMember == "nonmembers") {
                transactionSQLQuery = "SELECT  A.SaleTransactnId,A.TotalSalePrice as SalePrice, A.TotalSaleTax, max(strftime('%H:%M', A.SaleDate)) as SaleDate1," + "(sum((B.UnitPriceTax+B.UnitPrice)* B.Quantity)- A.subsidyamount )  as TotalSalePrice  , " + "A.RegNumber, A.MeetingOccurID " + "FROM SaleDetails A" + " inner join SaleItems B on A.[SaleTransactnId] = B.[SaleTransactnId]" + " where A.CountryID = '" + getCountryID() + "' AND A.MeetingOccurID=" + MeetingId + " and A.MeetingDate like '%" + today + "%' " + " and (A.IsSaleVoid is null or A.IsSaleVoid= '' or A.IsSaleVoid= 0)" + " and (A.IsReturn is null or A.IsReturn= '' or A.IsReturn= 0) " + " and (B.IsSaleItemVoid is null or B.IsSaleItemVoid= '' or B.IsSaleItemVoid= 0)" + " and (A.IsServiceProvider is null or A.IsServiceProvider= '' or A.IsServiceProvider= 0)" + " and B.Quantity >0 and (A.RegNumber='1' or A.RegNumber='1.0') " + " and A.LocationID=" + glbLocationID + " group by  A.SaleTransactnId" + orderClause;
            } else if (isMember == "employeesale") {
                transactionSQLQuery = "Select * from (SELECT B.LastName, B.FirstName, A.TotalSalePrice as SalePrice, A.TotalSaleTax, max(strftime('%H:%M', A.SaleDate)) " + " as SaleDate1,(sum((C.UnitPriceTax+C.UnitPrice) * C.Quantity)- A.subsidyamount) " + " as TotalSalePrice  ,A.RegNumber, A.MemberID, A.MeetingOccurID " + " FROM SaleDetails A " + " inner join MemberDetails B on B.MemberId=A.MemberId  " + " inner join SaleItems C on  C.SaleTransactnId=A.SaleTransactnId " + " where A.CountryID = '" + getCountryID() + "' AND  A.MeetingOccurID='" + MeetingId + "' and A.MeetingDate like '%" + today + "%' " + " and (A.IsSaleVoid is null or A.IsSaleVoid= '' or A.IsSaleVoid= 0) " + " and (A.IsReturn is null or A.IsReturn= '' or A.IsReturn= 0) and (C.IsSaleItemVoid is null or C.IsSaleItemVoid= '' or C.IsSaleItemVoid= 0) " + " and C.Quantity >0 " + " and   C.ProductID in (select ProductID from ProductDetail where ProductID=C.ProductID)  and (A.IsServiceProvider= 1) and  A.LocationID=" + glbLocationID + " group by A.MemberID " + orderClause + ")"; //**MEG 7383
            }
        } else {
            if (isMember == "members") {
                ...
                transactionSQLQuery = "SELECT  A.SaleTransactnId,A.TotalSalePrice as SalePrice, A.TotalSaleTax, max(strftime('%H:%M', A.SaleDate)) as SaleDate1," + "sum((B.UnitPriceTax+B.UnitPrice)* B.Quantity) as TotalSalePrice  , " + "A.RegNumber, A.MeetingOccurID " + "FROM SaleDetails A" + " inner join SaleItems B on A.[SaleTransactnId] = B.[SaleTransactnId]" + " where A.CountryID = '" + getCountryID() + "' AND A.MeetingOccurID=" + MeetingId + " and A.MeetingDate like '%" + today + "%' " + " and (A.IsSaleVoid is null or A.IsSaleVoid= '' or A.IsSaleVoid= 0)" + " and (A.IsReturn is null or A.IsReturn= '' or A.IsReturn= 0) " + " and (B.IsSaleItemVoid is null or B.IsSaleItemVoid= '' or B.IsSaleItemVoid= 0)" + " and (A.IsServiceProvider is null or A.IsServiceProvider= '' or A.IsServiceProvider= 0)" + " and B.Quantity >0 and (A.RegNumber='1' or A.RegNumber='1.0') " + " and A.LocationID=" + glbLocationID + " group by  A.SaleTransactnId" + orderClause;
            } else if (isMember == "employeesale") {
                transactionSQLQuery = "Select * from (SELECT B.LastName, B.FirstName, A.TotalSalePrice as SalePrice, A.TotalSaleTax, max(strftime('%H:%M', A.SaleDate)) " + " as SaleDate1,sum((C.UnitPriceTax+C.UnitPrice) * C.Quantity) " + " as TotalSalePrice  ,A.RegNumber, A.MemberID, A.MeetingOccurID " + " FROM SaleDetails A " + " inner join MemberDetails B on B.MemberId=A.MemberId " + " inner join SaleItems C on  C.SaleTransactnId=A.SaleTransactnId " + " where A.CountryID = '" + getCountryID() + "' AND  A.MeetingOccurID='" + MeetingId + "' and A.MeetingDate like '%" + today + "%' " + " and (A.IsSaleVoid is null or A.IsSaleVoid= '' or A.IsSaleVoid= 0) " + " and (A.IsReturn is null or A.IsReturn= '' or A.IsReturn= 0) and (C.IsSaleItemVoid is null or C.IsSaleItemVoid= '' or C.IsSaleItemVoid= 0) " + " and C.Quantity >0 " + " and   C.ProductID in (select ProductID from ProductDetail where ProductID=C.ProductID)  and (A.IsServiceProvider= 1) and  A.LocationID=" + glbLocationID + " group by A.MemberID " + orderClause + ")"; //**MEG 7383
            }
        }
    ...
    getProductsForReturnByMemberID: function(MeetingId, MemberId, isMember, bindProductTransactionsData) {
        ...
        var transactionSQLQuery = "";
        if (IsAWSLocationEnabled()) {
            transactionSQLQuery = "SELECT A.Id, A.ProductID, A.ProductSku, B.description, A.Quantity , case when B.Type == 'Fees' then (A.UnitPrice - S.SubsidyAmount) else A.UnitPrice end as UnitPrice, A.UnitPriceTax, B.Type, " + " case when B.Type == 'Fees' then (((A.UnitPrice+A.UnitPriceTax)* A.Quantity) - S.SubsidyAmount) else ((A.UnitPrice+A.UnitPriceTax)* A.Quantity) end as TotalPrice, A.SaleTransactnId,S.TotalSalePrice as SalePrice, S.TotalSaleTax," + " (Select count(*) from PreActivation where SaleTransactnId = A.SaleTransactnId) as isActivated " + " from SaleItems A  " + " inner join ProductDetail B on A.ProductID=B.ProductID and B.LocationID=" + glbLocationID + " inner join SaleDetails S on S.SaleTransactnId = A.[SaleTransactnId] and S.MemberID = '" + MemberId + "'" + " and S.MeetingOccurID = '" + glbMeetingNum + "' and S.MeetingDate like '%" + glbMeetingDate + "%'" + " where S.CountryID = '" + getCountryID() + "' AND A.Quantity >0  " + " and (A.IsSaleItemVoid is null or A.IsSaleItemVoid= '' or A.IsSaleItemVoid= 0) and A.ProductSku not in(" + convertArrayToString(getSKUList('Employee_Attendance')) + ") order by S.SaleDate desc";
        } else {
            transactionSQLQuery = "SELECT A.Id, A.ProductID, A.ProductSku, B.description, A.Quantity , A.UnitPrice as UnitPrice, A.UnitPriceTax, B.Type, " + " ((A.UnitPrice+A.UnitPriceTax)* A.Quantity) as TotalPrice, A.SaleTransactnId,S.TotalSalePrice as SalePrice, S.TotalSaleTax," + " (Select count(*) from PreActivation where SaleTransactnId = A.SaleTransactnId) as isActivated " + " from SaleItems A  " + " inner join ProductDetail B on A.ProductID=B.ProductID and B.LocationID=" + glbLocationID + " inner join SaleDetails S on S.SaleTransactnId = A.[SaleTransactnId] and S.MemberID = '" + MemberId + "'" + " and S.MeetingOccurID = '" + glbMeetingNum + "' and S.MeetingDate like '%" + glbMeetingDate + "%'" + " where S.CountryID = '" + getCountryID() + "' AND A.Quantity >0  " + " and (A.IsSaleItemVoid is null or A.IsSaleItemVoid= '' or A.IsSaleItemVoid= 0) and A.ProductSku not in(" + convertArrayToString(getSKUList('Employee_Attendance')) + ") order by S.SaleDate desc";
        }
        //**End
    ...
    getProductsForReturnByMeetingId: function(MeetingId, MemberId, isMember, SaleTransId, bindProductTransactionsData) {
        var transactionSQLQuery = "SELECT A.Id,A.ProductID,A.ProductSku,B.description,A.Quantity , " + "  (A.UnitPrice) as UnitPrice ,  A.UnitPriceTax , " + "  ((A.UnitPrice+A.UnitPriceTax )* A.Quantity) as TotalPrice, " + "  A.SaleTransactnId " + ",B.Type from SaleItems A " + " inner join ProductDetail B on A.ProductID=B.ProductID and B.Type='TangibleProduct' and B.LocationID=" + glbLocationID + " where A.CountryID = '" + getCountryID() + "' AND A.Quantity >0 " + " and A.SaleTransactnId='" + SaleTransId + "' " + " and (A.IsSaleItemVoid is null or A.IsSaleItemVoid= '' or A.IsSaleItemVoid= 0) " + " order by A.SaleTransactnId";
        kony.print("::transactionSQLQuery=" + transactionSQLQuery);
    ...
    returnProducts: function(productList, isAllProductReturned, productsReturnSuccessCallBack, voidInfo) {
        ...
        function updateSalePaymentSuccessCallback() {
            ...
            if (!isAllProductReturned) productsReturnSuccessCallBack(productCount, paymentMode, SaleTransId);
            else {
