        ...
        if (subTypeText != SubTypeKeyValues[0][1]) {
            ...
            switch (SubTypeKeyValues[0][1]) {
                ...
                case kony.i18n.getLocalizedString("strPrepaid"):
                    //** MEG 7171
                    if (!IsAWSLocationEnabled() && kony.application.getCurrentForm().id == frmProcessMember.id && frmProcessMember.lblProcessMemberSubHeaderlbl2.text == "Series Member") kony.print("Do Nothing");
                    else {
    ...
    try {
        ...
        if (res) {
            ...
            if (IsAWSLocationEnabled() && !in_array(selectKey, ['3-2-2', '3-1-1'])) {
                ...
                if (in_array(deviceLocale, Countries["CA"]) && (Flow == "Enroll") && (memberRegStatus == 'Online')) whrCondition = "where Action='" + Actions[splData[1]] + "' and Subscription='" + Subcriptions[splData[0]] + "' and Transact='" + Flow + "' and membertype like '" + membertype + "' and RegistrationStatus = 'ONLINE'";
                else whrCondition = "where Action='" + Actions[splData[1]] + "' and Subscription='" + Subcriptions[splData[0]] + "' and Transact='" + Flow + "' and membertype like '" + membertype + "' and RegistrationStatus <> 'ONLINE'";
                ...
                if (Flow == "Attend" && membertype.toUpperCase() == "LIFETIME" && Subcriptions[splData[0]] == "PAYG") {
                    ...
                    if (missedWeekCnt > 0) {
                        ...
                        if (in_array(deviceLocale, Countries["CA"])) {
                            ...
                            if (isSenior == true) missedSku = MissedWeekSku[courtesyTitle];
                            else missedSku = ((newSku = parseInt('701') + parseInt(missedWeekCnt)) > 703) ? MissedWeekSku[courtesyTitle] : newSku;
                        }
        ...
        boMeetings.getSeriesWeekLeft(function(res) {
            if (res.length > 0) {
                ...
                if (Actions[splitData[1]] == Actions[2]) { // Redeem
                    ...
                    if (currWeek <= enrollmentWeeks) /*MEG-7301 , MEG-7318*/ whrCondition = "where Transact='" + Flows[splitData[2]] + "' and Subscription = '" + Subcriptions[splitData[0]] + "' and Action ='" + Actions[splitData[1]] + "' and SeriesLength = '" + seriesLength + "'";
                    else whrCondition = "where Transact='" + Flows[splitData[2]] + "' and Subscription = '" + Subcriptions[splitData[0]] + "' and Action ='" + Actions[splitData[1]] + "' and Weeksleft=" + weekleft + " and LOWER(SeriesLength) = 'null'";
                }
                kony.print("::5819::Weekleft::" + weekleft);
                kony.print("::5819::whrCondition::" + whrCondition);
                if (isFromCheckout) {
...
function getWeeksComplete(res1, callback) {
    ...
    function getWeeksCompleteSuccessCallback(res) {
        ...
        if (res.length > 0) {
            ...
            if (parseInt(res[0]._WeekNo) >= weeksCompl) weeksCompl = parseInt(res[0]._WeekNo) - 1;
        }
