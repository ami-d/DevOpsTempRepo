...
function displayMemberDataWithAdvancedSearch(data) {
    ...
    if ((selectedMemberData != "" && selectedMemberData != undefined) && selectedMemberData.RegStatus == MemberRegEnum.Online && data == null) {
        ...
        //selectedMemberData = "";
        if (kony.application.getCurrentForm().id == frmMemberProfileSearch.id) alertEnrollOnlineMemberNotify();
        else displayMemberProfileScreenWithAdvancedSearch(true, false, true, data);
    } else {
        displayMemberProfileScreenWithAdvancedSearch(true, false, true, data);
    }
}

function alertEnrollOnlineMemberNotify() {
    var basicConf = {
        message: kony.i18n.getLocalizedString("strMsgNoMemFound"),
        alertType: constants.ALERT_TYPE_INFO,
        alertTitle: "",
        yesLabel: kony.i18n.getLocalizedString("strLblOk"), // "Yes",
        alertHandler: processOnlineMember
    };
    var pspConf = {};
    var infoAlert = kony.ui.Alert(basicConf, pspConf);
}

function processOnlineMember() {
    eventonClickvbxProcessMemberForProfileSearch();
    displayNote();
}
...
function getSimpleSearchData() {
    ...
    try {
        ...
        whrCondition = whrCondition + " and m.LocationID='" + glbLocationID + "' and m.Usertype <> '1'";
...
function createAdvancedSearchWhereClause(firstname, lastname, phone, memberid, email) {
    ...
    try {
        ...
        popupAdvancedSearch.destroy(); // Added by Praveen Kalal MEG-2712
        queryString = "where " + queryString + " AND m.Usertype <> '1'";
...
function compare(firstObj, secondObj) {
    if (firstObj.lblLastName < secondObj.lblLastName) return -1;
    if (firstObj.lblLastName > secondObj.lblLastName) return 1;
    if (firstObj.lblLastName == secondObj.lblLastName) {
        if (firstObj.lblFirstName < secondObj.lblFirstName) return -1;
        if (firstObj.lblFirstName > secondObj.lblFirstName) return 1;
    }
    return 0;
}
...
function compareCheckedin(firstObj, secondObj) {
    if (firstObj.lblMPSLastName < secondObj.lblMPSLastName) return -1;
    if (firstObj.lblMPSLastName > secondObj.lblMPSLastName) return 1;
    if (firstObj.lblMPSLastName == secondObj.lblMPSLastName) {
        if (firstObj.lblMPSFirstName < secondObj.lblMPSFirstName) return -1;
        if (firstObj.lblMPSFirstName > secondObj.lblMPSFirstName) return 1;
    }
    return 0;
}
...
function compareSubTypeCheckedin(firstObj, secondObj) {
    if (firstObj.SubscriptnType < secondObj.SubscriptnType) return -1;
    if (firstObj.SubscriptnType > secondObj.SubscriptnType) return 1;
    return 0;
}

function setSortFlagHome() {
    if (isSortAscendingHome == true) isSortAscendingHome = false;
    else isSortAscendingHome = true;
    return isSortAscendingHome;
}

function setSortFlagHomeFName() {
    if (isSortAscendingHomeFName == true) isSortAscendingHomeFName = false;
    else isSortAscendingHomeFName = true;
    return isSortAscendingHomeFName;
}

function setSortFlagHomeSubType() {
    if (isSortAscendingHomeSubType == true) isSortAscendingHomeSubType = false;
    else isSortAscendingHomeSubType = true;
...
function showPreRegData() {
    ...
    kony.print("IsAWSLocationEnabled------>>>" + IsAWSLocationEnabled());
    if (IsInfoSessionMeeting()) { //** MEG 7316
        // get members data for particular location id and display on home screen 
        //** MEG 7315
        var sql = "SELECT ID as PreRegNumber,FirstName,LastName,Phone,MtngOccrID FROM PreRegMemberDetails where MeetingDate = '" + glbMeetingDate + "'  and MtngOccrID ='" + glbMeetingNum + "'" + " UNION ALL " + " SELECT PreRegNumber,FirstName,LastName,Phone1 as Phone,MtngOccrID FROM MemberDetails where  PreRegNumber not in (SELECT ID FROM PreRegMemberDetails) and PreRegNumber > 0" + " and MtngOccrID ='" + glbMeetingNum + "' and RegStatus='" + MemberRegEnum.Pre_Registered + "'";
        kony.print("sql IsInfoSessionMeeting------>>>" + sql);
        boHomeScreenSearch.getMembersOfInfoSessionMeetingForHomeScreen(sql);
        //**End
    } else if (glbMeetingNum == "" || glbMeetingNum == undefined || IsAWSLocationEnabled()) {
...
function onClickTallyConfirmationAlert(response) {
    if (response == true) {
        ...
        frmTallyMeetingCashout.txtBankDepositSlip1No.text = "";
        frmTallyMeetingCashout.txtBankDepositSlip2No.text = "";
        frmTallyMeetingCashout.txtBankDepositSlip3No.text = "";
        frmTallyMeetingCashout.lblSlipDate1.text = "";
        frmTallyMeetingCashout.lblSlipDate2.text = "";
        frmTallyMeetingCashout.lblSlipDate3.text = "";
        //**MEG 7313
        if (IsInfoSessionMeeting()) {
            frmTallyMeetingCashout.txtCash.text = "0";
            frmTallyMeetingCashout.lblDiffCash.text = addDollar("0.00");
            frmTallyMeetingCashout.txtChecks.text = "0";
            frmTallyMeetingCashout.lblDiffChecks.text = addDollar("0.00");
            frmTallyMeetingCashout.txtCreditCard.text = "0";
            frmTallyMeetingCashout.lblDiffCreditCard.text = addDollar("0.00");
            frmTallyMeetingCashout.txtCreditSlipsRedeemed.text = "0";
            frmTallyMeetingCashout.lblDiffCreditSlipsRedeemed.text = addDollar("0.00");
            frmTallyMeetingCashout.txtCreditSlipsIssued.text = "0";
            frmTallyMeetingCashout.lblDiffCreditSlipsIssued.text = addDollar("0.00");
            frmTallyMeetingCashout.txtDebitCard.text = "0";
            frmTallyMeetingCashout.lblDiffDebitCard.text = addDollar("0.00");
            frmTallyMeetingCashout.txtBankDepositSlipNumber.setEnabled(false);
            frmTallyMeetingCashout.btnScan.setEnabled(false);
        } else {
            frmTallyMeetingCashout.txtBankDepositSlipNumber.setEnabled(true);
            frmTallyMeetingCashout.btnScan.setEnabled(true);
        }
        //show the form 
...
function onSelectRowCheckedInMember() {
    ...
    //RTB-126
    if (glbIsSelectedMemberExpired == true) {
        var IsSelectedMemExpired = checkGracePeriodByExpirationDate(selectedMemberData["ExpirationDate"]);
        if (IsSelectedMemExpired == false) {
