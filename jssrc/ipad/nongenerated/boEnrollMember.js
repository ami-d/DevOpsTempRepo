...
var boEnrollMember = {
            ...
            for (var i in response) {
                ...
                if (ProductDetail != undefined && ProductDetail != null && ProductDetail != "") {
                    ...
                    if (ProductInfo != undefined && ProductInfo != null && ProductInfo != "") {
                        ...
                        tax = ProductInfo.Tax;
                        ProductInfo.productinfo.hidIsAllowPriceOverride = kony.sync.getString(v.AllowPriceOverride);
                        kony.print("productinfo=" + JSON.stringify(ProductInfo.productinfo));
            ...
            if (productPaymentDetail.length > 0) {
                ...
                for (var i in productPaymentDetail) {
                    ...
                    var b = mapKeys(viewEnrollWeighPayment, {
                        ...
                        hidIsAllowPriceOverride: v.hidIsAllowPriceOverride
                    });
    ...
    generateDeviceLevelMemberID: function() {
        kony.print("ABA --- > generateDeviceLevelMemberID called");
        ...
        function createMemberWeightSuccessCallback(res) {
            ...
            if (MissedWeekWeightData.length > 0 && (strFlow == "Process" || strFlow == "Add")) {
                ...
                for (var i in MissedWeekWeightData) {
                    //*** MEG 6866
                    kony.print("ABA --> MissedWeekWeightData  " + JSON.stringify(MissedWeekWeightData[i]));
                    if (!checkValidString(MissedWeekWeightData[i].IsMemberAtRisk)) {
                        MissedWeekWeightData[i].IsMemberAtRisk = false;
                        kony.print("ABA --> MissedWeekWeightData entered to set IsMemberAtRisk ");
                    }
                    //** End
    ...
    addMissedWeekWeightData: function() {
        ...
        kony.print("MissedWeekWeightData " + JSON.stringify(MissedWeekWeightData));
    ...
    updateMemberDetails: function(updateMemberDetailsObject, createWeightDetailsObject, createNoteDetailsObject, createBMINoteDetailsObject, updateNoteObject, strFlow) {
        function updateMemberSuccessCallback(res) {
            ...
            if (isNotesDataPresent) {
                if (_.isEmpty(createNoteDetailsObject) == false) boEnrollMember.addMemberNoteDetails(createNoteDetailsObject, strFlow);
    ...
    insertToSaleTransaction: function(saleDetail, saleItemData, salePayment, saleSplitPayment, callBackCompleteTranscation) {
        glbMemberId = 0;
        ...
        function insertToSaleTransactionCallback(res) {
            ...
            boEnrollMember.addSaleItems(saleItemData, salePayment, saleSplitPayment, callBackCompleteTranscation);
        }
    ...
    addSaleItems: function(saleItemData, salePayment, saleSplitPayment, callBackCompleteTranscation) {
        function addSaleItemsCallback(res) {
            selectedKeyForReason = null;
            kony.print(getMessageTemplate("addSuccess", "SaleItemsData"), "info");
            boEnrollMember.addSalePayments(salePayment, saleSplitPayment, callBackCompleteTranscation);
        }
        DBSaleItemsController.createAll(saleItemData, addSaleItemsCallback);
    },
    addSalePayments: function(PaymentDetails, saleSplitPayment, callBackCompleteTranscation) {
        function addSalePaymentsCallback(res) {
            kony.print(getMessageTemplate("addSuccess", "SalePaymentData"), "info");
            if (IsAWSLocationEnabled() && saleSplitPayment.length > 0) {
                boEnrollMember.addSaleSplitPayment(saleSplitPayment, callBackCompleteTranscation);
            } else {
    ...
    addSaleSplitPayment: function(saleSplitPayment, callBackCompleteTranscation) {
        function addSaleSplitPaymentCallback(res) {
            kony.print(getMessageTemplate("addSuccess", "SplitSalePayment"), "info");
            callBackCompleteTranscation.call(null);
        }
        DBSplitSalePaymentController.createAll(saleSplitPayment, addSaleSplitPaymentCallback, eventErrorCallBack)
    },
    ...
    getWeightDetailsForLifetimeMemberEligibility: function(memberId, goalWeight, goalAchDate) {
        ...
        function getWeightDetailsForLifetimeMemberEligibilitySuccessCallback(res) {
            ...
            if (isMaintainace) boEnrollMember.getWeightEntriesTillSixthWeek(memberId, goalAchDate, goalWeight);
            else {
    ...
    getNewlyEnrolledMember_old: function(memberId) {
        ...
        function getNewlyEnrolledMemberSuccessCallback(res) {
            if (res.length > 0) {
                ...
                if (diff == 0) showNotificationsForNewlyEnrolledMember(5);
                else showNotificationsForNewlyEnrolledMember(diff);
            } else {
    ...
    getNewlyEnrolledMember: function(memberId) {
        ...
        function getNewlyEnrolledMemberSuccessCallback(res) {
            ...
            var diff = res.length;
            kony.print("diff : " + diff);
            if (diff == 0 || diff > 4) showNotificationsForNewlyEnrolledMember(5);
            else showNotificationsForNewlyEnrolledMember(diff);
        }
    ...
    updateMemberLifetimeType: function(memberId, isFreeLifeTime) {
        var whereClause = "where MemberID = '" + memberId + "'";
        var dataObj = {};
        if (isFreeLifeTime) dataObj["FreePaid"] = false;
        else dataObj["FreePaid"] = true;
    ...
    createBatchMember: function(createMemberObj, memberWeightObjArr, callback) { //** MEG 6848
        function createBatchMemberSuccessCallback(res) {
            kony.print("createBatchMemberSuccessCallback res " + JSON.stringify(res));
            ClearBatchMemberVariables();
            callback.call(null); //** MEG 6848
            boEnrollMember.addBatchMemberWeightData(memberWeightObjArr);
            kony.print(getMessageTemplate("addSuccess", "MemberDetails"), "info");
        }
        //** MEG 6848 
        function createBatchMemberErrorCallBack() {
            kony.print("createBatchMemberErrorCallBack");
            callback.call(null);
        }
        //** End
        createMemberObj.EmpID = glbEmployeeId;
        DBMemberDetailsController.create(createMemberObj, createBatchMemberSuccessCallback, createBatchMemberErrorCallBack);
    },
    addBatchMemberWeightData: function(memberWeightObjArr) {
        kony.print("ABA ---> memberWeightObjArr " + JSON.stringify(memberWeightObjArr));
    ...
    addOnlineMemberDetails: function(createMemberDetailsObject, isSegmentClicked) {
        function createMemberSuccessCallback(res) {
            ...
            if (isSegmentClicked) {
                ...
                var selectedRow = parseInt(index);
    ...
    getMemberNoteDataWS: function(memberID) {
        try {
            ...
            //** MEG 6493
            if (JsonService) {
                MemberNoteList_inputparam["httpheaders"] = setRESTHeader();
            } else {
    ...
    getMemberNoteDataWSCallback: function(status, MembersNoteList) {
        try {
            ...
            if (status == 400) {
                ...
                if (MembersNoteList["MemberNote"] && MembersNoteList["MemberNote"].length > 0) {
    ...
    getGoalWeightOfMember: function(memberId) {
        var whereClause = " where MemberID = '" + memberId + "' AND MemberID <> '0' ";
    ...
    //** MEG 6596
    getRegNumberbyMemberId: function(memberId, callback) {
        function getRegNumberSuccessCallback(res) {
            kony.print("getRegNumberSuccessCallback res" + JSON.stringify(res));
            if (res.length > 0) {
                var v = res[0];
                kony.print("v regnuber res" + JSON.stringify(v["RegNumber"]));
                callback.call(null, v["RegNumber"]);
            } else {
                callback.call(null, 0);
            }
        }
        var whereClause = " where MemberID = '" + memberId + "'";
        DBMemberDetailsController.find(whereClause, getRegNumberSuccessCallback, eventErrorCallBack);
        /* com.kony.WeightWatchers.MemberSyncScope.MemberDetails.getAllDetailsByPK({
             "MemberID": memberId
         }, getRegNumberSuccessCallback, eventErrorCallBack);*/
        //** End
    },
    checkForValidMP: function(whrCondition, membertype, Subtype, isOverride, flow, action, callback, isFromEditProfile, memberId, callbackEdit) {
            ...
            if (res.length > 0) {
                ...
                if (res.length == 1 && glbMemberId == res[0]["MemberID"]) {
                    if (checkIfNetworkIsAvailable() && !is20Week && !is20WeekRedeem) {
                        boEnrollMember.checkForValidMPViaWS(whrCondition, membertype, Subtype, isOverride, action, couponCode, callback, isFromEditProfile, memberId, callbackEdit);
                        ...
                        if (is20Week) {
                            ...
                            if (validExpiryDate(data)) {
                                ...
                                var expirationDate = getExpirationDateFromMPPass(data);
                                if (expirationDate != "" && expirationDate != undefined) {
                                    if (checkGracePeriodByExpirationDate(expirationDate) == false) {
                    ...
                    if (is20Week) {
                        ...
                        if (validExpiryDate(data)) {
                            ...
                            if (typeofSub == kony.i18n.getLocalizedString("strMPRedeem")) {
                                typeofSub = identifyPassDuration(data);
                            }
                            ...
                            var expirationDate = getExpirationDateFromMPPass(data);
                            if (expirationDate != "") {
                                if (checkGracePeriodByExpirationDate(data) == false) {
                                    glbIsSelectedMemberExpired = false;
                                    if (isFromEditProfile) {
                                        callbackEdit.call(null);
                                        return true;
    ...
    checkForValidMPViaWS: function(whrCondition, membertype, Subtype, isOverride, action, couponCode, callback, isFromEditProfile, memberId, callbackEdit) {
        var data = isFromEditProfile ? frmEditMemberProfile.txtSubID.text : glbFormName.txtSubscriptionID.text;
            ...
            try {
                if (status == 400) {
                    if (CheckValidMPResult["opstatus"] == 0) {
                        ...
                        if (CheckValidMPResult["MembersList"] && CheckValidMPResult["MembersList"].length > 0) {
                            ...
                            if (CheckValidMPResult["MembersList"].length == 1 && (glbMemberId == CheckValidMPResult["MembersList"][0]["ServerMemberID"] || CheckValidMPResult["MembersList"][0]["ServerMemberID"] == "" || (CheckValidMPResult["MembersList"][0]["ServerMemberID"] == 0 && CheckValidMPResult["MembersList"][0]["RegStatus"] == "Online"))) {
                                if (checkValidString(CheckValidMPResult["MembersList"][0]["ExpirationDate"])) {
                                    if (isValidExpriyDate(CheckValidMPResult["MembersList"][0]["ExpirationDate"]) || ((CheckValidMPResult["MembersList"][0]["IsGracePeriod"]) == true) || ((CheckValidMPResult["MembersList"][0]["IsGracePeriod"]) == "true")) {
                                        removeProgressView();
                                        if (isFromEditProfile) {
                                            frmEditMemberProfile.txtSubID.text = getMPAndDateInSixDigitFormate(data, CheckValidMPResult["MembersList"][0]["ExpirationDate"]);
                                            //**6638
                                            if (glbMeetingNum == "") {
                                                kony.print("Enter for No Meeting Edit Profile Scenario");
                                                if (CheckValidMPResult["MembersList"][0] != undefined && CheckValidMPResult["MembersList"][0] != null && checkValidString(CheckValidMPResult["MembersList"][0].EnterpriseID) && CheckValidMPResult["MembersList"][0].EnterpriseID != "0" && checkValidString(CheckValidMPResult["MembersList"][0].UserName) && checkValidString(CheckValidMPResult["MembersList"][0].EmailID)) {
                                                    kony.print("Set Link Info in No Meeting Edit Profile Scenario");
                                                    linkChangeDetected = true;
                                                    setLinkInfoForOnlineMember(CheckValidMPResult["MembersList"][0]);
                                                }
                                            }
                                            callbackEdit.call(null);
                                            return true;
                                        ...
                                        if (CheckValidMPResult["MembersList"][0]["ProgramDuration"] == "3" && typeofSub == kony.i18n.getLocalizedString("strMPRedeem")) {
                                            ...
                                            if (Flow.toUpperCase() == "ENROLL") {
                                                if (in_array(deviceLocale, Countries["CA"]) && (CheckValidMPResult["MembersList"][0]["RegStatus"] == 'Online')) whrCondition = "where Action='" + action + "' and Subscription='" + Subcriptions[8] + "' and Transact='" + Flow + "' and RegistrationStatus = 'ONLINE'";
                                                else whrCondition = "where Action='" + action + "' and Subscription='" + Subcriptions[8] + "' and Transact='" + Flow + "' and RegistrationStatus <> 'ONLINE'";
                                            } else {
                                        ...
                                        if (CheckValidMPResult["MembersList"][0]["ProgramDuration"] == "6" && typeofSub == kony.i18n.getLocalizedString("strMPRedeem")) {
                                            ...
                                            if (Flow.toUpperCase() == "ENROLL") {
                                                if (in_array(deviceLocale, Countries["CA"]) && (CheckValidMPResult["MembersList"][0]["RegStatus"] == 'Online')) whrCondition = "where Action='" + action + "' and Subscription='" + Subcriptions[10] + "' and Transact='" + Flow + "' and RegistrationStatus = 'ONLINE'";
                                                else whrCondition = "where Action='" + action + "' and Subscription='" + Subcriptions[10] + "' and Transact='" + Flow + "' and RegistrationStatus <> 'ONLINE'";
                                            } else {
                                                whrCondition = "where Action='" + action + "' and Subscription='" + Subcriptions[1] + "' and Transact='" + Flow + "'";
                                            }
                                        }
                                        if (CheckValidMPResult["MembersList"][0]["PlanType"] == 'LTC' && in_array(CheckValidMPResult["MembersList"][0]["CommitmentDuration"], ['3', '6', '12'])) {
        ...
        if (isFromEditProfile && glbMeetingNum == "") boPreActivation.searchMemberForDuplicateMPWSCall(memberId, couponCode, checkForValidMPViaWSCallback);
        else boPreActivation.searchMemberWSCall(couponCode, checkForValidMPViaWSCallback);
    },
    ...
    checkPersonalGoalWeightAvailability: function() {
        var whereClause = " where MemberID = '" + glbMemberId + "' AND MemberID <> '0' ";
        ...
        function checkPersonalGoalWeightAvailabilitySuccessCallback(res) {
            if (res.length > 0) {
                ...
                if (resultDate != undefined && resultDate != "" && resultDate != null && resultDate != "0001-01-01T00:00:00") personalGoalDate = resultDate;
            } else {
                isPersonalGoalWeight = false;
            }
        }
    ...
    getWeightEntries: function(callback) {
        ...
        var whereClause = "where MemberID = '" + glbMemberId + "' AND date(WeighInDate) > '" + lastPromptDate + "' AND MemberID <> '0' ";
    ...
    getDollerPriceValue: function(whereCond, subsType, action, callback) {
        ...
        var priceQuery = (glbIsTaxCollected == 'true' || glbIsTaxCollected == '1') ? 'ifnull(O.DiscountedProductPrice + O.OfferUnitPriceTax,P.UnitPrice+P.UnitPrice*P.TaxRate)' : 'ifnull(O.DiscountedProductPrice,P.UnitPrice)';
        if (glbIsStarterFeeFeatureEnable) {
    ...
    getOriginalUnitPrice: function(sku, callback) {
        var query = "select UnitPrice from ProductDetail where sku = '" + sku + "'"; //** Added inverted comma as getting sql erro 7019 error for F14
    ...
    getCCTranSalePaymentForMember: function(queryType, callback) {
        var transactionSQLQuery = "SELECT * from SalePayments WHERE SaleTransactnId  IN (Select C.SaleTransactnId from SalePayments C INNER JOIN  SaleDetails B " + " ON B.SaleTransactnId = C.SaleTransactnId WHERE B.MeetingOccurID = '" + glbMeetingNum + "' AND " + " B.MeetingDate = '" + glbMeetingDate + "' AND B.MemberId = '" + glbMemberId + "' AND B.IsSaleVoid = 0)";
        ...
        function getCCTranSalePaymentForMemberSuccessCallback(res) {
            ...
            if (res.length > 0) {
                if (queryType == "count") callback.call(null, true);
                else callback.call(null, res);
            } else {
    ...
    updateMemberEmail: function(updateObj, frmPage) {
        kony.print("ABA -- > updateMemberEmail" + JSON.stringify(updateObj));

        function updateMemberEmailSuccessCallback(res) {
            kony.print("in updateMemberEmail");
            if (frmPage == "DirectSale") kony.print("Email saved");
            else getCCTransactionsForMember("updatedEmail", updateObj.Email);
        }
        var whrClause = " WHERE MemberID = '" + updateObj.MemberID + "'";
        kony.print("ABA --> Where clause is : " + whrClause);
        //Change to 1 - if member mtngoccrid other than present mtngoccrid
        updateObj.EmpID = glbEmployeeId;
        updateObj.Locale = deviceLocale;
        updateObj.CountryID = getCountryID();
        //Update Defaults
        updateMemberDetailsSetDefaults(updateObj, successupdateEmailSuccessCallback);

        function successupdateEmailSuccessCallback(updateObj) {
    ...
    getMemberIDByRegNumber: function(WeighMemberIDs, WeighWeekNumber) {
        var updateObj = {};
        //updateObj.IsMemberAtRisk = ''; MEG 6810
    ...
    checkActiveMPMember: function(couponcode, isExpired, callback) {
        function checkActiveMPMemberCallback(status, CheckActiveMPResult) {
            try {
                if (status == 400) {
                    if (CheckActiveMPResult["opstatus"] == 0) {
                        kony.print("::DEBUG::CheckActiveMPResult::" + JSON.stringify(CheckActiveMPResult["MembersList"]));
                        removeProgressView();
                        if (CheckActiveMPResult["MembersList"] && CheckActiveMPResult["MembersList"].length > 0) {
                            if (isValidExpriyDate(CheckActiveMPResult["MembersList"][0]["ExpirationDate"])) {
                                callback.call(null, true);
                            } else {
                                callback.call(null, false);
                            }
        ...
        if (checkIfNetworkIsAvailable()) {
            boPreActivation.searchMemberWSCall(couponcode, checkActiveMPMemberCallback);
        } else if (isExpired) {
            callback.call(null, false);
        } else {
            callback.call(null, true);
    ...
    getSubsidyAmtBysaleTranID: function(saleTransId, callback) { //**MEG 7290
        var SQLQuery = "SELECT SubsidyAmount from SaleDetails WHERE SaleTransactnId = '" + saleTransId + "'";
        kony.print("SQLQuery : " + SQLQuery);

        function getSubsidyAmtBysaleTranIDSuccessCallback(res) {
            kony.print("In getSubsidyAmtBysaleTranIDSuccessCallback" + JSON.stringify(res));
            if (res && res.length > 0) {
                kony.print("SubsidyAmount " + res[0].SubsidyAmount);
                callback.call(null, res[0].SubsidyAmount);
            } else {
                callback.call(null, 0);
            }
        }

        function getSubsidyAmtBysaleTranIDeventErrorCallBack(res) {
            kony.print("getSubsidyAmtBysaleTranIDeventErrorCallBack " + JSON.stringify(res));
            callback.call(null, 0);
        }
        kony.sync.single_select_execute(kony.sync.getDBName(), SQLQuery, null, getSubsidyAmtBysaleTranIDSuccessCallback, getSubsidyAmtBysaleTranIDeventErrorCallBack);
    }
};