...
function onPostShowgetProductsAddWeighScreen() {
    kony.print("onPostShowgetProductsAddWeighScreen selectedKeyForReason " + selectedKeyForReason);
    selectedKeyForReason = null; //** MEG 7192
    ...
    try {
        ...
        if (in_array(deviceLocale, Countries["CA"])) getProductsFromTable(productCategoryEnum[2]);
        else getProductsFromTable(productCategoryEnum[1]);
        frmDirectSaleProductList.txtSearchProduct.text = "";
        if (in_array(deviceLocale, Countries["CA"])) kony.application.getCurrentForm().lblCatType.text = getLocalizedString("strAllProduct");
        else kony.application.getCurrentForm().lblCatType.text = getLocalizedString("strFeatured");
    } catch (e) {
        removeProgressView();
        GlobalException(e);
    }
...
function bindCategories(categories) {
    ...
    else popupCategoryType.pickerCategories.selectedKeys = ["0"];
...
function eventonClickDoneImageHeaderCategoryType() {
    ...
    if (selectedCategory == getLocalizedString("strFeatured")) getProductsFromTable(productCategoryEnum[1]);
    else if (selectedCategory == getLocalizedString("strAllProduct")) getProductsFromTable(productCategoryEnum[2]);
    else getProductsFromTable(selectedCategory);
}
...
function eventonClickDoneImageHeaderProductOfferType() {
    ...
    if ((offer == "1" || offer == "3") && isServiceProvider) {
        // discountval = "";
        ...
        if (offer == 1) {
            discountval = "";
            offerdesc = kony.i18n.getLocalizedString("strIncorrectPrice"); //"Incorrect Price";
            offerno = 1;
        }
        if (offer == 3) {
            discountval = "";
            offerdesc = kony.i18n.getLocalizedString("strConsumablesBundle"); //** MEG 7243
            offerno = 3;
        }
        //** End
    } else if (offer == "1" || offer == "2" || offer == "3") { //Selected Incorrect Price or Customer Coupon
        ...
        if (offer == 2) {
            offerdesc = kony.i18n.getLocalizedString("strCustomerCoupon"); //"Customer Coupon";
            offerno = 2;
        }
        if (offer == 3) {
            offerdesc = kony.i18n.getLocalizedString("strConsumablesBundle"); //**MEG 7243
    ...
    if (offer !== "0") {
        if (offerdesc == kony.i18n.getLocalizedString("strIncorrectPrice") /*"Incorrect Price"*/ || offerdesc == kony.i18n.getLocalizedString("strCustomerCoupon") /*"Customer Coupon"*/ || offerdesc == kony.i18n.getLocalizedString("strConsumablesBundle")) { //** MEG 7243 //** MEG 7037
            discountedprice = "";
            offertax = "";
        }
        ...
        if (!isServiceProvider && offer != "1" && offer != "2" && offer != "3") { //**MEG 7037
        ...
        } else if (openFlag && (offer == "1" && offer == "2" && offer == "3")) { //**MEG 7037
            popupItemDetails.txtUnitPrice.text = "";
            popupItemDetails.txtUnitPrice.setEnabled(true);
...
function showProductDetailsData() {
    ...
    if (priceOfProduct != objData.lblPrice) {
        if (isServiceProvider) {
            ...
            if (currPrice.toFixed(2) != (origPrrice * discountforServiceProvider).toFixed(2)) {
                ...
                popupItemDetails.txtUnitPrice.text = glbCurrency + displayPriceByLocale(priceOfProduct); //** 7210
            }
        } else {
    ...
    if (!isEmptyObject(objData) && objData.hidOfferDesc != "") {
        ...
        if (!isServiceProvider && selectedOfferForProduct.OfferDesc != kony.i18n.getLocalizedString("strIncorrectPrice") /*"Incorrect Price"*/ && selectedOfferForProduct.OfferDesc != kony.i18n.getLocalizedString("strCustomerCoupon") /*"Customer Coupon"*/ && selectedOfferForProduct.OfferDesc != kony.i18n.getLocalizedString("strConsumablesBundle")) { //**MEG 7243 //** 7037
            popupItemDetails.txtUnitPrice.text = "";
            popupItemDetails.txtUnitPrice.setEnabled(false);
        }
    } else {
...
function updateProductDetailSubTotal() {
    ...
    var subTotal = 0.0;
    kony.print("1 popupItemDetails.txtUnitPrice.text " + popupItemDetails.txtUnitPrice.text);
    var price = convertPriceToSaveByLocale(popupItemDetails.txtUnitPrice.text);
    kony.print("2 price " + price);
    kony.print("2 popupItemDetails.lblPrice1.text " + popupItemDetails.lblPrice1.text);
    if (price == "" || price == undefined || price == null || price.trim() == ".") {
        price = removeDollar(popupItemDetails.lblPrice1.text);
    }
    price = removeDollar(price);
    kony.print("3 price " + price);
...
function populateOffersforProduct() {
    ...
    try {
        if (!isServiceProvider) {
            ...
            if (in_array(deviceLocale, Countries["CA"])) //** MEG 7037
                temp.push([3, getLocalizedString("strConsumablesBundle")]);
            temp.push(100);
            ...
            if (labelTextOffer == "" || labelTextOffer == null || labelTextOffer == undefined) popupItemDetails.lblOffersData.text = getLocalizedString("strNone");
        }
    } catch (e) {
        GlobalException(e);
    }
...
function successCallbackSearchBySKUorUPC(productResponse) {
    ...
    if (productResponse.length <= 0) {
        ...
        arrData['hidTaxRate'] = parseFloat(productResponse[0]["_TaxRate"]).toFixed(3) //6590
...
function removeCommaFromUnitPriceTextBox(textBox) {
    ...
    //** MEG 7210 //** MEG 7307
    kony.print("popupItemDetails.txtUnitPrice.text " + popupItemDetails.txtUnitPrice.text);
    if (checkValidString(popupItemDetails.txtUnitPrice.text) && removeCurrencyAWS(popupItemDetails.txtUnitPrice.text).toString().length > -1) { //**MEG 7246
        kony.print("inside removeCommaFromUnitPriceTextBox");
        if (removeCurrencyAWS(popupItemDetails.txtUnitPrice.text).toString().length == 0) {
            popupItemDetails.txtUnitPrice.text = "";
        } else {
            popupItemDetails.txtUnitPrice.text = glbCurrency + removeCurrencyAWS(popupItemDetails.txtUnitPrice.text);
        }
    }
    //** End
}
...
function removeCommaFromNonTGUnitPriceTextBox(textBox) {
    ...
    if (removeCurrencyAWS(popupNonTengibleoffers[textBox].text).toString().length > -1) {
        kony.print("inside removeCommaFromNonTGUnitPriceTextBox");
        if (removeCurrencyAWS(popupNonTengibleoffers[textBox].text).toString().length == 0) {
            popupNonTengibleoffers[textBox].text = "";
        } else {
            popupNonTengibleoffers[textBox].text = glbCurrency + removeCurrencyAWS(popupNonTengibleoffers[textBox].text);
        }
    }
}
...
function eventonClickDoneImageHeaderOffersRegion() {
    ...
    if (popupNonTengibleoffers.lblOffersData.text == getLocalizedString("strNone")) {
        ...
        popupNonTengibleoffers.txtUnitPrice.text = glbCurrency + displayPriceByLocale(unitPrice);
...
function eventOnDoneHeaderApplyOffer() {
    try {
        ...
        if (removeCurrencyAWS(popupNonTengibleoffers.txtUnitPrice.text) == "") {
            if (IsAWSLocationEnabled()) {
                alertForMessages(kony.i18n.getLocalizedString("strAlertEnterPrice"));
            } else {
                alertForMessages(kony.i18n.getLocalizedString("strAlertAllowOnlyNumber"));
            }
        } else {
            if (removeCurrencyAWS(popupNonTengibleoffers.txtUnitPrice.text) == ".") {
                popupNonTengibleoffers.txtUnitPrice.text = glbCurrency + "0.0";
            ...
            if (offerDesc != getLocalizedString("strNone")) isDiscount = (checkValidString(offerDesc) ? true : false);
            else if (product.isDiscount == true) isDiscount = true;
            kony.print("isDiscount : " + isDiscount);
            var overrideprice = convertPriceToSaveByLocale(removeCurrencyAWS(popupNonTengibleoffers.txtUnitPrice.text));
            var isOverride = checkValidString(removeCurrencyAWS(popupNonTengibleoffers.txtUnitPrice.text)) ? true : false;
            ...
            //MEG-5839 Price override for AWS
            if (IsAWSLocationEnabled()) {
                var priceOverrideReason = popupNonTengibleoffers.lblReasonData.text;
                var unitprice = removeCurrencyAWS(popupNonTengibleoffers.txtUnitPrice.text);
                var subsidizedDisc = removeCurrencyAWS(popupNonTengibleoffers.txtSubsidyPrice.text);
                kony.print("subsidizedDisc ===" + subsidizedDisc);
                if (subsidizedDisc != "" || subsidizedDisc != 0) {
                    if (parseFloat(subsidizedDisc) > parseFloat(unitprice)) {
                        alert(kony.i18n.getLocalizedString("strSubsidyAmtAlert"));
                        return;
                    }
                    subsidizedDisc = parseFloat(subsidizedDisc).toFixed(2); //**MEG 7194
                    var actualPrice = parseFloat(displayPriceByLocale(removeDollar(frmAddAndWeighMemberTransaction.segSKUDataPOC.selectedItems[0]["lblUnit"]))) + parseFloat(frmAddAndWeighMemberTransaction.segSKUDataPOC.selectedItems[0]["hidSubsidyAmount"]);
                    kony.print("actualPrice ===" + actualPrice);
                    var isPriceOverride = (actualPrice != parseFloat(unitprice)) ? true : false;
                    kony.print("actualPrice isPriceOverride===" + isPriceOverride);
                    if (isPriceOverride && priceOverrideReason == getLocalizedString("strNone")) {
                        alert(kony.i18n.getLocalizedString("strEnterReason"));
                        return;
                    }
                } else {
                    subsidizedDisc = 0;
                }
                var overridePrice = parseFloat(unitprice) - parseFloat(subsidizedDisc);
                isOverride = checkValidString(removeCurrencyAWS(popupNonTengibleoffers.txtUnitPrice.text)) ? true : false;
                tax = parseFloat(overridePrice) * parseFloat(popupNonTengibleoffers.lblTaxRate.text);
                updatedTax = tax.toFixed(2);
                product.SubsidyAmount = subsidizedDisc;
                if (checkValidString(selectedKeyForReason)) {
                    product.ReasonId = selectedKeyForReason
                }
                ProductSale.Cart.update(product, 1, isOverride, overridePrice, isDiscount, 1, 0, (isOverride) ? overridePrice : product.price, priceOverrideReason, updatedTax);
            } else {
                if (offerDesc != getLocalizedString("strNone")) ProductSale.Cart.update(product, 1, isOverride, overrideprice, isDiscount, 1, 0, (isOverride) ? overrideprice : product.price, offerDesc, updatedTax);
                else if (product.isDiscount == true) ProductSale.Cart.update(product, 1, isOverride, overrideprice, isDiscount, product.offerNo, 0, (isOverride) ? overrideprice : product.price, offerDesc, updatedTax, product.offerId);
            }
...
/*
 * Function to display popup for price override reason
 * AWS MEG-5839
 */
function evenOnClickAWSPriceOverRideReason() {
    var context = {
        "widget": popupNonTengibleoffers.hboxSection2,
        "anchor": "right",
        "sizetoanchorwidth": false
    };
    popupAWSPriceOverrideReason.setContext(context);
    if (checkValidString(selectedKeyForReason)) {
        popupAWSPriceOverrideReason.pickerviewPriceOverrideReasons.setSelectedKeyInComponent(selectedKeyForReason);
    }
    popupAWSPriceOverrideReason.show();
}
/*
 * Function enabled or disabled AWS container for the offers
 * MEG-5839
 */
function enabledOrDisabledAWSContainer() {
    if (IsAWSLocationEnabled()) {
        popupNonTengibleoffers.hboxAWSPriceOverride.setVisibility(true);
        popupNonTengibleoffers.HboxAWSSubsidy.setVisibility(true);
    } else {
        popupNonTengibleoffers.hboxAWSPriceOverride.setVisibility(false);
        popupNonTengibleoffers.HboxAWSSubsidy.setVisibility(false);
    }
}

function enabledOrDisabledPriceOverride(allowPriceOverride) {
    //Uncomment below if condition to restrict the isAllowPriceOverride check on SKU from DB
    /*if(IsAWSLocationEnabled()){
    	popupNonTengibleoffers.hbox12443534677112237.setVisibility(true);
    }
    else{*/
    if (allowPriceOverride == "true" || allowPriceOverride == true || allowPriceOverride == 1 || allowPriceOverride == "1") allowPriceOverride = true;
    else allowPriceOverride = false;
    popupNonTengibleoffers.hbox12443534677112237.setVisibility(allowPriceOverride);
    //}
}
/*
 * Function to dismiss price override popup
 * MEG-5839
 */
function dismissPriceOverrideReasonPopup() {
    popupAWSPriceOverrideReason.dismiss();
}

function eventonClickvboxDoneImagePriceOverrideReasonPopup() {
    popupNonTengibleoffers.lblReasonData.text = popupAWSPriceOverrideReason.pickerviewPriceOverrideReasons.selectedKeyValues[0][1];
    selectedKeyForReason = popupAWSPriceOverrideReason.pickerviewPriceOverrideReasons.selectedKeyValues[0][0];
    popupNonTengibleoffers.txtUnitPrice.text = "";
    kony.print("::selectedKeyForReason::" + selectedKeyForReason);
    popupAWSPriceOverrideReason.dismiss();
}