...
function removeCurrency(amount) {
    ...
    if (a.toString().indexOf(glbCurrency) >= 0) {
        ...
        //fr_CA price format changes
        if (deviceLocale == "fr_CA") a = a.replace(",", ".");
    }
    return parseFloat(a);
}

function removeCurrencyAWS(amount) {
    var a = amount;
    if (a.toString().indexOf(glbCurrency) >= 0) {
        a = a.replace(glbCurrency, "");
    }
...
var Product = function(pid, sku, orgprice, img, desc, fulldesc, tax, maxQty, type, taxRate, isPrepaymentPlan, isAllowPriceOverride) {
    this.pid = pid;
    this.sku = sku;
    ...
    this.offerNo = 0;
    this.offerId = 0;
    this.SubsidyAmount = 0; //MEG-5839 AWS
    this.ReasonId = null; //MEG-5839 AWS
    //Added for 4832
    kony.print("Saving taxRate for product : " + taxRate);
    this.taxRate = taxRate;
    this.isPrepaymentPlan = isPrepaymentPlan;
    this.hidIsAllowPriceOverride = (isAllowPriceOverride) ? isAllowPriceOverride : true;
};
var ProductCart = function() {
    ...
    this.total = 0;
    this.totalSubsidy = 0;
    this.defaultOffers = [1, 2, 3]; //** MEG 7037
    ...
    this.empty = function() {
        ...
        _this.defaultOffers = [1, 2, 3]; //** MEG 7037
        _this.calculateTotal();
    };
    this.calculateTotal = function() {
        _this.subTotal = 0;
        _this.taxTotal = 0;
        _this.totalSubsidy = 0;
        var self = _this;
        _.each(_this.products, function(pr) {
            kony.print("pr==" + JSON.stringify(pr));
            self.totalSubsidy += parseFloat(pr.SubsidyAmount);
            ...
            if (glbIsTaxCollected == "true" && pr.isDiscount) {
                ...
                //MEG-6590
                var prTax = (parseFloat(removeCurrency(pr.price)) * parseFloat(removeCurrency(pr.taxRate))).toFixed(3);
                kony.print("SJ----" + prTax);
                prTax = parseFloat(prTax).toFixed(2);
                self.taxTotal += parseFloat((pr.qty * parseFloat(prTax)).toFixed(2));
    ...
    this.changeSegment = function() {
        ...
        _.each(_this.products, function(p) {
            ...
            ProductTaxCust = parseFloat(ProductTaxCust).toFixed(2);
            ...
            if (p.typeOfProduct.toString().toLowerCase() == "tangible") {
                ...
                data.hidTaxTotal = ProductTaxCust * p.qty; //MEG-6590
                ...
                var data = {};
                //data.lblDesc = (IsAWSLocationEnabled())?getAWSProductName(p.sku):p.nameOfProduct; //commented for MEG-7158
                ...
                data.hidTaxRate = p.taxRate;
                data.hidIsPrepaymentPlan = p.isPrepaymentPlan;
                data.hidSubsidyAmount = p.SubsidyAmount;
                data.hidReasonId = p.ReasonId;
                data.hidIsAllowPriceOverride = p.hidIsAllowPriceOverride;
        ...
        if (kony.application.getCurrentForm().id == frmAddAndWeighMemberTransaction.id) {
            ...
            kony.print("NonTangible data present");
            isNonTangibleDataPresent = true;
        }
        ...
        if (_this.products.length > 0) {
            ...
            if (pro) {
                ...
                if (!isServiceProvider) {
                    ...
                    //If Incorrect or Customer coupon or Consumables Bundle
                    if (_.contains(_this.defaultOffers, offerNo)) {
                        if (offerNo == 1) pro.discountDesc = kony.i18n.getLocalizedString("strIncorrectPrice"); //"Incorrect Price";
                        else if (offerNo == 2) pro.discountDesc = kony.i18n.getLocalizedString("strCustomerCoupon"); //"Customer Coupon";
                        else if (offerNo == 3) pro.discountDesc = kony.i18n.getLocalizedString("strConsumablesBundle"); //**MEG 7243 //** MEG 7037
                        pro.offerNo = offerNo;
                        pro.offerId = offerId;
                        pro.isDiscount = true;
                        ...
                        if (isOverride) {
                            ...
                            if (parseFloat(discountTax) > 0) pro.discountTax = pro.tax;
                            else pro.discountTax = discountTax;
                        }
                        //pro.tax = 0;
                        ...
                        if (isOverride) {
                            ...
                            if (parseFloat(discountTax) > 0) pro.tax = discountTax;
                            else pro.tax = pro.tax;
                        }
                        pro.isOverride = false;
                        pro.overridePrice = 0;
                        pro.isDiscount = false;
                    ...
                    else if (offerNo == 3) pro.discountDesc = kony.i18n.getLocalizedString("strConsumablesBundle"); //** MEG 7243 //** 7037
                    pro.offerNo = offerNo;
                    pro.offerId = offerId;
                    pro.isDiscount = true;
                    pro.price = (removeCurrency(price) * discountforServiceProvider).toFixed(2);
                    //Added for 4832
                    if (isOverride) pro.discountTax = checkValidString(discountTax) ? discountTax : pro.tax;
                    else {
                        if (parseFloat(discountTax) > 0) pro.discountTax = (removeCurrency(pro.tax) * discountforServiceProvider).toFixed(2); //pro.tax; //Added for MEGCA-375
                        else pro.tax = (removeCurrency(discountTax) * discountforServiceProvider).toFixed(2); //discountTax; //Added for MEGCA-375
                    }
                } else {
                    pro.isDiscount = false;
                    pro.price = (removeCurrency(price) * discountforServiceProvider).toFixed(2);
                    //Added for 4832
                    if (isOverride) pro.tax = checkValidString(discountTax) ? ((removeCurrency(discountTax) * discountforServiceProvider).toFixed(2)) : ((removeCurrency(pro.tax) * discountforServiceProvider).toFixed(2)); //pro.tax = checkValidString(discountTax)?discountTax:pro.tax; //Added for MEGCA-375
                    else pro.tax = (removeCurrency(discountTax) * discountforServiceProvider).toFixed(2); //discountTax; //Added for MEGCA-375;
                }
...
function onPostShowPopulateOpenPromotion() {
    ...
    if (!isServiceProvider) boAddWeighTransaction.getOpenPromotionProducts(successPromotions);
}
...
function eventOnClickProductInSegment(products, type) {
    ...
    if (type == "NonTangible") {
        ...
        _.each(ntpros, function(pro) {
            var p = new Product(pro.hidProductId, pro.hidSKU, pro.hidPrice, pro.imgEditIcon, pro.lblDesc, pro.lblDesc, pro.hidProductTax, 0, "NonTangible", pro.hidTaxRate, pro.hidIsPrepaymentPlan, pro.hidIsAllowPriceOverride); //Added for 4832 - field : hidTaxRate
...
function eventOnClickPopupItemDetailsUpdate() {
    ...
    if (checkValidString(popupItemDetails.txtUnitPrice.text) && popupItemDetails.txtUnitPrice.text !== ".") {
        isOverride = true;
        newPrice = convertPriceToSaveByLocale(removeDollar(popupItemDetails.txtUnitPrice.text)); //**7210
        //Added for 4832
    ...
    if (selectedOfferForProduct !== null && selectedOfferForProduct !== undefined) {
        if (!isEmptyObject(selectedOfferForProduct) && selectedOfferForProduct.DiscountValue != "0") {
            ...
            //Added for 4832
            var newTax = parseFloat(newPrice) * parseFloat(p.taxRate);
            if (isOverride == false) discountTax = checkValidString(selectedOfferForProduct.OfferTax) ? selectedOfferForProduct.OfferTax : newTax.toFixed(2);
            else discountTax = newTax.toFixed(2);
        } else {
            offerNo = selectedOfferForProduct.OfferNo;
...
function getAWSProductName() {
    kony.print("::glbSelectSubType::" + glbSelectSubType);
    var key = glbSelectSubType.split("-")
    var ProductDesc = {
        "2": kony.i18n.getLocalizedString("strAWSRedeem"),
        "5": kony.i18n.getLocalizedString("strAWSBridge"),
        "4": kony.i18n.getLocalizedString("strAWSEnroll")
    }
    return ProductDesc[key[1]];
}

function getAWSProductSku(sku) {
    kony.print("::glbSelectSubType::" + glbSelectSubType);
    var key = glbSelectSubType.split("-")
    if (key[1] == '4') {
        return 220;
