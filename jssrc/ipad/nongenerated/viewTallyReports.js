...
function onPostShowTallyReports() {
    if (in_array(deviceLocale, Countries["CA"])) {
        ...
        frmTallyReports.hboxThreeMonthLTCPass.setVisibility(false);
        frmTallyReports.hbox3MPLTCActivated.setVisibility(false);
    } else {
        ...
        frmTallyReports.hboxsixMonthPass.isVisible = false; //false; //MEG-6592
        frmTallyReports.hboxMPactivated.isVisible = true;
        frmTallyReports.hbox3MPactivated.isVisible = true;
        frmTallyReports.hbox6MPactivated.isVisible = false; //MEG-6592
        frmTallyReports.hboxSixMonthLTCPass.setVisibility(true); //MEG-6592 MEG-6646
        frmTallyReports.hbox6MPLTCactivated.setVisibility(true); //MEG-6592
        frmTallyReports.hboxThreeMonthLTCPass.setVisibility(true); //MEG-6955
        frmTallyReports.hbox3MPLTCActivated.setVisibility(true); //MEG-6955
    }
...
function showLocationDetailsInHeader() {
    ...
    boTallyMeetingReport.getLocationNameAndNumber(function(res) {
        ...
        if (IsAWSLocationEnabled() || IsInfoSessionMeeting()) {
            frmATWTallyReports.lblHdrLocationName.text = res.TallyLocationName;
            frmATWTallyReports.lblHdrLocationNumber.text = glbLocationNum;
        } else {
...
function onSelectTallyMeetingDate() {
    ...
    if (IsAWSLocationEnabled() || IsInfoSessionMeeting()) {
        var context1 = {
            "widget": frmATWTallyReports.hboxDatePickerPopup,
            "anchor": "right",
    ...
    if (IsAWSLocationEnabled() || IsInfoSessionMeeting()) {
        if (frmATWTallyReports.lblHdrMeetingDatePopup.text != kony.i18n.getLocalizedString("strTallyReportSelectMeetingDate")) {
            var dateThere = frmATWTallyReports.lblHdrMeetingDatePopup.text;
...
function setMeetingDateForTallyMeeting(date, month, year) {
    ...
    //** MEG 5827 	
    if (IsAWSLocationEnabled() || IsInfoSessionMeeting()) {
        frmATWTallyReports.lblHdrMeetingDatePopup.text = dateToShow;
    } else {
    ...
    kony.print("dateSelected ----->+++++++" + dateSelected);
    var d = new Date();
    var todayDate = new Date(d.getFullYear(), d.getMonth(), d.getDate());
    kony.print("todayDate ----->+++++++" + todayDate);
    kony.print("checkIfNetworkIsAvailable ----->+++++++" + !checkIfNetworkIsAvailable());
    if (todayDate.getTime() == dateSelected.getTime() || !checkIfNetworkIsAvailable()) {
        //** DB Call to get meeting cahnged by WS above  fro MEG 7357
        kony.print("ABA -- > In to getAllMeetingsForDateFrom DB");
        boTallyMeetingReport.getAllMeetingsForDate(selectedTallyReportMeetingDate, function(res) {
            setMeetingsInTallyDropdownData(res, false);
            kony.print("getAllMeetingsForDateFromDB popupDateOfBirth.dismiss");
            popupDateOfBirth.dismiss();
        });
    } else {
        kony.print("ABA -- > In to getAllMeetingsForDateFrom WS");
        boTallyMeetingReport.getAllMeetingsForDateFromWS(selectedTallyReportMeetingDate, function(res) {
            kony.print("ABA -- > getAllMeetingsForDateFromWS res " + JSON.stringify(res));
            if (res && res.Meetings) res = res.Meetings;
            setMeetingsInTallyDropdownData(res, true);
            kony.print("getAllMeetingsForDateFromWS popupDateOfBirth.dismiss");
            popupDateOfBirth.dismiss();
        });
    }
    //**End
 ...
 * @returns {} 
 */
function setMeetingsInTallyDropdownData(response, isDataFromWS) {
    kony.print("ABA -- >::setMeetingsInTallyDropdownData::res=" + JSON.stringify(response));
    removeProgressView();
    kony.print("ABA -- > response.length " + response.length);
    if (checkValidString(response.length) && response.length > 0) {
        var Meetings_temp = [];
        var meetingResponse_segmentMeetings_temp = [];
        var bindMeetingDataHomeScreen = [];
        //**MEG 7357
        bindMeetingDataHomeScreen = setMeetingDataForDropDown(response, isDataFromWS);
        ...
        if (isDataFromWS) {
            //** DB Call to get meeting if no meetings found from WS
            kony.print("ABA -- > In to getAllMeetingsForDateFrom DB");
            boTallyMeetingReport.getAllMeetingsForDate(selectedTallyReportMeetingDate, function(res) {
                setMeetingsInTallyDropdownData(res, false);
            });
...
//**MEG 7357
/* This function Set Meetings in dropdown from the response 
 * @param {} response
 * @returns {} 
 */
function setMeetingDataForDropDown(response, isDataFromWS) {
    kony.print("ABA -- > ::setMeetingDataForDropDown ::res=" + JSON.stringify(response));
    kony.print("ABA -- > isDataFromWS " + isDataFromWS);
    // var Meetings_temp = [];
    // var meetingResponse_segmentMeetings_temp = [];
    var mtngTypeId = "";
    var meetingTime = "";
    var meetingStatus = "";
    var tempBindMeetingDataHomeScreen = [];
    if (isDataFromWS) {
        kony.print("ABA -- > In To set WS data");
        for (var i in response) {
            var vData = response[i];
            kony.print("vData:=== " + JSON.stringify(vData));
            mtngTypeId = vData.MeetingTypeID;
            ...
            kony.print("meeting status:== " + vData.MeetingStatus);
            meetingTime = vData.DTCTime;
            meetingTime = getFormatedValue(meetingTime);
            TallyMettingDate = meetingTime;
            tempBindMeetingDataHomeScreen.push({
                ...
                "meetingStatus": "Open",
                "meetingDayTimeCode": vData.DayTimeCodeID,
                "isTimesheetModified": "",
                "meetingDate": vData.MeetingDate,
                "meetingDTCTime": vData.DTCTime,
                "backOfficerRefID": vData.BackOfficeRefID,
                "LineOfBsinessID": vData.LineOfBusinessID,
                "IsAtWork": vData.IsAtWork,
                "SeriesCompMeetings": vData.SeriesCompletedMeetings,
                "SeriesLength": vData.SeriesLength,
                "RenewalNo": vData.RenewalNo,
                "StartDate": vData.StartDate,
                "SeriesNo": vData.SeriesNo,
                "IsInfoSession": vData.IsInfoSession
            });
        }
    } else {
        kony.print("ABA -- > In To set DB data");
        for (var i in response) {
            var vData = response[i];
            kony.print("vData:=== " + JSON.stringify(vData));
            ...
            tempBindMeetingDataHomeScreen.push({
                ...
                "LineOfBsinessID": kony.sync.getString(vData._LineOfBsinessID),
                "IsAtWork": kony.sync.getString(vData._IsAtWork),
                "SeriesCompMeetings": kony.sync.getString(vData._SeriesCompMeetings),
                "SeriesLength": kony.sync.getString(vData._SeriesLength), //**7233
                "RenewalNo": kony.sync.getString(vData._RenewalNo), //**7233
                "StartDate": kony.sync.getString(vData._StartDate), //**7233
                "SeriesNo": kony.sync.getString(vData._SeriesNo), //** 7261
                "IsInfoSession": kony.sync.getString(vData._IsInfoSession)
            });
        }
    }
    kony.print("ABA -- > tempBindMeetingDataHomeScreen " + JSON.stringify(tempBindMeetingDataHomeScreen));
    return tempBindMeetingDataHomeScreen;
}
//**End
/**
...
function onSelectMeetingForTallyReport(CurrMeetingKeyValues) {
    ...
    //** MEG 5827
    if (IsAWSLocationEnabled() || IsInfoSessionMeeting()) {
        frmATWTallyReports.lblCurrMeetingInfoPopup.text = CurrMeetingKeyValues[0].meetingName;
        frmATWTallyReports.lblselectReportType.text = kony.i18n.getLocalizedString("strSelectReportType");
    } else {
        frmTallyReports.lblselectReportType.text = kony.i18n.getLocalizedString("strSelectReportType");
        if (in_array(deviceLocale, Countries["CA"])) {
...
function onClickTallyTabs() {
    ...
    if (IsAWSLocationEnabled() || IsInfoSessionMeeting()) {
        var context = {
            "widget": frmATWTallyReports.vbox674499,
            "anchor": "bottom",
            "sizetoanchorwidth": true
        };
    } else {
...
function checkIfMeetingIsClosedAndPresentInDevice() {
    kony.print("checkIfMeetingIsClosedAndPresentInDevice");
    kony.print("checkIfMeetingIsClosedAndPresentInDevice selectedTallyReportMeetingDate " + selectedTallyReportMeetingDate);
    kony.print("checkIfMeetingIsClosedAndPresentInDevice selectedTallyReportMeetingOccID " + selectedTallyReportMeetingOccID);
    boTallyMeetingReport.getClosedMeetingInfo(selectedTallyReportMeetingDate, selectedTallyReportMeetingOccID, function(res) {
        kony.print("checkIfMeetingIsClosedAndPresentInDevice res " + JSON.stringify(res));
        if (res) {
...
function loadDeviceTallyDataReport() {
    ...
    //**MEG 5827	
    if (IsAWSLocationEnabled() || IsInfoSessionMeeting()) {
        boTallyMeetingReport.getRegion(selectedTallyReportMeetingDate, selectedTallyReportMeetingOccID, DisplayATWTallyReportDataSuccessCallback);
    } else {
...
function setDataToReportTally(status, result) {
    ...
    if (status) {
        ...
        frmTallyReports.lbl3MonthLTCPass.text = result["Section3Num3MonthLTCPassSold"]; //MEG-6955
        frmTallyReports.lblThreeMPLTCActiveCount.text = result["Section3NumThreeMPsLTCActivationSold"]; //MEG-6955
...
function loadMeetingTallyDataReport() {
    boTallyMeetingReport.getMeetingTallyService(selectedTallyReportMeetingDate, selectedTallyReportMeetingOccID, function(status, result) {
        //**MEG 5827
        if (IsAWSLocationEnabled() || IsInfoSessionMeeting()) {
            setDataToReportATWTally(status, result);
        } else {
...
function getMemberAttendanceAndMilestoneData() {
    //** MEG 5827	
    if (IsAWSLocationEnabled() || IsInfoSessionMeeting()) {
        boTallyMeetingReport.getMemberAttendanceAndMilestoneData(selectedTallyReportMeetingDate, selectedTallyReportMeetingOccID, bindDataToMilestoneAttendanceATWReport);
    } else {
...
function bindDataToMilestoneAttendanceReport(status, result) {
    if (status) {
        ...
        frmTallyReports.segAttendMilstoneRep.widgetDataMap = {
            lblAttdReport: "lblAttdReport",
            lblRegNumber: "lblRegNumber",
            lblMemberName: "lblMemberName",
            lblTodayWT: "lblTodayWT",
            lblMemName: "lblMemName",
            lblMilestoneReached: "lblMilestoneReached",
            lblFees: "lblFees",
...
function setMemberSubType(subscriptionType) {
    ...
    if (checkValidString(subType)) {
        if (deviceLocale == "fr_CA") {
            if (subType == "MonthlyPass") subType = getLocalizedString("strMPAbbr");
            else if (subType == "WeeklyPass") subType = getLocalizedString("str20WEEKSAbbr");
            else subType = getLocalizedString("strPAYGAbbr")
        } else {
