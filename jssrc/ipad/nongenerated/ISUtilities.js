...
function clearEditMemberFields(inputVal) {
    ...
    if (inputVal) frmEditMemberProfile.segEditMemberProfileDetails.removeAll();
...
function timeConvert24To12(timeStr) {
    ...
    if (hours < 12) {
        ...
        hours = hours - 12;
        if (hours == 0) hours = 12;
        else hours = (hours.length < 10) ? '0' + hours : hours;
        return hours + ':' + min + ' PM';
    }
}
...
function removeCommaAtEndFromString(strMessage) {
    if (strMessage.slice(-1) == ',') strMessage = strMessage.substr(0, strMessage.length - 1);
    return strMessage;
}
