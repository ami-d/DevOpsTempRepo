...
function completeTrasForAddMember(entryDate,updatedEmailFromMPActivated,flow){
        ...
        setAddMemberSubscriptionData(createMemberDetailsObject, updatedEmailFromMPActivated, entryDate);
...
function onClickCompleteTransaction() {
    ...
    if (IsEnrollMember == FlowForScreens.Enroll || IsAddIndividual == FlowForScreens.AddIndividual) {
        ...
        (IsProductFlowFromSearch) ? currentFlow = "IsProductFlowFromSearch" : currentFlow = "IsProductFlowFromCheckedIn";
    }
    ...
    if (IsSimpleReturn == FlowForScreens.SimpleReturn) {
        var tempObj = {};
        tempObj["Amount"] = 0 - parseFloat(ProductSale.Cart.total);
        ...
        if (amountReturnedType == kony.i18n.getLocalizedString("strCash")) {
            ...
            if (parseFloat(amountReturned) > 0) {
                ...
                tempObj1["Amount"] = 0 - amountReturned;
    ...
    boEnrollMember.insertToSaleTransaction(saleDetailsObject, saleItemsObject, salePaymentObject, callBackCompleteTranAndRedirectToHome);
    ...
    if((frmPayment.imgSendReceipt.src == "icn_checkbox_checked.png") && (email != "") && (email != undefined))
    	sendReceiptEmail(saleItemsObject,salePaymentObject);
...
function getSubscriptionData(SubscriptionID) {
    ...
    var is20WeekBuy = (glbFormName.lblSubType.text == kony.i18n.getLocalizedString("str20WkBuyNew") || frmEditMemberProfile.lblSubTypeInfo.text == "20 Week Pass") ? true : false; //for 20 week buy now
...
function getMPSubcriptionId() {
    ...
    if (glbLastCouponCode !== undefined && glbLastCouponCode != null && glbLastCouponCode.trim().length > 0) {
