var boTallyMeetingReport = {
        ...
        function getTallyMeetingReportCallback(res) {
            ...
            if (res.length > 0) {
                for (var i in res) {
                    ...
                    saleItemUnitPrice = kony.sync.getString(v.UnitPrice);

                    boTallyMeetingReport.setAtandanceAndMettingFeesDetails(saleItemProductSKU, saleItemUnitPrice, kony.sync.getString(v.MemberType));
    ...
    setAtandanceAndMettingFeesDetails: function(SKU, UnitPrice, MemberType) {
    ...
    GetSalesPrepayment: function() {
        ...
        // Commented by Praveen Kalal for starter Fee Implementation
        var whereClause = "select CalcTotal,Quantity,UnitPriceTax from SaleItems s inner join SaleDetails d on s.SaleTransactnId = d.SaleTransactnId where d.CountryID = '" + getCountryID() + "' AND d.MeetingOccurID='" + meetingQuery + "' and d.MeetingDate like '%" + dateQuery + "%' and d.IsServiceProvider = 0 and s.ProductSku not in("+convertArrayToString(getSKUList('QueryCombination'))+") and s.IsSaleItemVoid <> '1' and d.IsReturn <> '1' and (s.ProductType <> 'TangibleProduct' and s.ProductSku in (Select SKU from SKURuleEngine where CountryID = '" + getCountryID() + "'))"; //  group by CalcTotal";
        // Ended  by Praveen Kalal for starter Fee Implementation
    ...
    GetProductSales: function() {
        ...
        var whereClause = "select CalcTotal,UnitPriceTax, Quantity from SaleItems s inner join SaleDetails d on s.SaleTransactnId=d.SaleTransactnId where d.CountryID = '" + getCountryID() + "' AND d.MeetingOccurID='" + meetingQuery +
            "' and d.MeetingDate like '%" + dateQuery + "%' and s.IsSaleItemVoid <> '1' and d.IsReturn <> '1' " +
            "and d.IsServiceProvider = 0 and (s.ProductType = 'TangibleProduct' or s.ProductSku not in (Select SKU from SKURuleEngine where CountryID = '" + getCountryID() + "'))"; //group by CalcTotal";*/
    ...
    GetSalesTax: function() {
        ...
        function GetSalesTaxCallback(res) {
            ...
            if (res.length > 0) {
                for (var i in res) {
                    var v = res[i];
                    boTallyMeetingReport.Section4SalesTax += parseFloat(kony.sync.getString(v.TotalSaleTax));
                }
            }
        ...
        function getSalePaymentCallback(res) {
            ...
            if (res.length > 0) {
                for (var i in res) {
                    ...
                    if (reason != "")
                        boTallyMeetingReport.Section5Reason = reason;
                    else
    ...
    getFinalResult: function(callback) {
        ...
        var Section2TotalPaidAttandace = boTallyMeetingReport.Section2NumOfTotalCurrentAttd + boTallyMeetingReport.Section2NumOfMissedWKCurrAttd + boTallyMeetingReport.Section2NumOfEnrollPayg + boTallyMeetingReport.Section2NumOfPaidLTAttandance + boTallyMeetingReport.Section2NumOfPrepaidAttd + boTallyMeetingReport.Section2NumOfMonthlyPassLTAttd;

        var Section2TotalMemberFees = boTallyMeetingReport.Section2SumOfTotalCurrentAttd + boTallyMeetingReport.Section2SumOfMissedWKCurrAttd + boTallyMeetingReport.Section2SumOfEnrollPayg + boTallyMeetingReport.Section2SumOfPaidLTAttandance;
        Section2TotalMemberFees = parseFloat(Section2TotalMemberFees).toFixed(2);
        var Section2TotalMemberAttandace = boTallyMeetingReport.Section2NumOfTotalCurrentAttd + boTallyMeetingReport.Section2NumOfMissedWKCurrAttd + boTallyMeetingReport.Section2NumOfEnrollPayg + boTallyMeetingReport.Section2NumOfPaidLTAttandance + boTallyMeetingReport.Section2NumOfPrepaidAttd + boTallyMeetingReport.Section2NumOfFreeLTAttandance + boTallyMeetingReport.Section2NumOfMonthlyPassLTAttd+boTallyMeetingReport.Section2NumEmpAttendance;
    ...
    createTallySummary: function(callback) {
        ...
        function createTallySummaryCallback(res) {
            ...
            boTallyMeetingReport.TallySummaryobj = {
                ...
                "MonthlyPassesSold": boTallyMeetingReport.Section3NumMonthlyPassSold + boTallyMeetingReport.Section3Num3MonthPassSold + boTallyMeetingReport.Section3Num6MonthPassSold + boTallyMeetingReport.Section3Num6MonthLTCPassSold,
                "MonthlyPassesActivated": boTallyMeetingReport.Section3NumMPsActivationSold + boTallyMeetingReport.Section3NumThreeMPsActivationSold + boTallyMeetingReport.Section3NumSixMPsActivationSold + boTallyMeetingReport.Section3NumSixMPsLTCActivationSold,
    ...
    createTallyAttendance: function(callback) {
        boTallyMeetingCashout.selectMaxIdFromTable("TallyMeetingFee", function(resID) {
            boTallyMeetingReport.TallyAttendanceAndFeesobj = {
                ...
                "EnrollAtt": boTallyMeetingReport.Section2NumOfEnrollPayg,
                "EnrollAmount": boTallyMeetingReport.Section2SumOfEnrollPayg,
                ...
                "PrepaidAtt": boTallyMeetingReport.Section2NumOfPrepaidAttd,
                ...
                "TotalMemberFees": boTallyMeetingReport.Section2TotalMemberFees,
    ...
    getAllMeetingsForDate: function(meetingDate, callback) {
        ...
        var strWhere = "where locationID = " + glbLocationID + " and DayCodeID = " + daycodeid;
            ...
            try {
                if (status == 400) {
                    if (resultsTallyService["opstatus"] == 0) {
                        if (resultsTallyService["MemberReport"].length > 0) {
    ...
    makeTallyResultFromService: function(resultsTallyService, callback) {
        ...
        for (var i in glbTallyDiffReasonArray) {
            var obj = glbTallyDiffReasonArray[i];
            if (obj[0] == parseInt(TallyPayment.TallyDifferenceReasonID))
                reason = obj[1];
        }
     ...
     GetEmployeeSalesHomeScreen: function(totalAttendanceAndFees,totalProductSales,callback) {
        ...
        function GetEmployeeSalesHomeScreenCallback(res) {
            ...
            if (res.length > 0) {
                ...
                if(totalAttendanceAndFees == undefined)
                	callback.call(null, 0, parseFloat(0).toFixed(2), totalProductEmpSale.toFixed(2));
                else
                	callback.call(null, totalAttendanceAndFees.totalMemberAttandace, totalAttendanceAndFees.totalMemberFees, totalProductEmpSale.toFixed(2));
            }else{
            	if(totalAttendanceAndFees == undefined)
                	callback.call(null, 0, parseFloat(0).toFixed(2), totalProductEmpSale.toFixed(2));
                else
...
//	
	...
	//
        var sqlQuery = "SELECT B.MemberID,sum(UnitPrice) as Fees,B.SubscriptnType, A.Weight,(B.FirstName || ' ' || B.LastName) as MemberName ,B.RegNumber FROM  MemberDetails B INNER JOIN  WeighDetails A on A.MemberID=B.MemberID INNER JOIN SaleDetails S ON S.MemberID = B.MemberID INNER JOIN SaleItems SI ON SI.SaleTransactnId =S.SaleTransactnId AND SI.ProductType ='Fees' where date(S.MeetingDate)= '"+dateQuery+"' AND B.CountryID = '" + getCountryID() + "' AND B.MtngOccrID='" + meetingQuery + "' and A.MemberID != 0 and date(A.WeighInDate) = '"+dateQuery+"' and A.MemberID in (Select MemberID from SaleDetails where CountryID = '" + getCountryID() + "' AND date(MeetingDate) = '"+dateQuery+"' AND TransactionType='NonTangible')  group by S.SaleTransactnId";
