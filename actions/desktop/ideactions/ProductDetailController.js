...
function getAllProductData(){
	...
	if(glbLocationID != ""){
		...
		whrCondition ="select p.ProductID, p.Category, p.CategoryNo, p.IsActive, p.IsMonthlyPass, p.IsNewEnrollment, p.IsSeventeenWkPs, p.IsSeventeenWPEn, p.IsTaxIncluded, p.PosProductType, p.UnitPrice, p.UnitPriceTax, p.description, p.sku, p.TaxRate, p.IsPrepaymentPlan, DiscountedProductPrice from productdetail p " +
			"left join offerdetail o on p.productid = o.productid and o.[AutoApplied] = 'true' and o.LocationID = '"+glbLocationID+"' "+ 
			"and o.offerno in ( select offerno from offerdetail od " + 
			"where  p.productid = od.productid and od.AutoApplied = 'true' and od.LocationID = '"+glbLocationID+"' limit 1) " +
			"where p.LocationID = '"+glbLocationID+"' " +
			"and p.sku in (select SKU from SKURuleEngine)";
	}
	//alert(whrCondition);
    function getAllProductDataSuccessCallback(res){
		...
		if(res.length > 0){
	        for(var i in res){
	            ...
	            //Added for 4832
				if(kony.sync.getString(v.IsTaxIncluded) == "true" || kony.sync.getString(v.IsTaxIncluded) == true)
	            	tempObj["TaxRate"] = kony.sync.getString(v.TaxRate);
	            else 
	            	tempObj["TaxRate"] = 0;
