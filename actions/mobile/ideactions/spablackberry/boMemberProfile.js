...
var boMemberProfile = {
                    ...
                    if (res.length <= 0) {
                        if (!isVoided || isVoided != "true") {
                            ...
                            try {
                                ...
                                if (!isNaN(memberID)) {
                                    callMemberWeightAndMilestone(memberID);
                                }
                            } catch (e) {
                                GlobalException(e);
                            }
    ...
    fillTermMemberInfoObjectInMemberProfile: function(res) {
        ...
        termMemberInfo.LastNteEntrDate = checkUndefined(res.LastNteEntrDate).trim();
    ...
    getMemberProfileWeightDetailsWSCallback: function(status, GetMemberProfile) {
        try {
            ...
            if (status == 400) {
                ...
                var frmMemberProfileDetilsFullHistory_temp = [];
                kony.print("Response===" + JSON.stringify(GetMemberProfile));
                if (GetMemberProfile["MemberWeightList"].length > 0) {
    ...
    insertOnlineWeightDetails: function(v) {
        kony.print("Inserting weight record into local db ::: " + JSON.stringify(v));
