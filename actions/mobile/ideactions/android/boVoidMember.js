...
var boVoidMember = {
    ...
    getPaymentForReturn: function(transaction, callback) {
        ...
        var transactionSQLQuery = "SELECT (select count(1) from saleitems WHERE SaleTransactnId ='" + transaction + "' and IsSaleItemVoid = '1') as Partial, " + "Sum(B.AdjustTotal) as TotalAmount, Sum(B.UnitPriceTax * B.Quantity) as TotalTax," + " A.AuthorizationCode, A.CCLastFourDigits, A.CardType, A.ExpirationDate, A.HasToken, A.InvoiceNumber," + " A.ReferenceNumber, A.RequestId, A.Token, A.TokenExpirationDate, A.TransactionStatus, A.TransactionType, A.IsTrack," + " A.Amount, " + " A.IsIngenicoPayment, " + " A.Type, " + " A.SaleTransactnId " + " FROM SalePayments A " + " INNER JOIN SaleItems B ON B.SaleTransactnId = A.SaleTransactnId " + " AND A.CountryID = '" + getCountryID() + "'" + " AND A.SaleTransactnId ='" + transaction + "'" + " AND B.IsSaleItemVoid=0 " + " INNER JOIN ProductDetail C ON B.ProductID=C.ProductID " + " WHERE (A.RefundAmount ='NULL' " + "OR A.RefundAmount IS NULL " + "OR A.RefundAmount= '' " + "OR A.RefundAmount= 0) " + " AND C.LocationID=" + glbLocationID + " GROUP BY A.Type, " + "A.SaleTransactnId";
