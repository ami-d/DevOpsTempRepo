...
var boPreActivation = {
		...
		if(kony.application.getCurrentForm().id == frmHomeScreen.id){
	        ...
	        if (resRequire) {
	            if (res) {
	                var resSubID = valid.validSubscriptiodIdLength(popupPreActivation.txtVoucherNumber.text, msgtype).isValid();
	                if (resSubID) {
	                	var resAddr=true,resZip=true;
	                	if(checkValidString(popupPreActivation.txtCity.text)){
	                		resAddr = valid
	            			.checkCity(popupPreActivation.txtCity.text)
	            			.isValid();
	            	    }
	            	    if(checkValidString(popupPreActivation.txtZip.text)) {
	            	    	resZip = valid
	            	    	.validateZipCode(popupPreActivation.txtZip.text)
	            	    	.isValid();
	            	    }
    ...
    successBillingInformation: function() {
        kony.print("Succesfully Validates The MP-PASS");
		...
		if(popupPreActivation.checkBoxBillingInfo.src == "icn_checkbox_unchecked.png")
			preActivationObj.sameBillInfo = "icn_checkbox_unchecked.png";
		else
			preActivationObj.sameBillInfo = "icn_checkbox_checked.png";
    ...
    getMPPreActivation : function(preActivation_offerId, preActivation_ProgramDuration, preActivation_SubscriptnType,preActivation_PlanType,preActivation_CommitmentDuration){ //** MEG 6434
		...
		try{
			...
			GetMPPreActivate = Network.makeServiceCall(GetPreActivate_inputparam, boPreActivation.getMPPreActivationSuccess, true);
		}catch(e){
			GlobalException(e);
		}
	...
	checkRequiredInfo : function () {
		...
		if(popupPreActivation.hboxHeight.isVisible){
			resRequire = valid.checkForRequiredFields(popupPreActivation.txtEmail.text, "", "strValidEmail")
					 .checkForRequiredFields(popupPreActivation.txtAddress1.text, "", "strValidAddr1")
					 .checkForRequiredFields(popupPreActivation.txtCity.text, "", "strValidBillingCity")
					 .checkForRequiredFields(popupPreActivation.txtZip.text, "", "strMsgZip")
					 .checkForRequiredFields(popupPreActivation.lblState.text, "", "strSelectState")
					 .checkForRequiredFields(popupPreActivation.txtShippingAdd1.text, "", "strValidShippingAddr")
					 .checkForRequiredFields(popupPreActivation.txtShippingCity.text, "", "strValidShippingCity")
					 .checkForRequiredFields(popupPreActivation.txtShippingZip.text, "", "strValidShippingZip")
					 .checkForRequiredFields(popupPreActivation.lblShippingState.text, "", "strValidShippingState")
					 .checkForRequiredFields(popupPreActivation.lblheight.text, "", "strValidHeight")
					 .checkMemberID(popupPreActivation.txtRegNum.text, "Registration ID",true)
					 .isValid();
        ...
        if (resRequire) {
        	var res;
        	var valid = new validationcls;
        	res = valid.checkEmail(popupPreActivation.txtEmail.text)
            		   .checkCity(popupPreActivation.txtCity.text)
            		   .validateZipCode(popupPreActivation.txtZip.text)
            		   .isValid();
            kony.print("::res::"+res);
...
function populatePreActivationPopup(payMode, isCraditFailure){
		...
		if(IsFromPM == FlowForScreens.ProcessMember)
			popupPreActivation.txtRegNum.text = (glbRegNo == "") ? glbRegNoForProcess : glbRegNo;
		else if(IsEnrollMember == FlowForScreens.Enroll)
			popupPreActivation.txtRegNum.text = frmEnrollNewMember.txtMemberIDInfo.text;
		else if(IsAddIndividual == FlowForScreens.AddIndividual)	
			popupPreActivation.txtRegNum.text = frmAddIndividulaMember.txtMemberID.text;
		else if(IsReEnrollScreen == FlowForScreens.ReEnroll)
			popupPreActivation.txtRegNum.text = frmEnrollReturningMember.txtRegistrationID.text;
		...
		if(preActivationObj.hasOwnProperty('voucherNumber')){
			...
			if(preActivationObj.sameBillInfo == "icn_checkbox_unchecked.png")
