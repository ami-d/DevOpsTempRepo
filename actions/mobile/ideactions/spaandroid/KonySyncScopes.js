if (typeof(com) === "undefined") {
    com = {};
}
if (typeof(com.kony) === "undefined") {
    com.kony = {};
}
if (typeof(com.kony.WeightWatchers) === "undefined") {
    com.kony.WeightWatchers = {};
}
if (typeof(com.kony.WeightWatchers.ProductSyncScope) === "undefined") {
    com.kony.WeightWatchers.ProductSyncScope = {};
}
if (typeof(com.kony.WeightWatchers.ProductSyncScope.ProductSyncScope) === "undefined") {
    com.kony.WeightWatchers.ProductSyncScope.ProductSyncScope = {};
}
//API call will trigger ProductSyncScope reset
com.kony.WeightWatchers.ProductSyncScope.ProductSyncScope.reset = function(successcallback, errorcallback) {
    if (!kony.sync.isSyncInitialized(errorcallback)) {
        return;
    }
    kony.sync.scopeReset("ProductSyncScope", successcallback, errorcallback);
}
if (typeof(com) === "undefined") {
    com = {};
}
if (typeof(com.kony) === "undefined") {
    com.kony = {};
}
if (typeof(com.kony.WeightWatchers) === "undefined") {
    com.kony.WeightWatchers = {};
}
if (typeof(com.kony.WeightWatchers.MeetingUpload) === "undefined") {
    com.kony.WeightWatchers.MeetingUpload = {};
}
if (typeof(com.kony.WeightWatchers.MeetingUpload.MeetingUpload) === "undefined") {
    com.kony.WeightWatchers.MeetingUpload.MeetingUpload = {};
}
//API call will trigger MeetingUpload reset
com.kony.WeightWatchers.MeetingUpload.MeetingUpload.reset = function(successcallback, errorcallback) {
    if (!kony.sync.isSyncInitialized(errorcallback)) {
        return;
    }
    kony.sync.scopeReset("MeetingUpload", successcallback, errorcallback);
}
if (typeof(com) === "undefined") {
    com = {};
}
if (typeof(com.kony) === "undefined") {
    com.kony = {};
}
if (typeof(com.kony.WeightWatchers) === "undefined") {
    com.kony.WeightWatchers = {};
}
if (typeof(com.kony.WeightWatchers.MemberSyncScope) === "undefined") {
    com.kony.WeightWatchers.MemberSyncScope = {};
}
if (typeof(com.kony.WeightWatchers.MemberSyncScope.MemberSyncScope) === "undefined") {
    com.kony.WeightWatchers.MemberSyncScope.MemberSyncScope = {};
}
//API call will trigger MemberSyncScope reset
com.kony.WeightWatchers.MemberSyncScope.MemberSyncScope.reset = function(successcallback, errorcallback) {
    if (!kony.sync.isSyncInitialized(errorcallback)) {
        return;
    }
    kony.sync.scopeReset("MemberSyncScope", successcallback, errorcallback);
}
if (typeof(com) === "undefined") {
    com = {};
}
if (typeof(com.kony) === "undefined") {
    com.kony = {};
}
if (typeof(com.kony.WeightWatchers) === "undefined") {
    com.kony.WeightWatchers = {};
}
if (typeof(com.kony.WeightWatchers.LookUpTables) === "undefined") {
    com.kony.WeightWatchers.LookUpTables = {};
}
if (typeof(com.kony.WeightWatchers.LookUpTables.LookUpTables) === "undefined") {
    com.kony.WeightWatchers.LookUpTables.LookUpTables = {};
}
//API call will trigger LookUpTables reset
com.kony.WeightWatchers.LookUpTables.LookUpTables.reset = function(successcallback, errorcallback) {
    if (!kony.sync.isSyncInitialized(errorcallback)) {
        return;
    }
    kony.sync.scopeReset("LookUpTables", successcallback, errorcallback);
}
if (typeof(com) === "undefined") {
    com = {};
}
if (typeof(com.kony) === "undefined") {
    com.kony = {};
}
if (typeof(com.kony.WeightWatchers) === "undefined") {
    com.kony.WeightWatchers = {};
}
if (typeof(com.kony.WeightWatchers.Logger) === "undefined") {
    com.kony.WeightWatchers.Logger = {};
}
if (typeof(com.kony.WeightWatchers.Logger.Logger) === "undefined") {
    com.kony.WeightWatchers.Logger.Logger = {};
}
//API call will trigger Logger reset
com.kony.WeightWatchers.Logger.Logger.reset = function(successcallback, errorcallback) {
    if (!kony.sync.isSyncInitialized(errorcallback)) {
        return;
    }
    kony.sync.scopeReset("Logger", successcallback, errorcallback);
}
if (typeof(com) === "undefined") {
    com = {};
}
if (typeof(com.kony) === "undefined") {
    com.kony = {};
}
if (typeof(com.kony.WeightWatchers) === "undefined") {
    com.kony.WeightWatchers = {};
}
if (typeof(com.kony.WeightWatchers.MeetingSyncScope) === "undefined") {
    com.kony.WeightWatchers.MeetingSyncScope = {};
}
if (typeof(com.kony.WeightWatchers.MeetingSyncScope.MeetingSyncScope) === "undefined") {
    com.kony.WeightWatchers.MeetingSyncScope.MeetingSyncScope = {};
}
//API call will trigger MeetingSyncScope reset
com.kony.WeightWatchers.MeetingSyncScope.MeetingSyncScope.reset = function(successcallback, errorcallback) {
    if (!kony.sync.isSyncInitialized(errorcallback)) {
        return;
    }
    kony.sync.scopeReset("MeetingSyncScope", successcallback, errorcallback);
}
if (typeof(com) === "undefined") {
    com = {};
}
if (typeof(com.kony) === "undefined") {
    com.kony = {};
}
if (typeof(com.kony.WeightWatchers) === "undefined") {
    com.kony.WeightWatchers = {};
}
if (typeof(com.kony.WeightWatchers.Tally) === "undefined") {
    com.kony.WeightWatchers.Tally = {};
}
if (typeof(com.kony.WeightWatchers.Tally.Tally) === "undefined") {
    com.kony.WeightWatchers.Tally.Tally = {};
}
//API call will trigger Tally reset
com.kony.WeightWatchers.Tally.Tally.reset = function(successcallback, errorcallback) {
    if (!kony.sync.isSyncInitialized(errorcallback)) {
        return;
    }
    kony.sync.scopeReset("Tally", successcallback, errorcallback);
}
if (typeof(com) === "undefined") {
    com = {};
}
if (typeof(com.kony) === "undefined") {
    com.kony = {};
}
if (typeof(com.kony.WeightWatchers) === "undefined") {
    com.kony.WeightWatchers = {};
}
if (typeof(com.kony.WeightWatchers.TallySyncScope) === "undefined") {
    com.kony.WeightWatchers.TallySyncScope = {};
}
if (typeof(com.kony.WeightWatchers.TallySyncScope.TallySyncScope) === "undefined") {
    com.kony.WeightWatchers.TallySyncScope.TallySyncScope = {};
}
//API call will trigger TallySyncScope reset
com.kony.WeightWatchers.TallySyncScope.TallySyncScope.reset = function(successcallback, errorcallback) {
        if (!kony.sync.isSyncInitialized(errorcallback)) {
            return;
        }
        kony.sync.scopeReset("TallySyncScope", successcallback, errorcallback);
    }
    // **********************************Start Scope definition************************
konysyncClientSyncConfig = {
        "ArrayOfSyncScope": [{
            "DataSource": "LocationLocalRest",
            "ScopeName": "ProductSyncScope",
            "ScopeDatabaseName": "MEGDevelopment",
            "Strategy": "OTA_SYNC",
            "ScopeTables": [{
                "Name": "ProductDetail",
                "Columns": [{
                    "Name": "sku",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "UnitPrice",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "description",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ProductID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "Category",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CategoryNo",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "IsActive",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "IsMonthlyPass",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "boolean"
                }, {
                    "Name": "IsNewEnrollment",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "boolean"
                }, {
                    "Name": "IsSeventeenWkPs",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "boolean"
                }, {
                    "Name": "IsSeventeenWPEn",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "boolean"
                }, {
                    "Name": "IsTaxIncluded",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "LocationID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "PosProductType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "UnitPriceTax",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Type",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsFeaturedProduct",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "ProductCategoryDesc",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "UPC",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "TaxRate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsPrepaymentPlan",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["ProductID", "LocationID"],
                "Relationships": {
                    "OneToMany": [{
                        "TargetObject": "OfferDetail",
                        "SourceObject_Attribute": "ProductID",
                        "TargetObject_Attribute": "ProductID"
                    }]
                }
            }, {
                "Name": "OfferDetail",
                "Columns": [{
                    "Name": "AutoApplied",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "DiscountValue",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "OfferDesc",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "ProductID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "OfferNo",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "LocationId",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "DiscountedProductPrice",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "OfferUnitPriceTax",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "StartDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "EndDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "OfferID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["ProductID", "OfferDesc", "LocationId"],
                "Relationships": {}
            }, {
                "Name": "SKURuleEngine",
                "Columns": [{
                    "Name": "Action",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Id",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "AgeGreaterThan",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "IsDefault",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "MemberType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MissedWeekMax",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MissedWeekMin",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "SKU",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Subscription",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Transact",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "WeightDiff",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "LocationId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "LocationNum",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "isPrepaid",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "PriceOverride",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "PriceOverideSKU",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "RegistrationStatus",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["Id"],
                "Relationships": {}
            }, {
                "Name": "MilestoneLookUp",
                "Columns": [{
                    "Name": "MilestoneID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "MilestoneTypeID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MilestoneName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MilestonTypName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MilestoneDesc",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }],
                "Pk_Columns": ["MilestoneID", "CountryID"],
                "Relationships": {}
            }, {
                "Name": "MeetingTypes",
                "Columns": [{
                    "Name": "MeetingTypeName",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "MeetingTypeID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "Length",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }],
                "Pk_Columns": ["CountryID", "MeetingTypeID"],
                "Relationships": {}
            }, {
                "Name": "StatesLookup",
                "Columns": [{
                    "Name": "StateName",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "StateID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "StateCode",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "StateABBR",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }],
                "Pk_Columns": ["StateID", "CountryID"],
                "Relationships": {}
            }, {
                "Name": "TallyDiffReasonLookup",
                "Columns": [{
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "TallyDiffReasonId",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "TallyDiffReasonDesc",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["CountryID", "TallyDiffReasonId"],
                "Relationships": {}
            }, {
                "Name": "PaymentTypeLookup",
                "Columns": [{
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "PaymentTypeDesc",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "PaymentTypeId",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "PaymentDisplayName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["CountryID", "PaymentTypeId"],
                "Relationships": {}
            }]
        }, {
            "DataSource": "LocationLocalRest",
            "ScopeName": "MeetingUpload",
            "ScopeDatabaseName": "MEGDevelopment",
            "Strategy": "OTA_SYNC",
            "ScopeTables": [{
                "Name": "MeetingsUpdate",
                "Columns": [{
                    "Name": "ID",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "MeetingDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "TransactionType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Date",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "meetingid",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["meetingid"],
                "Relationships": {}
            }]
        }, {
            "DataSource": "MemberDS",
            "ScopeName": "MemberSyncScope",
            "ScopeDatabaseName": "MEGDevelopment",
            "Strategy": "OTA_SYNC",
            "ScopeTables": [{
                "Name": "MemberDetails",
                "Columns": [{
                    "Name": "DontContByEmail",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "DateOfBirth",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "Email",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "EmpID",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "FirstName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Gender",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Height",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "LastName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "SubscriptnID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "SubscriptnType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "RegNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "LastUpdatedTime",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "MemberType",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "BillingAddr1",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "BillingAddr2",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "BillingCity",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "BillingCountry",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "BillingState",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "BillingZipCode",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ConsWeightGain",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "CrntLifeTimeSta",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "DontRecvCalls",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "DontCnctPhone",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "DontCnctSMS",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "DontSendCard",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "DontSendCoupon",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "EnrollmentDate",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "date"
                }, {
                    "Name": "FeePaid",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "LastAchvdMStone",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "LastContactDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "LastNteEntrDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "LocationID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MeetingDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "MtngOccrID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "CouponCode",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ExpirationDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "LastUsedDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "ProductID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MissWeekPasses",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "NoWeeksAttended",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "Phone1",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "PhoneType1",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Phone2",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "PhoneType2",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "PrevLifeTimeSta",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "ShippingAddr1",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ShippingAddr2",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ShippingCity",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ShippingCountry",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ShippingState",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "ShippingZipCode",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "StartDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "MemberStatus",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "TransactionType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "WeeksCompleted",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "WeightGain",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "MeetingId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MemTypeUpdateDt",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "long"
                }, {
                    "Name": "RegStatus",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MemberID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "GoalWeight",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "GoalAchDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "StartWeight",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "RegDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "IncompleteData",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "PreRegNumber",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "UniqueID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "UserStsEndPrd",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "UserComments",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "UserStsChngRsn",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsPAYG",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsMemberInMtns",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "EmailID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "EnterpriseID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "LinkType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "UserName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsFreshStart",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "RefreshDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "SessionNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "TrackerID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MaintenanceMode",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "TrackerStartDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "FailedDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "Eligible",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "EligibleDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "PaidLastFee",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "WeightCountMet",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "ReedeemedPasses",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "RedeemedDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "IsDateRedeemed",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsLink",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsCurrentWeekWeighed",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "LastAttendanceDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "MemberRole",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ProgramDuration",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "isVoided",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "PersonalGoalWeight",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "PersonalGoalWeightDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "SubscriptnDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "Usertype",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "PlanType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CommitmentDuration",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["PreRegNumber", "MemberID"],
                "Relationships": {
                    "OneToMany": [{
                        "TargetObject": "WeighDetails",
                        "SourceObject_Attribute": "MemberID",
                        "TargetObject_Attribute": "MemberID"
                    }, {
                        "TargetObject": "MilestoneAchieved",
                        "SourceObject_Attribute": "MemberID",
                        "TargetObject_Attribute": "MemberID"
                    }, {
                        "TargetObject": "MilestoneEligible",
                        "SourceObject_Attribute": "MemberID",
                        "TargetObject_Attribute": "MemberID"
                    }, {
                        "TargetObject": "NoteDetails",
                        "SourceObject_Attribute": "MemberID",
                        "TargetObject_Attribute": "MemberID"
                    }, {
                        "TargetObject": "RefferalDetails",
                        "SourceObject_Attribute": "MemberID",
                        "TargetObject_Attribute": "MemberID"
                    }, {
                        "TargetObject": "SaleDetails",
                        "SourceObject_Attribute": "MemberID",
                        "TargetObject_Attribute": "MemberID"
                    }, {
                        "TargetObject": "PreActivation",
                        "SourceObject_Attribute": "MemberID",
                        "TargetObject_Attribute": "MemberID"
                    }]
                }
            }, {
                "Name": "WeighDetails",
                "Columns": [{
                    "Name": "DailyPtTarget",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "EmpID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "IsAtndgMeeting",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "NoWeighIn",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "ManualWeighIn",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "MeetingDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "MtngOccrID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MilestoneID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ReachedDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "WeekNumber",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "WeighInDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "Weight",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "WeighId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "WeighMSDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Height",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "LocationID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "SessionNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MemberID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "WeightLoss",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "modifiedLast",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsMemberAtRisk",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "WeeklyPointsAllowance",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }],
                "Pk_Columns": ["WeekNumber", "MemberID"],
                "Relationships": {
                    "OneToMany": [{
                        "TargetObject": "MilestoneAchieved",
                        "SourceObject_Attribute": "WeekNumber",
                        "TargetObject_Attribute": "WeekNumber"
                    }]
                }
            }, {
                "Name": "MilestoneAchieved",
                "Columns": [{
                    "Name": "MilestoneID",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "ReachedDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "MemberID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "WeighInDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "LocationNum",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "WeekNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MilestoneName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MeetingID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MemberMilestoneID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ID",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }],
                "Pk_Columns": ["MemberID", "MemberMilestoneID", "ID"],
                "Relationships": {}
            }, {
                "Name": "MilestoneEligible",
                "Columns": [{
                    "Name": "MilestoneID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "TargetWeight",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MemberID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "IsAchieved",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "LocationNum",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ReachedDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "WeekNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "WeighInDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["MilestoneID", "MemberID"],
                "Relationships": {}
            }, {
                "Name": "SaleDetails",
                "Columns": [{
                    "Name": "EmpID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "IsReturn",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "IsSaleVoid",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "LocationID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MeetingDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "MeetingOccurID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "SaleDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "TotalSalePrice",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "double"
                }, {
                    "Name": "TotalSalePriceNew",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "double"
                }, {
                    "Name": "TotalSaleTax",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "double"
                }, {
                    "Name": "SaleTransactnId",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "Response",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "RegNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MemberID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "IsServiceProvider",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "IsReturnTransaction",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "isVoidAllowed",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "TransactionType",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "IsEmployeeSale",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "boolean"
                }, {
                    "Name": "EmployeeNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsPreActivated",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }],
                "Pk_Columns": ["SaleTransactnId", "MemberID", "TransactionType", "CountryID"],
                "Relationships": {
                    "OneToMany": [{
                        "TargetObject": "SaleItems",
                        "SourceObject_Attribute": "SaleTransactnId",
                        "TargetObject_Attribute": "SaleTransactnId"
                    }, {
                        "TargetObject": "SalePayments",
                        "SourceObject_Attribute": "SaleTransactnId",
                        "TargetObject_Attribute": "SaleTransactnId"
                    }]
                }
            }, {
                "Name": "SaleItems",
                "Columns": [{
                    "Name": "ProductID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "AdjustTotal",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "double"
                }, {
                    "Name": "CalcTotal",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "double"
                }, {
                    "Name": "CouponCode",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsUpload",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "IsSaleItemVoid",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "MissWeekPassNo",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "OfrCouponCode",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "OfferId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "PrepaidCoupNo",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "ProductSku",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Quantity",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "OriginalQuantity",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "ReasonCode",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "UnitPrice",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "double"
                }, {
                    "Name": "UnitPriceTax",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "double"
                }, {
                    "Name": "SaleTransactnId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Id",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "ProductType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsReturnItem",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "ReturnQuantity",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "ClientSaleItemId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsFailedMPVoucher",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["Id"],
                "Relationships": {}
            }, {
                "Name": "SalePayments",
                "Columns": [{
                    "Name": "Amount",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "double"
                }, {
                    "Name": "PaymentDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "Type",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "SaleTransactnId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ID",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "RefundAmount",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "IsRefundAmount",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "boolean"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "AuthorizationCode",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CCLastFourDigits",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CardType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ExpirationDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "HasToken",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "InvoiceNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ReferenceNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "RequestId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Token",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "TokenExpirationDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "TransactionStatus",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "TransactionType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsTrack",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "CardSignature",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsIngenicoPayment",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }],
                "Pk_Columns": ["ID"],
                "Relationships": {}
            }, {
                "Name": "NoteDetails",
                "Columns": [{
                    "Name": "EmployeeID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "EmployeeName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "EntryDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "NoteTypeID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Note",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "MemberID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "LocationID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }],
                "Pk_Columns": ["ID", "MemberID", "CountryID"],
                "Relationships": {}
            }, {
                "Name": "RefferalDetails",
                "Columns": [{
                    "Name": "LocationID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MeetingDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "MeetingOccurrenceID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MemberID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "ReferredByMemberID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ReferredByRegistrationNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Type",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["MemberID", "Type"],
                "Relationships": {}
            }, {
                "Name": "PreActivation",
                "Columns": [{
                    "Name": "MemberID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "SaleTransactnId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MtngOccrID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "LocationID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "PreactivationDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "ActivationDate",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "MeetingDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "ActivationStatus",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "CouponCode",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "ExpirationDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "PassType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "PassDuration",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ID",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "IsCurrentSyncData",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["MemberID", "CouponCode", "ID", "ActivationDate", "ActivationStatus"],
                "Relationships": {}
            }]
        }, {
            "DataSource": "LocationLocalRest",
            "ScopeName": "LookUpTables",
            "ScopeDatabaseName": "MEGDevelopment",
            "Strategy": "OTA_SYNC",
            "ScopeTables": [{
                "Name": "Location",
                "Columns": [{
                    "Name": "locationno",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "zip",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "displayvalue",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "lastSelected",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ModifiedLast",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "City",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "StateId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "AreaSiteNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "locationTypeId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsActive",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "LocationID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "IsTaxCollected",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsCreditCardEnabled",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "MPActivationEnabled",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }],
                "Pk_Columns": ["locationno"],
                "Relationships": {}
            }, {
                "Name": "MeetingTypeLookup",
                "Columns": [{
                    "Name": "meetingTypeId",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "countryCode",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "meetingTypeDesc",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["meetingTypeId"],
                "Relationships": {}
            }, {
                "Name": "UserDetailsLookup",
                "Columns": [{
                    "Name": "UserName",
                    "Autogenerated": "false",
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "Password",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "FirstName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "LastName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "EmployeeNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "EmployeeID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ID",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }],
                "Pk_Columns": ["ID"],
                "Relationships": {}
            }, {
                "Name": "MeetingInstance",
                "Columns": [{
                    "Name": "ID",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "MeetingOccID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MeetingDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "MeetingStatus",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "LocationID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "DTCTime",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "DayCodeID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MeetingTypeID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "DayTimeCodeID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["ID"],
                "Relationships": {}
            }, {
                "Name": "SyncLookUp",
                "Columns": [{
                    "Name": "id",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "updatedLast",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "isSyncSuccess",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "LocationID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }],
                "Pk_Columns": ["id"],
                "Relationships": {}
            }, {
                "Name": "ProductMaxQuantity",
                "Columns": [{
                    "Name": "Id",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "ProductSKU",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MaxQuantity",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }],
                "Pk_Columns": ["Id"],
                "Relationships": {}
            }, {
                "Name": "SavedLocation",
                "Columns": [{
                    "Name": "ID",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "locationno",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "EmployeeNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["ID"],
                "Relationships": {}
            }, {
                "Name": "AppSettingLookUp",
                "Columns": [{
                    "Name": "ID",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "ActiveFromDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "CountryId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsActive",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "SettingName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsApplicableToUI",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "LanguageId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "SettingDescription",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "SettingTypeName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["ID"],
                "Relationships": {}
            }]
        }, {
            "DataSource": "LocationLocalRest",
            "ScopeName": "Logger",
            "ScopeDatabaseName": "MEGDevelopment",
            "Strategy": "OTA_SYNC",
            "ScopeTables": [{
                "Name": "Log",
                "Columns": [{
                    "Name": "id",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "user",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "action",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "description",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["id"],
                "Relationships": {}
            }, {
                "Name": "ExceptionDetail",
                "Columns": [{
                    "Name": "id",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "user",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "errortype",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "description",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "errorcode",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "name",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["id"],
                "Relationships": {}
            }, {
                "Name": "MonitorLogging",
                "Columns": [{
                    "Name": "Id",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "MonitorJSON",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "blob"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["Id"],
                "Relationships": {}
            }, {
                "Name": "SyncErrorLog",
                "Columns": [{
                    "Name": "ID",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "SyncObject",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ErrorCode",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ErrorType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ErrorMessage",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Key",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "SyncSessionID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["ID"],
                "Relationships": {}
            }, {
                "Name": "SyncFeedback",
                "Columns": [{
                    "Name": "ID",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "Key",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "RowsDeleted",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "RowsFailedToUpload",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "RowsInserted",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "RowsUpdated",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "RowsUploaded",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "SyncObject",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "SyncSessionID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["ID"],
                "Relationships": {}
            }, {
                "Name": "CcDeviceInfo",
                "Columns": [{
                    "Name": "ID",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "IsIngenico",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "Deviceid",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ReaderName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "ReaderID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "LastPairedTime",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "CreatedTime",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "LastConnectedTime",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["ID"],
                "Relationships": {}
            }]
        }, {
            "DataSource": "LocationLocalRest",
            "ScopeName": "MeetingSyncScope",
            "ScopeDatabaseName": "MEGDevelopment",
            "Strategy": "OTA_SYNC",
            "ScopeTables": [{
                "Name": "Meetings",
                "Columns": [{
                    "Name": "ID",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "string"
                }, {
                    "Name": "BackOfficeRefID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "DTCTime",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "DayCodeID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "DayTimeCodeID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Description",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "DisplayValue",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "EndDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "EndTime",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsPersnlMeeting",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "LeaderID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "LineOfBsinessID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "LocationID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MeetingDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "MeetingNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MtingOcrncStsID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MeetingStatus",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MtingTerminalID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MeetingTypeID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "StartDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "TallyStatus",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "TransactionType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Date",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "isTimesheetModified",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["ID"],
                "Relationships": {}
            }]
        }, {
            "DataSource": "TallyJSON",
            "ScopeName": "Tally",
            "ScopeDatabaseName": "MEGDevelopment",
            "Strategy": "OTA_SYNC",
            "ScopeTables": [{
                "Name": "TallyMeeting",
                "Columns": [{
                    "Name": "Id",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "MeetingId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MeetingDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "Cash",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "Checks",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "CreditCard",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "CreditSlipsRedeemed",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "CreditSlipsIssued",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "DebitCardInteract",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "BankCollected",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "BankDepositSlipNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "DifferenceReason",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "DifferenceAmount",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["Id"],
                "Relationships": {}
            }, {
                "Name": "TallyTimesheet",
                "Columns": [{
                    "Name": "Id",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "MeetingId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MeetingDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "EmpId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "EmpRole",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "EmpNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "EmpName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "TimeIn",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "TimeOut",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "BreakIn",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "BreakOut",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "MeetingSetup",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MentorTraineeName",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["Id"],
                "Relationships": {}
            }, {
                "Name": "TallyMeetingFee",
                "Columns": [{
                    "Name": "id",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "MeetingDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "MeetingOccrID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "CurrentAttNumber",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "CurrentAttAmount",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "MissedAtt",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MissedAmount",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "EnrollAtt",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "EnrollAmount",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "PaidLifeAtt",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "PaidLifAmount",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "PrepaidAtt",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "FreeLifAtt",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "TotalPaidAtt",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "TotalMemberAtt",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "TotalMemberFees",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "LTPrepaidMember",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["id"],
                "Relationships": {}
            }, {
                "Name": "TallySales",
                "Columns": [{
                    "Name": "id",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "MeetingDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "MeetingOccrID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MemberFees",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "Prepayment",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "ProductSales",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "ProductReturn",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "EmployeeSales",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "SalesTax",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "TotalSales",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["id"],
                "Relationships": {}
            }]
        }, {
            "DataSource": "TallyJSON",
            "ScopeName": "TallySyncScope",
            "ScopeDatabaseName": "MEGDevelopment",
            "Strategy": "OTA_SYNC",
            "ScopeTables": [{
                "Name": "TallySummary",
                "Columns": [{
                    "Name": "Id",
                    "Autogenerated": "false",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "MeetingOccurrenceID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "MeetingDate",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "date"
                }, {
                    "Name": "Coupons",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "Staying",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "TenPercent",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "FivePercent",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "Goal",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "Lifetime",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "Losing",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "TotalLoss",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "float"
                }, {
                    "Name": "MonthlyPassesSold",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "TwentyWeekPassesSold",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MonthlyPassesActivated",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "ThreeMonthsPassesActivated",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "OneMonthPassesActivated",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "EmployeeAttendance",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "SixMonthsPassesActivated",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }, {
                    "Name": "SixMonthsPassesLTCActivated",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "int"
                }],
                "Pk_Columns": ["Id"],
                "Relationships": {}
            }, {
                "Name": "TallyInfo",
                "Columns": [{
                    "Name": "Id",
                    "Autogenerated": "true",
                    "IsPrimaryKey": true,
                    "IsNullable": false,
                    "type": "int"
                }, {
                    "Name": "MeetingId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MemberId",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MemberType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "SubType",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "MissweekPass",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Flow",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Sku",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "UnitPrice",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IspaidLifetime",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "IsAtndgMeeting",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "weightloss",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "isSenior",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "isFromMissWeek",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "boolean"
                }, {
                    "Name": "MilestoneID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "Locale",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }, {
                    "Name": "CountryID",
                    "Autogenerated": "false",
                    "IsNullable": true,
                    "type": "string"
                }],
                "Pk_Columns": ["Id"],
                "Relationships": {}
            }]
        }],
        "ArrayOfDataSource": [{
            "ID": "MemberDS",
            "type": "WEBSERVICE"
        }, {
            "ID": "TallyJSON",
            "type": "JSON"
        }, {
            "ID": "LocationLocalRest",
            "type": "JSON"
        }],
        "AppID": "MEGDevelopment",
        "Version": "59178cfeca08c6f3aed921eb4f7069ae63ab6851332c5c67b4e7cd6562818939"
    }
    //**********************************End Scope definition************************