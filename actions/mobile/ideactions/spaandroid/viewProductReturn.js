...
function onSelectRowOfProductReturnsList() {
    ...
    if (isMemberSelected == "members") popupProductReturns.lblTitle.text = kony.i18n.getLocalizedString("strTransactionFor") + " " + frmProductReturns.segPrdReturns.selectedItems[0]["lblFname"] + " " + frmProductReturns.segPrdReturns.selectedItems[0]["lblLname"];
    else popupProductReturns.lblTitle.text = kony.i18n.getLocalizedString("strEmployeeTransaction") + " " + frmProductReturns.segPrdReturns.selectedItems[0]["lblFname"] + " " + frmProductReturns.segPrdReturns.selectedItems[0]["lblLname"];
...
function isAllRowSelected() {
    ...
    if (selectedCnt == totalcnt) return true;
    else return false;
}
...
function voidPaymentwithCC(selectedPrdList, isMulitplePayment) {
    if (checkIfNetworkIsAvailable()) {
        getCreditCardVoidWS(function getCreditCardVoidWSSuccess(msgText, voidCCrequestID) {
...
function getallproductsReturnSuccessCallBack(response, isVoidSuccess) {
    if (isMemberSelected == "members" || isMemberSelected == "employeesale") totalSelectedTranscation--;
    kony.print("productCount found.... = " + JSON.stringify(response));
    kony.print("isVoidSuccess = " + isVoidSuccess);
    if (response.length > 1 && glbIsDisplayCCAmtRefundAlert == true && isVoidSuccess == true) messageText += kony.i18n.getLocalizedString("strProductReturnSuccessMsg");
    else if (response.length == 1 && glbIsDisplayCCAmtRefundAlert == true && isVoidSuccess == true) messageText += "";
    else messageText += kony.i18n.getLocalizedString("strAlertSimpleReturn");
    if (response.length > 0) {
            ...
            if (getPaymentModeStrById(kony.sync.getString(v.Type)) == "CreditCard" && glbIsDisplayCCAmtRefundAlert == true) {
                ...
                if (response.length > 1 && glbIsDisplayCCAmtRefundAlert == true && isVoidSuccess == true) {
                    ...
                    if (deviceLocale == "fr_CA") {
                        ...
                        var returnAmt = " " + addCurrency(parseFloat(kony.sync.getString(v.Amount)).toFixed(2)) + " ";
                        messageText = messageText.replace("###", returnAmt);
                        kony.print("allproductsReturnSuccessCallBack mode messageText==" + messageText);
                    } else {
                        messageText += " " + addCurrency(parseFloat(kony.sync.getString(v.Amount)).toFixed(2)) + " " + kony.i18n.getLocalizedString("strIn") + " " + getPaymentModeStrById(kony.sync.getString(v.Type));
                    }
                }
                messageText += ",";
            }
        ...
        if (totalSelectedTranscation > 0) {
            ...
            if (messageText.slice(-1) == ',') messageText = messageText.substr(0, messageText.length - 1);
...
function onClickVoidProducts(event) {
    ...
    boVoidMember.voidMember(glbMemberIDSelectedForProducReturn, transactionid, isNonTangibleSale, function(msg) {
        ...
        if (msg.slice(-1) == ',') msg = msg.substr(0, msg.length - 1);
