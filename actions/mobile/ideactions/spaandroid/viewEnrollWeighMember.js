    ...
    if (isValidDPT) {
        for (i = 0; i < dtpValue.length; i++) {
            if (dtpValue.charAt(i) == ".") {
                ...
                var number = 0;
                if (decimalIndex == 0 || decimalIndex == "0") number = 0;
                else number = parseInt(dtpValue.substring(0, i));
        ...
        if (res) {
            ...
            if (IsAddIndividual) {
                ...
                if (weightVal >= 60 && weightVal <= 999.9) {
                    ...
                    if (IsDisplayStatisticsOnStartWeight) frmEnrollWeighMember.lblTodayMilestoneInfo.text = "";
                    else frmEnrollWeighMember.lblTodayMilestoneInfo.text = glbMilestoneName; //Added By PK MEGCA-346
                }
            }
...
function openBMINotePopup() {
    try {
        ...
        var context1;
        if (kony.application.getCurrentForm().id == frmEnrollWeighMember.id) context1 = {
            "widget": frmEnrollWeighMember.lblNotes,
            "anchor": "top",
            "sizetoanchorwidth": true
        };
        else if (kony.application.getCurrentForm().id == frmProcessMember.id) context1 = {
