...
var boMonitor = {
    log: function(action, control, data, flow, isFlowCompleted) {
        try {
            if (isMonitorLogEnable == true) {
                var tempDetails = {
                    "EmployeeNumber": glbEmployeeNumber,
                ...
                if (monitorObject['Details'].length > 0) {
    ...
    submitActivityLog: function(isFromDB, Id) {
        ...
        try {
            ...
            if (isFromDB) inputParams["logMonitor"] = JSON.stringify(localMonitorData);
            else inputParams["logMonitor"] = JSON.stringify(monitorObject.Details[0]);
