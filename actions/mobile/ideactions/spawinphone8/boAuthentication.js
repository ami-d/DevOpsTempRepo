...
var boAuthentication = {
    ...
    authenticateViaWS: function(loginname, loginpassword) {
        ...
        loginService_inputparam["pwd"] = loginpassword;
            ...
            if (status == 400) {
                ...
                if (loginService["status"] != undefined && loginService["status"] == "Successful") {
                    ...
                    if (checkIfNetworkIsAvailable()) {
                        ...
                        if (msrInfo) sendMSRValidatedData(msrInfo);
                    }
                } else {
    ...
    authenticateViaWSBeforeSyncCallback: function(status, loginService) {
        try {
            if (status == 400) {
                ...
                if (loginService != undefined && loginService.AuthorizeDetails != undefined && loginService.AuthorizeDetails[0] != undefined && loginService.AuthorizeDetails[0]["status"] != undefined && loginService.AuthorizeDetails[0]["status"] == "Successful") {
                    loginService = loginService.AuthorizeDetails[0];
