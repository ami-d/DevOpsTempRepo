...
var boMeetings = {
    getMeetingsByLocationId: function(locationId) {
         ...
         */
        strWhere = "SELECT M.ID, M.DTCTime, " + "CASE M.MeetingStatus WHEN 'Submitted' THEN 'Disable' " + "ELSE CASE M.TallyStatus WHEN 'LeaderSignOff' THEN 'Close' ELSE IFNULL(MI.MeetingStatus, 'Open') END END AS MeetingStatus, " + "M.MeetingTypeID, M.isTimesheetModified, M.DayTimeCodeID, M.DayCodeID, M.MeetingDate, M.BackOfficeRefID " + "from Meetings M LEFT JOIN MeetingInstance MI ON MI.MeetingOccID == M.ID AND " + "MI.MeetingDate like '%" + dateQuery + "%' AND MI.locationID = " + locationId + " " + "Where M.CountryID = '" + getCountryID() + "' AND M.locationID = " + locationId + " and M.MeetingDate like '%" + dateQuery + "%'";
