var boAddWeighTransaction = {
    getProducts: function(category, callback) {
        ...
        if (category == productCategoryEnum[2]) strWhere = " where P.ProductID is not null AND P.Type = 'TangibleProduct' and P.LocationID=" + glbLocationID + excludeSKUList;
        else if (category == productCategoryEnum[1]) strWhere = " where P.ProductID is not null AND P.Type = 'TangibleProduct' and P.IsFeaturedProduct='true' and P.LocationID=" + glbLocationID + excludeSKUList;
        else strWhere = " where P.ProductID is not null AND P.Type = 'TangibleProduct' and P.ProductCategoryDesc='" + category + "' and P.LocationID=" + glbLocationID + excludeSKUList;
    ...
    ConvertTableToObject: function(res) {
        ...
        if (res !== null) {
            for (var i in res) {
                ...
                //Added for 4832 
                if (res[i].IsTaxIncluded == true || res[i].IsTaxIncluded == "true") obj._TaxRate = res[i].TaxRate;
                else obj._TaxRate = 0;
    ...
    getOffersForProduct: function(productID, callback) {
        ...
        var strWhere = " where ProductID is not null and AutoApplied = 'false' and ProductID =" + productID + " and LocationID = " + glbLocationID + " and date('" + today + "') BETWEEN date(StartDate) and date(EndDate)";
    ...
    getOpenPromotionProducts: function(callback) {
        var today = supportTime(new Date(), "", kony.i18n.getLocalizedString("strDBInsertionDateFormat")); //** MEG 6386
        var sqlQuery = " where AutoApplied = 'true' and LocationID = " + glbLocationID + " and date('" + today + "') BETWEEN date(StartDate) and date(EndDate)";
    ...
    GetBarcodeSKUProduct: function(searchText, callback) {
        ...
        if (numberPattern.test(searchText)) {
            //Number Search
            ProductSQLQuery = "Select Q.MaxQuantity,P.* from ProductDetail P Left Join ProductMaxQuantity Q on P.sku= Q.ProductSKU AND CountryID = '" + getCountryID() + "'" + " Where (UPPER(P.sku)='" + searchText + "' OR UPPER(P.UPC)='" + searchText + "' OR UPPER(P.description) like '%" + searchText + "%') and P.ProductID is not null AND P.Type = 'TangibleProduct' and P.LocationID=" + glbLocationID + excludeSKUList + strExtraWherCat;
        } else {
            //Name Search
            ProductSQLQuery = "Select Q.MaxQuantity,P.* from ProductDetail P Left Join ProductMaxQuantity Q on P.sku= Q.ProductSKU AND CountryID = '" + getCountryID() + "'" + " Where P.description like '%" + searchText + "%' and P.ProductID is not null AND P.Type = 'TangibleProduct'  and P.LocationID=" + glbLocationID + excludeSKUList + strExtraWherCat;
        }
    ...
    getBarcodeOfferforproduct: function(OfferNo, productID, callback) {
        var today = supportTime(new Date(), "", kony.i18n.getLocalizedString("strDBInsertionDateFormat")); //** MEG 6386
        var strWhere = " where ProductID is not null and AutoApplied = 'false' and ProductID =" + productID + " and OfferNo = " + OfferNo + " and LocationID = " + glbLocationID + " and date('" + today + "') BETWEEN date(StartDate) and date(EndDate)";
