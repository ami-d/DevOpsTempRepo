...
var boVoidMember = {
   ...
   // kony.i18n.getLocalizedString("strAlertSimpleReturn")
        ...
        boVoidMember.getPaymentForReturn(boVoidMember.SaleTransactnId, function(message,response) {
			...
			if(glbIsDisplayCCAmtRefundAlert)
			{
				if(checkIfNetworkIsAvailable()){	
					getCreditCardVoidWS(function getCreditCardVoidWSSuccess(msgText)
					{
					...
					//if(message.length == 0)
						...
						var mode = getPaymentModeStrById(kony.sync.getString(p.Type));
	    					if(mode != "CreditCard")
	    						message += " "+glbCurrency +parseFloat(kony.sync.getString(p.Amount)).toFixed(2)+ " " +kony.i18n.getLocalizedString("strIn")+" "+getPaymentModeStrById(kony.sync.getString(p.Type));
    ...
    getPaymentForReturn: function(transaction, callback) {
        ...
        var transactionSQLQuery = "SELECT (select count(1) from saleitems WHERE SaleTransactnId ='" + transaction + "' and IsSaleItemVoid = '1') as Partial, " +
            "Sum(B.AdjustTotal) as TotalAmount, Sum(B.UnitPriceTax * B.Quantity) as TotalTax,"+
            " A.AuthorizationCode, A.CCLastFourDigits, A.CardType, A.ExpirationDate, A.HasToken, A.InvoiceNumber,"+
            " A.ReferenceNumber, A.RequestId, A.Token, A.TokenExpirationDate, A.TransactionStatus, A.TransactionType, A.IsTrack,"+
            " A.Amount, " +
            " A.IsIngenicoPayment, "+
            " A.Type, " +
            " A.SaleTransactnId " +
            " FROM SalePayments A " +
            " INNER JOIN SaleItems B ON B.SaleTransactnId = A.SaleTransactnId " +
            " AND A.CountryID = '" + getCountryID() + "'" +
            " AND A.SaleTransactnId ='" + transaction + "'" +
            " AND B.IsSaleItemVoid=0 " +
            " INNER JOIN ProductDetail C ON B.ProductID=C.ProductID " +
            " WHERE (A.RefundAmount ='NULL' " +
            "OR A.RefundAmount IS NULL " +
            "OR A.RefundAmount= '' " +
            "OR A.RefundAmount= 0) " +
            " AND C.LocationID=" + glbLocationID +
            " GROUP BY A.Type, " +
            "A.SaleTransactnId";
        ...
        kony.sync.single_select_execute(kony.sync.getDBName(), transactionSQLQuery, null, function(response) {
            ...
            if (response.length > 0) {
                ...
                var total = parseFloat(kony.sync.getString(v.TotalAmount));
                ...
                if (isPartial >= 1) {
                    ...
                    if (response.length == 1) {
                        ...
                        if(messageToBeSent.slice(-1) ==  ',')
                        	messageToBeSent= messageToBeSent.substr(0,messageToBeSent.length-1);
                    }
                }
						...
						if(res && res.length > 0){
			    			...
			    			if(checkValidString(MemberData.SubscriptnDate)){
								...
								if(subscriptionDateFormatted == currDate){
	                                ...
	                                //Added for MEG-5267
	                                if((MemberData.LinkType == "Link" || MemberData.LinkType == "UnLink") && MemberData.EnterpriseID > 0 && MemberData.userName != "")
                                		memberObj.IsLink = "false";
                                	else
                                		memberObj.IsLink = "true";
		...
		function getSaleItemDetailsBySaleTranIDCallback(result){
			...
			if(result.length > 0){
				for (var i in result) {
					...
					if(vRes.IsFailedMPVoucher == true || vRes.IsFailedMPVoucher == 'true')
						isFailedMPVoucher = true;
				}
