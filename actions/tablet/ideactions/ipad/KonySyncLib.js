    ...
    for (var i = startposition; i < endposition; i++) {
        ...
        if (MergedWithEIS !== "1" && MergedWithEIS !== "0") {
            ...
            if (!kony.sync.isNullOrUndefined(pkset)) {
                ...
                if (kony.sync.reconcileForeignKey(tx, pkset, pkwcs, tablename) === false) {
                    return false;
                }
            }
            if (isError === false) {
...
kony.sync.deleteRecordsAfterUpload = function(callback) {
    ...
    var isError = false;
    if (kony.sync.OTAChangestobeDeleted.length === 0 || kony.sync.isNullOrUndefined(kony.sync.currentSyncConfigParams[kony.sync.removeAfterUpload])) {
        callback(0);
        return;
    }
    var results = kony.sync.OTAChangestobeDeleted;
    ...
    function myTransactionCallBack(tx) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            if (kony.sync.currentScope[kony.sync.syncStrategy] !== kony.sync.syncStrategy_OTA || kony.sync.checkForDeleteAfterUpload(tablename, scopename) !== true) {
                continue;
            }
            //skipping if error
            if (!kony.sync.isNullOrUndefined(row.__metadata.errorMessage)) {
    ...
    function myErrorCallBack() {
        sync.log.trace("Entering myErrorCallBack");
    ...
    function mySucessCallBack() {
        sync.log.trace("Entering mySucessCallBack");
        kony.sync.OTAChangestobeDeleted = [];
        callback(0);
    }
};
...
kony.sync.checkIntegrityinTransaction = function(tx, rMap) {
    ...
    for (var obj in rMap) {
        for (var key in rMap[obj]) {
            ...
            var sql = "select * from " + obj + " where " + rMap[obj][key].sourceAttribute + " = " + rMap[obj][key].targetAttributeValue + "";
...
kony.sync.removeCascadeHelper = function(tx, srcAttribute, targetAttribute, tbname, wcs, targetObjectRemoveCascade, targetObjectName, isCascade, errorcallback, markForUpload, pkRecord, isLocal) {
...
sync.reset = function(successcallback, errorcallback) {
    ...
    kony.table.insert(dbList, {
        ...
        tableList: [kony.sync.syncConfigurationTableName, kony.sync.chunkMetaTableName, kony.sync.chunkTableName, kony.sync.metaTableName, kony.sync.pendingUploadTableName, "konysyncDIAGNOSTICS"]
    });
...
kony.sync.reconcileForeignKeyForRelationShip = function(tx, setClause, whereClause, tablename, relationshipSet) {
    ...
    if (!kony.sync.isNullOrUndefined(relationshipSet)) {
        for (var i in relationshipSet) {
            ...
            if (!kony.sync.isNullOrUndefined(setClause[relationshipSet[i].SourceObject_Attribute])) {
                var setC = {};
