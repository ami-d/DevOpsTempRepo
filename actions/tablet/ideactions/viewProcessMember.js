...
function getMemberWeighInDataForNWIResponse(isNWIAllowed) {
    ...
    if (isNWIAllowed) { //Allow NWI
        ...
        if (frmProcessMember.imgNoWeighIn.src == "noweighin_disable.png") {
            ...
            if (roundWeightPM == "") {
            	/*if(lastWeighIn != ""){
            		frmProcessMember.txtAreaWeightProcess.text = parseFloat(lastWeighIn);
            	}else */if (CurrentMemberValues != undefined && CurrentMemberValues != null) {
...
function showSubscriptionDetails() {
    try {
        ...
        if (gblSubType == undefined || gblSubType == "" || glbIsSelectedMemberExpired) {
            gblSubType = "PAYG";
        }
...
function notifyNewlyEnrolledMember(isEnrollFlow) {
    if (isEnrollFlow) showNotificationsForNewlyEnrolledMember(0);
        ...
        boEnrollMember.getNewlyEnrolledMember(glbMemberId);
}
...
function onDoneUpdateGoalWeight() {
    ...
    if (res) {
        ...
        boMemberProfile.updateMemberGoalWeight(whrCond, updateObj);
    } else {
        if (error != "") {
...
function lbsToNextMilestone() {
	...
	if (IsFromPM == FlowForScreens.ProcessMember) {
    	...
    	lbsToNextMilestoneList = _.each(lbsToNextMilestoneList,function(a){a.strmilestoneName = a.MilestoneName});
    ...
    if (lbsToNextMilestoneList.length > 0) {
        for (var i in lbsToNextMilestoneList) {
            ...
            var milestoneID = "";
            if (IsFromPM == FlowForScreens.ProcessMember) 
            	milestoneID = lbsToNextMilestoneList[i]["milestoneID"];
            else if (IsAddIndividual == FlowForScreens.AddIndividual)
            	milestoneID = lbsToNextMilestoneList[i]["MilestoneID"];
            isEligible = false;
            kony.print("Milestone name====" + milestoneName.toString());
...
{
	lastWeightFor5Lb = 0;
	var transactionSQLQuery = "SELECT WD.Weight from Milestoneachieved MA " +
	  "INNER JOIN MilestoneLookup ML on MA.MilestoneID = ML.MilestoneID " +
	  "INNER JOIN WeighDetails WD on WD.MemberID = MA.MemberID AND WD.WeighInDate = MA.WeighInDate " +
	  "WHERE MA.memberid = '" + glbMemberId +"' AND ML.MilestoneName = '5lb Award'" +
	  " AND WD.CountryID = '" + getCountryID() + "' ORDER BY WD.WeighInDate Desc limit 0,1";
...
function checkLbsTo5LbMilestone(lastweight,startWeight,currentWeight){
	try 
	{
		...
		var lastRange = _.find(_.range(parseFloat(startWeight), 0, -5), function(num){ return num < parseFloat(lastwt); });
		if(lastRange > currentWeight){
			lastRange = _.find(_.range(parseFloat(startWeight), 0, -5), function(num){ return num < parseFloat(currentWeight); });
		}
...
function onSelectCalender() {
    ...
    for (var i = 0; i < weekDays.length; i++) {
        ...
        var segWeekDatesData = new Array();
    ...
    if (!isMissedWeekPopupShowed) {
        ...
        if (IsAddIndividual == FlowForScreens.AddIndividual) {
            ...
            if (frmAddIndividulaMember.txtWeeksCompleted.text == '' || frmAddIndividulaMember.txtWeeksCompleted.text.length == 0) weeksCompleted = 0;
                ...
                weeksCompleted = parseInt(frmAddIndividulaMember.txtWeeksCompleted.text)
        ...
        boMemberProfile.getMaxWeekNumber(glbMemberId, function(maxWeekNumber) {
            ...
            if (IsFromPM == FlowForScreens.ProcessMember) {
            	...
            	if(MissedWeekWeightData.length > 0){
                    ...
                    var weightData = [{"Weight":frmAddIndividulaMember.txtStartWeight.text,"NoWeighIn":"false","WeighInDate":supportTime(frmAddIndividulaMember.lblStartDateInfo.text, "", "yyyy-mm-ddTHH:NN:SS")},{"Weight":frmEnrollWeighMember.txtWeight.text,"NoWeighIn":"false","WeighInDate":supportTime(new Date(), "", "yyyy-mm-ddTHH:NN:SS")}]
    ...
    if (checkAppSettingEnable(Settings["PG"])) {
        ...
        if (MissedWeekWeightData.length > 0) {
            ...
            if (isDisplayPersonalGoalAlert && !isDisplayPersonalGoalAlertOnce) {
                ...
                if (personalGoalDate == null)
                    showPersonalGoalAlert("strPersonalGoalAlert1stTime");
                else
...
function setSubScriptionInfo(){
	if(in_array(deviceLocale,Countries["US"]) && (in_array(glbFormName.lblSubType.text,[kony.i18n.getLocalizedString("strMPBuyNew"),kony.i18n.getLocalizedString("str3MPBuy"),kony.i18n.getLocalizedString("str6MPBuy")]))){
         glbFormName.txtSubscriptionID.setEnabled(false);
      	 glbFormName.imgsub.isVisible = false;
