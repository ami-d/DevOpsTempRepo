var boTransacations = {
    ...
    getProductsForReturnByMemberID: function(MeetingId, MemberId, isMember, bindProductTransactionsData) {
        var transactionSQLQuery = "SELECT A.Id, A.ProductID, A.ProductSku, B.description, A.Quantity , (A.UnitPrice) as UnitPrice, A.UnitPriceTax, B.Type, " + " ((A.UnitPrice+A.UnitPriceTax )* A.Quantity) as TotalPrice, A.SaleTransactnId,S.TotalSalePrice as SalePrice, S.TotalSaleTax," + " (Select count(*) from PreActivation where SaleTransactnId = A.SaleTransactnId) as isActivated " + " from SaleItems A  " + " inner join ProductDetail B on A.ProductID=B.ProductID and B.LocationID=" + glbLocationID + " inner join SaleDetails S on S.SaleTransactnId = A.[SaleTransactnId] and S.MemberID = '" + MemberId + "'" + " and S.MeetingOccurID = '" + glbMeetingNum + "' and S.MeetingDate like '%" + glbMeetingDate + "%'" + " where S.CountryID = '" + getCountryID() + "' AND A.Quantity >0  " + " and (A.IsSaleItemVoid is null or A.IsSaleItemVoid= '' or A.IsSaleItemVoid= 0) and A.ProductSku not in(" + convertArrayToString(getSKUList('Employee_Attendance')) + ") order by S.SaleDate desc";
