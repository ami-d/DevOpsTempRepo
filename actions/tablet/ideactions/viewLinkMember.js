...
function eventOnClickSegmentSaveLinkMember_org() {
    ...
    if (linkType == "Link") {
        ...
        if (linkButtonText == getLocalizedString("strLink")) {
            ...
            else if (kony.application.getCurrentForm().id == frmBatchAddMember.id)
                frmBatchAddMember.btnUnlink.text = getLocalizedString("strUnlink");
...
function compareName(firstObj, secondObj) {
    ...
    if (firstObj.lblName > secondObj.lblName)
        return 1;
    if (firstObj.lblName == secondObj.lblName) {
        if (firstObj.lblUsername < secondObj.lblUsername)
            return -1;
        if (firstObj.lblUsername > secondObj.lblUsername)
            return 1;
    }
